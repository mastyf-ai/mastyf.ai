import { LOCAL_SEMANTIC_RULES, } from "./local-semantic-rules.js";
import { execPattern } from "./safe-pattern-match.js";
import { extractHttpUrls, isSafeUrlHost } from "./url-allowlist.js";
import { listLearnedRules, reloadLearnedRules as reloadLearnedRulesStore, registerLocalSemanticCacheBust, } from "./learned-rules-store.js";
...
function compileStaticLocalRules() {
    return LOCAL_SEMANTIC_RULES.map((rule) => ({
        ...rule,
        re: new RegExp(rule.regex, "ims"),
    }));
}
function compileLearnedLocalRules() {
...
function getCompiledLocalRules() {
    if (!compiledRules) {
...
/** @internal */
export function resetLocalSemanticRulesForTests() {
    compiledRules = null;
}
...
export function isCoreLocalSemanticEnabled() {
    const v = process.env["MASTYF_AI_LOCAL_SEMANTIC"];
    if (v === "false" || v === "0")
        return false;
    return true;
}
function shouldSkipExfilHit(ruleId, fullText) {
...
export function runLocalSemanticFallback(tool) {
...
//# sourceMappingURL=local-semantic-fallback.js.map