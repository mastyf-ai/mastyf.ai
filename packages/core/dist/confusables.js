/**
 * Unicode TR39 confusables for offline @mastyf_ai/core regex scans.
...
export function foldHomoglyphs(input) {
    let out = "";
...
function shouldApplySource(source) {
...
function parseConfusablesFile(content) {
...
export function getConfusablesData() {
    if (cachedData)
        return cachedData;
    if (loadAttempted) {
...
export function resetConfusablesCache() {
    cachedData = null;
    loadAttempted = false;
}
export function normalizeConfusables(input) {
    const { single, multi } = getConfusablesData();
    if (single.size === 0 && multi.length === 0)
        return input;
    let result = "";
    let i = 0;
...
function resolveUnicodeNormalizeMode(input, unicodeStrict) {
    ...
    unicodeOpsCount += 1;
    if (unicodeOpsCount > unicodeMaxOpsPerMinute())
        return "light";
    return "full";
}
/** @internal */
export function resetUnicodeBudgetForTests() {
    unicodeOpsWindowStart = Date.now();
    unicodeOpsCount = 0;
}
/** Homoglyph fold → TR39 confusables → NFKC (offline regex pre-pass). */
export function normalizeUnicode(input, unicodeStrict = true) {
...
export function hasConfusableDelta(raw, unicodeStrict = true) {
    if (!unicodeStrict)
        return false;
