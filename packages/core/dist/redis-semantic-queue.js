...
export async function tryAcquireRedisSemanticSlot(tenantId) {
    ...
    try {
        ...
        const ok = await client.eval(ACQUIRE_SCRIPT, 2, GLOBAL_KEY, hasTenant ? tenantRedisKey(tid) : GLOBAL_KEY, semanticQueueMax(), semanticPerTenantMax(), hasTenant ? "1" : "0");
        return ok === 1;
    }
    catch {
...
export async function releaseRedisSemanticSlot(tenantId) {
    ...
    try {
        ...
        const tid = tenantId?.trim();
        const hasTenant = Boolean(tid);
        await client.eval(RELEASE_SCRIPT, 2, GLOBAL_KEY, hasTenant ? tenantRedisKey(tid) : GLOBAL_KEY, hasTenant ? "1" : "0");
    }
    catch {
        /* best-effort */
    }
...
/** @internal */
export function resetRedisSemanticQueueForTests() {
    if (redis) {
        redis.disconnect();
        redis = null;
    }
