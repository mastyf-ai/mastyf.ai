...
/** @internal */
export function resetSafeUrlAllowlistCacheForTests() {
    cachedSuffixes = null;
}
export function isSafeUrlHost(hostname, suffixes = loadSafeUrlSuffixes()) {
...
export function findUnsafeUrls(text) {
    const unsafe = [];
    for (const raw of extractHttpUrls(text)) {
        try {
            const hostname = new URL(raw).hostname;
            if (!isSafeUrlHost(hostname))
                unsafe.push(raw);
        }
        catch {
            unsafe.push(raw);
...
export function getSafeUrlSuffixes() {
