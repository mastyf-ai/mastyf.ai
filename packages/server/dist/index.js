...
const server = new Server({ name: 'mastyf-ai', version: process.env.npm_package_version || '2.3.4' }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === 'scan_mcp_tools') {
        try {
            ...
            let transport = "stdio";
            if (args?.serverUrl) {
                tools = await fetchToolsFromHttp({ url: args.serverUrl });
                transport = "http";
            }
            ...
            const result = await scanServer(args?.serverCommand ?? args?.serverUrl ?? 'unnamed', tools, transport, { skipSemantic: args?.skipSemantic ?? false });
    ...
    if (name === 'verify_manifest') {
        try {
            ...
            const result = verifyToolDefinitions(tools, args?.serverName ?? 'scanned-server');
            return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
        }
        catch (err) {
