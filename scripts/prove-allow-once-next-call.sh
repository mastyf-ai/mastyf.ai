...
curl -sf --max-time 5 "${AUTH[@]}" "$BFF/api/gateway/status" >/dev/null || { bad "BFF down"; exit 1; }

DEC1=$(curl -sS -X POST "$BFF/api/gateway/decide" "${AUTH[@]}" -H 'Content-Type: application/json' \
...
ALLOW=$(curl -sS -X POST "$BFF/api/gateway/escalation/resolve" "${AUTH[@]}" -H 'Content-Type: application/json' \
...
DEC2=$(curl -sS -X POST "$BFF/api/gateway/decide" "${AUTH[@]}" -H 'Content-Type: application/json' \
  -d "{\"tool_name\":\"$TOOL\",\"server_name\":\"ci-server\",\"tool_args\":{\"probe\":true},\"write_receipt\":true}")
...
DEC3=$(curl -sS -X POST "$BFF/api/gateway/decide" "${AUTH[@]}" -H 'Content-Type: application/json' \
  -d "{\"tool_name\":\"$TOOL\",\"server_name\":\"ci-server\",\"tool_args\":{\"probe\":true},\"write_receipt\":true}")
