...
/** Legacy env-var bypass — emits a deprecation warning. Replaced by GUARDIAN_CI_TOKEN (signed JWT). */
export function isCiLicenseBypass(): boolean {
  if (process.env['GUARDIAN_CI_BYPASS_LICENSE'] === 'true') {
    Logger.warn(
      '[license] GUARDIAN_CI_BYPASS_LICENSE is deprecated — use GUARDIAN_CI_TOKEN (signed JWT) generated by the maintainer. ' +
        'This bypass will be removed in v3.3.',
    );
    return true;
  }
  return false;
