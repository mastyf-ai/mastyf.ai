...
// ── In-Memory License & API Key Validation Cache ──────────────────────────────
interface CachedKey {
  valid: boolean;
...
try {
  ...
  if (existsSync(policyPath)) {
    policyConfig = yaml.load(readFileSync(policyPath, 'utf8')) as PolicyConfig;
  } else {
    policyConfig = { rules: [], allowlists: [], blocklists: [] } as unknown as PolicyConfig;
  }
} catch {
  policyConfig = { rules: [], allowlists: [], blocklists: [] } as unknown as PolicyConfig;
}
...
async function validateApiKey(key: string): Promise<{ valid: boolean; tier: 'developer' | 'enterprise' }> {
...
app.post('/v1/validate', authMiddleware, async (req: Request, res: Response): Promise<void> => {
...
app.post('/v1/scan-response', authMiddleware, (req: Request, res: Response): void => {
  const t0 = performance.now();
