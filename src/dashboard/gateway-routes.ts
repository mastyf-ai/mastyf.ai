...
import {
  ...
  GatewayError,
} from '../clients/gateway-client.js';
...
let gatewayClientInstance: MastyfGatewayClient | null = null;

export function getGatewayClient(): MastyfGatewayClient {
...
export function setGatewayClient(client: MastyfGatewayClient | null): void {
  gatewayClientInstance = client;
}

/** Load tools from ~/.mastyf/tool-manifests/{server}.json when discovery tools[] is empty. */
async function enrichServersFromManifests(servers: any[]): Promise<number> {
  let enriched = 0;
  try {
    ...
    const loadManifestFile = (file: string): {
    ...
    const mapTool = (t: Record<string, unknown>) => ({
      name: String(t.name),
    ...
    const mapResource = (r: Record<string, unknown>) => ({
    ...
    for (const s of servers) {
      ...
      let loadedTools: Array<Record<string, unknown>> | null = null;
      let loadedResources: Array<Record<string, unknown>> | null = null;
      ...
      for (const cand of candidatesFor(name)) {
        ...
        if (loadedTools && loadedResources) break;
      }
      ...
      if (loadedTools && existingTools.length === 0) {
        ...
        for (const t of loadedTools) {
          if (typeof t.name === 'string') byName.set(t.name, t);
        }
        let merged = 0;
        s.tools = existingTools.map((t: Record<string, unknown>) => {
          const nameKey = String(t.name || '');
          const fromManifest = byName.get(nameKey);
          if (!fromManifest) return t;
          const hasSchema =
            (t.inputSchema && Object.keys(t.inputSchema as object).length > 0) ||
            (t.input_schema && Object.keys(t.input_schema as object).length > 0);
          if (hasSchema) return t;
          const schema =
            (fromManifest.inputSchema as Record<string, unknown>) ||
            (fromManifest.input_schema as Record<string, unknown>) ||
            {};
          if (!schema || Object.keys(schema).length === 0) return t;
          merged += 1;
          return {
            ...t,
      ...
      if (loadedResources && (!Array.isArray(s.resources) || s.resources.length === 0)) {
        s.resources = loadedResources
          ...
          .map(mapResource)
          .filter((r) => r.uri || r.name);
        if (s.resources.length > 0) s.resources_status = 'from_manifest';
  ...
  try {
    ...
    if (pathname === '/api/gateway/status' && method === 'GET') {
      try {
        ...
        const ledger = (status as { ledger?: { chain_integrity?: boolean; escalated?: number } }).ledger;
        const verification = (status as { verification?: { last_verified_at?: number } }).verification;
    ...
    if (pathname === '/api/gateway/protection' && method === 'GET') {
      ...
      let totalServers: number | null = null;
      try {
        const { discoverAllServers } = await import('../fleet/unified-server-registry.js');
        const unified = discoverAllServers();
        if (Array.isArray(unified) && unified.length > 0) {
      ...
      if (totalServers == null) {
        const fromProtection =
          typeof (protection as any)?.total_servers === 'number'
            ? (protection as any).total_servers
            : null;
        totalServers = fromProtection;
      }
      if (totalServers == null && typeof (protection as any)?.servers_count === 'number') {
        totalServers = (protection as any).servers_count;
      }
      const unmediated =
        (protection as any)?.unmediated_count ?? protection?.total_unmediated ?? null;
      const payload: Record<string, unknown> = { ...protection };
    ...
    if (pathname === '/api/gateway/servers' && method === 'GET') {
      let clientServers: any[] = [];
      try {
        const res = await client.servers();
        if (Array.isArray(res)) clientServers = res;
      } catch {
        clientServers = [];
      }
      ...
      servers.forEach((s: any) => {
        ...
        if (Array.isArray(s.args)) {
          s.args = s.args.map((a: string) =>
            typeof a === 'string' && a.includes('/Users/') && a.includes('.mastyf')
    ...
    if (pathname === '/api/gateway/servers/reintrospect' && method === 'POST') {
      ...
      try {
        ...
        if (Array.isArray(resServers)) clientServers = resServers;
      } catch {
        clientServers = [];
      }
      const serverMap = new Map<string, any>();
      for (const s of clientServers) {
        if (s && s.name) serverMap.set(s.name, { ...s });
      }
      try {
        const { discoverAllServers } = await import('../fleet/unified-server-registry.js');
        const unified = discoverAllServers();
      ...
      servers.forEach((s: any) => {
        if (!Array.isArray(s.tools)) s.tools = [];
        s.tools_count = s.tools.length;
        if (s.tools.length === 0) s.tools_status = 'introspection_pending';
        else delete s.tools_status;
      });
      writeJson(res, 200, {
        ok: true,
      ...
      if (serverRoute && method === 'GET') {
        ...
        if (kind === 'receipts') {
          ...
          if (Array.isArray(page?.receipts)) {
            page.receipts = page.receipts.map((r: any, idx: number) => ({
              ...r,
              receipt_id:
                r.receipt_id ||
                r.request_id ||
    ...
    if (pathname === '/api/gateway/policy' && method === 'GET') {
      ...
      try {
        ...
        if (fs.existsSync(yamlPath)) {
          ...
          if (yaml.trim()) {
            (policy as any).policy_yaml = yaml;
            (policy as any).policy_yaml_source = '~/.mastyf/active_policy.yaml';
          }
        }
      } catch {
        /* optional */
    ...
    if (pathname === '/api/gateway/receipts' && method === 'GET') {
      ...
      if (Array.isArray(receiptsPage?.receipts)) {
        receiptsPage.receipts = receiptsPage.receipts.map((r: any, idx: number) => ({
          ...r,
          receipt_id:
            r.receipt_id ||
            r.request_id ||
    ...
    if (
      (pathname === '/api/gateway/receipts/stats' ||
      ...
      const aggregateFromPage = async (gatewayError?: string) => {
        const page = await client.receipts({ limit: 500 }).catch(() => ({ receipts: [] as any[] }));
        const list = Array.isArray((page as any)?.receipts) ? (page as any).receipts : [];
        const now = Date.now();
        const msMap: Record<string, number | null> = {
          '1h': 3600_000,
          '24h': 86400_000,
          '7d': 7 * 86400_000,
          all: null,
        ...
        for (const r of list) {
          ...
          if (Number.isFinite(untilMs) && ts > untilMs) continue;
          ...
          else if (d === 'BLOCK') blocked += 1;
          else if (d === 'ESCALATE') escalated += 1;
          else other += 1;
        }
        return {
      ...
      try {
        ...
        writeJson(res, 200, {
          ...
          source: (stats as { source?: string }).source || 'live-ledger',
        });
      } catch (err: unknown) {
    ...
    if (pathname === '/api/gateway/siem-status' && method === 'GET') {
      const enabled =
        process.env['MASTYF_AI_SIEM_ENABLED'] === 'true' ||
        process.env['MASTYF_AI_SIEM_EXPORT_ENABLED'] === 'true';
      const protocol =
        process.env['MASTYF_AI_SIEM_PROTOCOL'] ||
        process.env['MASTYF_AI_SIEM_EXPORT_FORMAT'] ||
        null;
      const endpointConfigured = Boolean(
        process.env['MASTYF_AI_SIEM_ENDPOINT'] || process.env['MASTYF_AI_SIEM_EXPORT_PATH'],
      );
      writeJson(res, 200, {
        source: 'bff-env',
        enabled,
    ...
    if (pathname === '/api/gateway/system/geo-timezone' && method === 'GET') {
      ...
      const computeUtcOffset = (tz: string): string => {
        try {
      ...
      const computeTzAbbr = (tz: string): string => {
    ...
    if (pathname === '/api/gateway/chat' && method === 'POST') {
      ...
      const contextEvent = body['contextEvent'] as any;
      ...
      try {
        ...
        if (p.status === 'fulfilled') protection = p.value;
        if (srv.status === 'fulfilled') servers = srv.value;
        if (rct.status === 'fulfilled') receipts = rct.value?.receipts || [];
      } catch (err) {
        isLive = false;
      }
      ...
      const chainIntegrity =
        chainRaw === true || chainRaw === 'VALID' || chainRaw === 'valid'
          ? 'VALID'
      ...
      const totalReceipts =
        status?.ledger?.total_receipts != null
...
7. Format your output using clean GitHub-flavored Markdown with bold headers, concise bullet points, and code/policy blocks where helpful.`;
      ...
      try {
        ...
        if (ollamaRes.ok) {
          ...
          const isClassifierRefusal = rawAnswer && (
            rawAnswer.includes("don't have access to the internal decision-making log") ||
      ...
      if (!answer) {
        ...
        const recent = receipts.slice(0, 5).map((r: any) => {
...
Start Ollama / configure a local model for conversational answers. Until then, use Activity, Audit, and Explain on a real receipt id.`;
    ...
    if (
      (pathname === '/api/gateway/escalation/allow' ||
      ...
      let action:
        ...
        | 'destination'
        | 'tool'
        | 'quarantine' = pathname.endsWith('/block')
        ? 'block'
      ...
      if (pathname.endsWith('/resolve') && allowedActions.has(rawAction)) {
        action = rawAction as typeof action;
      }
      try {
    ...
    if (pathname === '/api/gateway/lockdown/release' && method === 'POST') {
      ...
      try {
        writeJson(
          res,
          200,
          await client.lockdownRelease(controlId, body['operator'] ? String(body['operator']) : undefined),
        );
      } catch (err: unknown) {
        if (err instanceof GatewayUnavailableError || err instanceof GatewayVersionMismatchError) {
          writeJson(res, err.statusCode || 503, { error: err.message, code: err.code });
    ...
    if (pathname === '/api/gateway/attack-lab/owasp-mcp' && method === 'GET') {
      const loadLocalCatalog = async (): Promise<Record<string, unknown> | null> => {
        try {
          ...
          for (const path of candidates) {
            ...
            return {
              ...
              pass_rate: null,
              pass_rate_note:
                'UNAVAILABLE — never invent; run live canaries / Attack Lab probes separately',
              source: path,
    ...
    if (pathname === '/api/gateway/escalations' && method === 'GET') {
      const u = new URL(req.url || '', 'http://localhost');
      const st = (u.searchParams.get('status') || 'open') as 'open' | 'resolved' | 'all';
      try {
    ...
    if (pathname === '/api/gateway/alerting-status' && method === 'GET') {
      try {
        const { isAppAlertingConfigured, getAlertDestinationsForLogging } = await import(
          '../alerting/alert-env.js'
        );
        writeJson(res, 200, {
          ...
          rules: {
            ...
            decide_p95:
              'MastyfDecideP95High (mastyf_ai_decide_latency_ms) + in-app SLO (n≥5, 800ms)',
            escalation_backlog:
              'MastyfEscalateBacklog + in-app SLO — mastyf_ai_escalate_backlog',
            chain_integrity:
              'MastyfLedgerChainBroken + in-app SLO — mastyf_ai_ledger_chain_valid',
            self_test_stale:
              'MastyfSelfTestStale + in-app SLO — mastyf_ai_self_test_age_seconds > 86400',
          },
          alertmanager_rules_file: 'deploy/observability/alert-rules.yml',
          note:
            'Destinations from ALERT_SLACK_WEBHOOK / ALERT_PAGERDUTY_KEY. Threshold rules: deploy/observability/alert-rules.yml via pnpm obs:up',
    ...
    if (pathname === '/api/gateway/observability-status' && method === 'GET') {
      ...
      const otelConfigured = Boolean(
        otelEndpoint || process.env['OTEL_ENABLED'] === 'true' || process.env['MASTYF_OTEL_ENABLED'] === 'true',
      );
      const metricsPort = process.env['METRICS_PORT'] || (process.env['METRICS_ENABLED'] === 'true' ? '9090' : null);

      async function probe(url: string): Promise<{ ok: boolean; status: number | null; detail: string }> {
        try {
          const ac = new AbortController();
          const t = setTimeout(() => ac.abort(), 1500);
          const r = await fetch(url, { signal: ac.signal }).catch(() => null);
          clearTimeout(t);
          if (!r) return { ok: false, status: null, detail: 'UNAVAILABLE — unreachable' };
          return {
      ...
      writeJson(res, 200, {
        ...
        grafana_trace_url:
          process.env['NEXT_PUBLIC_MASTYF_GRAFANA_TRACE_URL'] || LOCAL_OBS_GRAFANA_TRACE_URL,
    ...
    if (pathname === '/api/gateway/incidents' && method === 'GET') {
      try {
        ...
        const limit = Math.min(
          200,
          Math.max(1, Number(new URL(req.url || '', 'http://localhost').searchParams.get('limit') || 50)),
        );
      ...
      try {
        ...
        const incident = {
          ...
          threat_candidate_id: body['threat_candidate_id']
            ...
            : null,
          policy_generation:
            typeof body['policy_generation'] === 'number' ? body['policy_generation'] : null,
    ...
    if (pathname === '/api/gateway/spend-caps' && method === 'GET') {
      try {
        ...
        const tenant =
          String(
            new URL(req.url || '', 'http://localhost').searchParams.get('tenant') || 'default',
          ).trim() || 'default';
    ...
    if (pathname === '/api/gateway/fleet/posture-digest' && method === 'GET') {
      try {
        ...
        const pol = policy as Record<string, unknown> | null;
        const policyHash = String(
          (pol as { hash?: string } | null)?.hash ||
            (st as { policy?: { hash?: string } } | null)?.policy?.hash ||
            'UNAVAILABLE',
        );
        const receiptHead =
          (st as { ledger?: { head_receipt_hash?: string } } | null)?.ledger?.head_receipt_hash ||
          'UNAVAILABLE';
        let certSet = 'none';
        try {
          const file = join(homedir(), '.mastyf', 'certified_catalog_digest.txt');
          if (existsSync(file)) certSet = readFileSync(file, 'utf8').trim().slice(0, 64) || 'none';
        } catch {
          certSet = 'none';
        }
    ...
    if (pathname === '/api/gateway/decide' && method === 'POST') {
      ...
      try {
        ...
        const decideBody: Record<string, unknown> = { ...body };
    ...
    if (pathname === '/api/gateway/evidence-pack' && method === 'GET') {
      try {
        ...
        const [status, protection, policy, receiptsPage, controlPage, serversStats, manifest, windowStats] =
          await Promise.all([
            client.status().catch(() => null),
            ...
            windowParam
              ? client.receiptStats({ window: windowParam }).catch((err: unknown) => ({
                  error: err instanceof Error ? err.message : String(err),
                  window: windowParam,
                }))
              : Promise.resolve(null),
          ]);
        const receiptList = Array.isArray((receiptsPage as { receipts?: unknown[] })?.receipts)
          ? (receiptsPage as { receipts: unknown[] }).receipts
          : [];
        const controlList = Array.isArray((controlPage as { receipts?: unknown[] })?.receipts)
          ? (controlPage as { receipts: unknown[] }).receipts
          : Array.isArray((controlPage as { control_receipts?: unknown[] })?.control_receipts)
            ? ((controlPage as unknown) as { control_receipts: unknown[] }).control_receipts
            : [];
        const narrativeStops = (receiptList as Array<Record<string, unknown>>)
          ...
          .map((r) => ({
            ...
            claim:
              Number(r.backend_execution_count ?? 0) === 0
        ...
        const traceCandidates = [
          ...
          ...(receiptList as Array<Record<string, unknown>>)
            .map((r) => String(r.receipt_id || r.request_id || ''))
            .filter(Boolean),
        ]
          .filter((id, i, arr) => id && arr.indexOf(id) === i)
          .slice(0, 10);
        const actionTraces: Array<Record<string, unknown>> = [];
        ...
        if (includeSelfTest) {
          selfTest = await client.selfTest().catch((err: unknown) => ({
            error: err instanceof Error ? err.message : String(err),
          }));
        }
        writeJson(res, 200, {
          ...
          system_of_record: {
            claim:
              'Compliance = control mapping over live objects. Evidence Pack = auditor artifact proving attack never reached server when NOT_SENT + backend_execution_count=0.',
          ...
          ledger_summary: {
            ...
            chain_integrity: (status as { ledger?: { chain_integrity?: boolean } } | null)?.ledger
              ?.chain_integrity,
            zero_byte_enforcements: (status as { ledger?: { zero_byte_enforcements?: number } } | null)
          ...
          flywheel: await (async () => {
            ...
            let threatPending = 0;
            try {
              const esc = (await client.escalations('open').catch(() => null)) as {
                count?: number;
              } | null;
      ...
      try {
        ...
        const updated = lines.map((line) => {
          try {
            ...
            if (row.id !== candidateId) return line;
            foundRow = row;
            row.status =
    ...
    if (pathname === '/api/gateway/threat-lab/corpus-replay' && method === 'POST') {
      ...
      try {
        ...
        const finalDecision = String(
          decide.final_decision || decide.decision || decide.arbiter_decision || '',
        ).toUpperCase();
    ...
    if (pathname === '/api/gateway/certified-catalog' && method === 'GET') {
      try {
        ...
        const discovered = Array.isArray((serversRaw as { servers?: unknown[] })?.servers)
          ? ((serversRaw as { servers: unknown[] }).servers as Array<Record<string, unknown>>)
          : Array.isArray(serversRaw)
            ? ((serversRaw as unknown) as Array<Record<string, unknown>>)
            : [];
        const stats = Array.isArray((statsRaw as { servers?: unknown[] })?.servers)
          ? (statsRaw as { servers: Array<Record<string, unknown>> }).servers
          : [];

        let certifications: Array<Record<string, unknown>> = [];
        let registrySource: string = 'unavailable';
        try {
        ...
        const catalog = discovered.map((s) => {
          ...
          const st = stats.find(
            (x) =>
              String(x.server_name || x.server_id || '').toLowerCase() === name.toLowerCase(),
          );
          return {
            server_name: name,
        ...
        const pol = policy as Record<string, unknown> | null;
        const requireCert =
          (pol as { require_certification?: string } | null)?.require_certification ||
          (pol as { policy?: { require_certification?: string } } | null)?.policy
        ...
        let signedPinReason =
          'Signing key not configured (MASTYF_CERT_SIGNING_KEY or ~/.mastyf/signing/ed25519.pem)';
        ...
        writeJson(res, 200, {
          ...
          note:
            'certified:true only when history registry has an attestation — never invented; signed pin UNAVAILABLE until signing keys exist (no Trust score)',
        });
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
        writeJson(res, 500, { error: msg });
    ...
    if (pathname === '/api/gateway/bff-info' && method === 'GET') {
      ...
      let historyDbBytes: number | null = null;
      try {
    ...
    if (pathname === '/api/gateway/runtime-health' && method === 'GET') {
      const gatewayUrl = (
        process.env['MASTYF_GATEWAY_URL'] ||
        process.env['MASTYF_CONTROL_URL'] ||
        'http://127.0.0.1:8443'
      ).replace(/\/$/, '');
      let gatewayOk = false;
      let gatewayDetail: string | null = null;
      try {
        const ac = new AbortController();
        const t = setTimeout(() => ac.abort(), 1500);
      ...
      try {
        status = (await client.status()) as unknown as Record<string, unknown>;
      ...
      const intel = (status?.intelligence || {}) as Record<string, unknown>;
