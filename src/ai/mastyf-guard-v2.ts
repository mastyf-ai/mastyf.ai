...
import type {
  ToolSecurityContext,
  MastyfGuardV2Prediction,
  ToolSecurityCategory,
  MastyfVerdict,
} from './mastyf-guard-v2-schema.js';
...
export class MastyfGuardV2Auditor {
   ...
   */
  public static buildStructuredPrompt(ctx: ToolSecurityContext): string {
   ...
   */
  public evaluate(ctx: ToolSecurityContext): MastyfGuardV2Prediction {
    ...
    const violatingParams: string[] = [];

    let invariantViolated: 'recipient_destination_deviation' | 'scope_expansion' | 'privilege_overgrant' | 'secondary_side_effect' | 'none' = 'none';
    ...
    if (/amount|total|payout|credit|debit|line_items|entries|disbursement|invoice/i.test(argsJson)) {
      ...
      let authorizedCeiling: number | null = null;
      if (authNumMatches) {
        const cleanNum = authNumMatches[0].replace(/[^0-9.]/g, '');
        authorizedCeiling = parseFloat(cleanNum);
      }
      
      let proposedPayout: number | null = null;
      const rawArgs = ctx.arguments as any;
      if (typeof rawArgs.total_payout === 'number') proposedPayout = rawArgs.total_payout;
      else if (typeof rawArgs.total_amount === 'number') proposedPayout = rawArgs.total_amount;
      else if (typeof rawArgs.authorized_base === 'number' && rawArgs.total_payout) proposedPayout = rawArgs.total_payout;
      ...
      if (Array.isArray(entries)) {
        ...
        for (const entry of entries) {
          ...
          if (typeof amt === 'number' && amt > 0) sumCharges += amt;
        }
    ...
    let primaryCategory: ToolSecurityCategory = 'benign';
    ...
    let verdict: MastyfVerdict = 'ALLOW';
    if (maxThreat >= 0.70) {
      verdict = 'BLOCK';
