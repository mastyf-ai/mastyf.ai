...
function isMonorepoRoot(root) {
    return (existsSync(join(root, 'pnpm-workspace.yaml'))
        && existsSync(join(root, 'package.json')));
}
async function maybeBuildDashboard(installRoot, force) {
    if (!force && isDashboardSpaBuilt(installRoot))
        return;
    const script = join(installRoot, 'scripts', 'build-dashboard-spa.sh');
    if (!existsSync(script)) {
        if (!isDashboardSpaBuilt(installRoot)) {
            console.error(chalk.yellow('  Dashboard SPA not built; npm package should include deploy/dashboard-spa/out/. ' +
...
async function needsAutoWrap() {
    ...
    try {
        ...
        return servers.some((s) => {
            if (!s.command)
                return false;
            if (isMastyfAiProxyCommand(s.command))
                return false;
...
async function runIdeManagedStart(opts, installRoot) {
...
export async function runStart(opts = {}) {
    ...
    applyStartEnv();
    const autoBuild = opts.buildDashboard
    ...
    if (autoBuild) {
        try {
            ...
            console.error(chalk.red(`Dashboard build failed: ${err.message}`));
            process.exit(1);
        }
    }
