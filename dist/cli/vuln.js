...
function loadServers(configPath) {
    ...
    if (paths.length)
        return ConfigParser.parse(paths[0]);
...
export async function runVulnCli(argv) {
    ...
    if (!process.env.MASTYF_AI_VULN_DISCOVERY_ALLOWLIST &&
    ...
    const { runVulnDiscovery, runAgenticVulnDiscovery, proposeBlockFromFinding, listFindings, getFinding, validateFinding, rejectFinding, markDisclosed, analyzeFinding, analyzeAll, loadReport, approveAnalysisReport, probeUpstreamApis, purgeNoiseFindings, } = await import('../vuln-discovery/index.js');
    switch (argv.action) {
        case 'run': {
        ...
        case 'list': {
            const findings = listFindings({
                minSeverity: argv.severity || undefined,
            });
            if (argv.format === 'json') {
                ...
                for (const f of findings) {
                    console.log(`${f.severity.padEnd(8)} ${f.status.padEnd(10)} ${f.class.padEnd(14)} ${f.id}  ${f.title.slice(0, 80)}`);
                }
        ...
        case 'analyze': {
            if (!argv.id || argv.id === 'all') {
                const reports = await analyzeAll({
                    minSeverity: argv.severity || 'HIGH',
                    status: 'validated',
                });
        ...
        case 'export': {
            ...
            try {
                ...
                if (pkg.acceptedPolicyRuleId)
                    console.log(`- Accepted policy rule: \`${pkg.acceptedPolicyRuleId}\``);
                if (pkg.corpusFixtureId)
                    console.log(`- Corpus fixture: \`${pkg.corpusFixtureId}\``);
                return 0;
            }
            catch (err) {
        ...
        case 'purge-noise': {
            ...
            console.log(JSON.stringify(result, null, 2));
            console.log(chalk.yellow(`Rejected ${result.rejected} noisy finding(s); deleted ${result.deletedCaches.length} NVD cache file(s).`));
        ...
        default:
            console.error(chalk.red(`Unknown action: ${argv.action}. Use: run|agentic-run|list|show|validate|reject|disclose|approve-analysis|propose-block|analyze|prepare-disclosure|export|probe|purge-noise`));
            return 1;
    }
}
