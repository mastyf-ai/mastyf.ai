...
export class AuditTrailSync {
    ...
    async initialize() {
        ...
        try {
            ...
            await client.query(`INSERT INTO mastyf_ai_instances (instance_id, instance_name, hostname, version, started_at, last_heartbeat, status, metadata)
        ...
        try {
            ...
            for (const tenantId of tenants.length > 0 ? tenants : ['default']) {
                ...
                for (const serverName of servers) {
                    const client = await this.pgPool.connect();
                    try {
                        const cursorRes = await client.query(`SELECT last_source_id FROM audit_sync_cursors
               WHERE instance_id = $1 AND tenant_id = $2 AND server_name = $3`, [this.config.instanceId, tenantId, serverName]);
                        const afterId = Number(cursorRes.rows[0]?.last_source_id) || 0;
                        const records = await this.localDb.getCallRecordsAfterId(serverName, afterId, this.config.batchSize, tenantId);
                        if (records.length === 0)
                            continue;
                        await client.query('BEGIN');
                        let maxId = afterId;
                        for (const record of records) {
                            if (record.sourceId > maxId)
                                maxId = record.sourceId;
                            const action = record.blocked ? 'block' : 'pass';
                            const severity = record.blocked ? 'warn' : 'info';
                            await client.query(`INSERT INTO unified_audit_trail
               ...
               ON CONFLICT DO NOTHING`, [
                        ...
                        await client.query(`INSERT INTO audit_sync_cursors (instance_id, tenant_id, server_name, last_source_id, last_synced_at)
               VALUES ($1, $2, $3, $4, NOW())
               ...
               DO UPDATE SET last_source_id = GREATEST(audit_sync_cursors.last_source_id, EXCLUDED.last_source_id),
                             last_synced_at = NOW()`, [this.config.instanceId, tenantId, serverName, maxId]);
                        await client.query('COMMIT');
                        Logger.debug(`[AuditTrailSync] Synced ${records.length} call records for ${serverName} (tenant=${tenantId}, afterId=${afterId})`);
                    }
                    catch (err) {
        ...
        try {
            ...
            for (const tenantId of tenants.length > 0 ? tenants : ['default']) {
                ...
                for (const serverName of servers) {
                    ...
                    const client = await this.pgPool.connect();
                    try {
                        await client.query(`INSERT INTO unified_security_scans
        ...
        try {
            ...
            for (const tenantId of tenants.length > 0 ? tenants : ['default']) {
               ...
               WHERE instance_id = $1 AND tenant_id = $2 AND server_name = $3`, [this.config.instanceId, tenantId, serverName]);
                        ...
                        await client.query(`INSERT INTO unified_cost_records
            ...
            for (const tenantId of tenants.length > 0 ? tenants : ['default']) {
                ...
                for (const serverName of servers) {
                    ...
                    if (!Number.isFinite(latencyMs) || !Number.isFinite(toolCount)) {
                        Logger.debug(`[AuditTrailSync] Skipping unified health for ${serverName}: measured latency/tool count unavailable`);
                        continue;
                    }
                    const client = await this.pgPool.connect();
                    try {
                        await client.query(`INSERT INTO unified_health_checks
    ...
    async sendHeartbeat() {
        try {
            const client = await this.pgPool.connect();
            try {
                await client.query(`UPDATE mastyf_ai_instances
           SET last_heartbeat = NOW(), status = $2,
               metadata = COALESCE(metadata, '{}'::jsonb) || $3::jsonb
           WHERE instance_id = $1`, [this.config.instanceId, 'active', JSON.stringify(this.instanceMetadata())]);
            }
            finally {
                client.release();
            }
    ...
    async recordLearningOutcome(outcome) {
        try {
            const client = await this.pgPool.connect();
            try {
                await client.query(`INSERT INTO ai_learning_outcomes_shared
    ...
    async persistBaseline(baseline) {
        try {
            const client = await this.pgPool.connect();
            try {
                await client.query(`INSERT INTO ai_baselines_shared
    ...
    async getSharedBaselines() {
        try {
            const client = await this.pgPool.connect();
            try {
                const result = await client.query('SELECT * FROM ai_baselines_shared ORDER BY server_name, tool_name');
    ...
    async getAggregatedMetrics(tenantId) {
        try {
            const client = await this.pgPool.connect();
            try {
                const instances = await client.query('SELECT * FROM mastyf_ai_instances WHERE last_heartbeat > NOW() - INTERVAL \'5 minutes\'');
                if (tenantId) {
               ...
               WHERE timestamp > NOW() - INTERVAL '1 hour' AND tenant_id = $1`, [tenantId]),
                        client.query(`SELECT COALESCE(SUM(cost_usd), 0) as total_cost
    ...
    async getAttackLearningState(tenantId = 'default') {
        try {
            const client = await this.pgPool.connect();
            try {
                const result = await client.query('SELECT state_json, updated_at FROM ai_attack_learning_state_shared WHERE tenant_id = $1', [tenantId]);
    ...
    async persistAttackLearningState(state, tenantId = 'default') {
        try {
            const client = await this.pgPool.connect();
            try {
                await client.query(`INSERT INTO ai_attack_learning_state_shared (tenant_id, state_json, updated_at)
           VALUES ($1, $2, NOW())
           ON CONFLICT (tenant_id) DO UPDATE
           SET state_json = $2, updated_at = NOW()`, [tenantId, JSON.stringify(state)]);
            }
            finally {
                client.release();
            }
...
//# sourceMappingURL=audit-trail-sync.js.map