export class UserToolEnforcementEngine {
    userToolAllowlists = new Map();
    userToolDenylists = new Map();
    userPathRules = new Map();
    userRateLimits = new Map();
    userTokenLimits = new Map();
    registeredPolicies = [];
    getPolicies() {
        return this.registeredPolicies;
    }
    registerUserPolicies(policies) {
    ...
    evaluate(context) {
        ...
        if (pathRules && pathRules.length > 0 && context.arguments) {
            for (const pathKey of ['path', 'file', 'directory', 'target', 'source']) {
                const pathValue = context.arguments[pathKey];
    ...
    registerRolePolicies(policies) {
        for (const p of policies)
            this.rolePolicies.set(p.roleName, p);
    }
    getEffectiveRolePolicy(roles) {
...
//# sourceMappingURL=user-tool-enforcement.js.map