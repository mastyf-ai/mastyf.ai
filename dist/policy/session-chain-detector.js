...
import { appendFlowEventSync, getFlowHistorySync, } from './session-flow-store.js';
...
const SENSITIVE_ARG_HINTS = /\b(?:\/etc\/passwd|\.env|\.ssh|id_rsa|credentials|secret|token|api[_-]?key)\b/i;
function isSafeExfilUrl(text) {
    try {
        const urls = text.match(/https?:\/\/[^\s"'<>,;]+/g) || [];
        return urls.length > 0 && urls.every(u => {
            try {
                return SAFE_EXFIL_DOMAINS.has(new URL(u).hostname);
            }
            catch {
                return false;
            }
...
function classifyNode(toolName, args) {
...
export function buildSessionChainGraph(sessionKey, flow) {
...
export function detectChainPatterns(graph) {
    const { nodes } = graph;
    const matches = [];
    if (nodes.length < 2)
        return matches;
...
export function evaluateSessionChainGuard(ctx) {
...
export function recordSessionChainEvent(ctx) {
    const key = flowSessionKey(ctx);
    const node = classifyNode(ctx.toolName, ctx.arguments);
...
//# sourceMappingURL=session-chain-detector.js.map