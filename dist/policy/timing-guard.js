...
const USERNAME_ORACLE_PATTERNS = [
...
const probeCounters = new LRUCache({
    max: 20_000,
    ttl: PROBE_WINDOW_MS,
});
const enumCounters = new LRUCache({
    max: 30_000,
    ttl: ENUM_WINDOW_MS,
});
export function isTimingGuardEnabled() {
    return process.env['MASTYF_AI_TIMING_GUARD'] !== 'false';
}
function probeSessionKey(ctx) {
...
function isEnumerationProbeCandidate(blob, toolName) {
...
export function scanTimingProbePatterns(blob) {
    const ruleIds = [];
    if (!blob.trim())
...
export function enumerationFingerprint(blob) {
...
function incrementCounter(cache, key, windowMs) {
    const now = Date.now();
...
export function resetTimingProbeCounters() {
    probeCounters.clear();
    enumCounters.clear();
}
export function evaluateTimingGuard(ctx) {
    if (!isTimingGuardEnabled())
        return null;
...
//# sourceMappingURL=timing-guard.js.map