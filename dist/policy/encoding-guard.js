...
const SHELL_FRAGMENT_RE = /\b(?:rm\s+-rf|union\s+select|sleep\s*\(|benchmark\s*\(|\/etc\/passwd|\/bin\/sh|select\s+\*|\/dev\/tcp|delete\s+account|drop\s+table)\b/i;
function encodingTransformApplied(original, decoded) {
...
function decodedContentSuspicious(original, decoded) {
    if (!encodingTransformApplied(original, decoded))
        return false;
    if (SUSPICIOUS_DECODED_RE.test(decoded)
...
export function isEncodingGuardEnabled() {
...
export function scanEncodingEvasion(blob) {
    if (!blob.trim())
...
export function evaluateEncodingGuard(ctx) {
    if (!isEncodingGuardEnabled())
        return null;
...
//# sourceMappingURL=encoding-guard.js.map