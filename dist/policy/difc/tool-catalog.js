...
const KNOWN_TOOL_PROFILES = {
    ...
    // Internal formatters — not egress
 ...
 */
export function getToolSecurityProfile(toolName) {
    ...
    const isSink = EGRESS_SINK_PATTERNS.some((p) => p.test(normalized)) ||
        /\b(?:send|email|mail|post|webhook|upload|transmit|forward|notify|curl|request|message|publish|share|tweet|sms)\b/i.test(normalized);
    const isSource = SENSITIVE_READ_PATTERNS.some((p) => p.test(normalized)) ||
        /\b(?:get|read|fetch|view|download|search|list|dump|query|inspect|access|history|record|details?|info|data|contact|payment|account|order|profile|password|credential|prescriptions?|generate|create|synthesize|deepfake|clone|extract)\b/i.test(normalized);
    if (isSink && !isSource) {
        return {
            type: 'sink',
            egressClearance: new Set(),
