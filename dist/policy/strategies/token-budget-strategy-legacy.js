...
export async function evaluateRedisTokenBudgetLegacy(context, deps) {
    ...
    try {
        ...
        for (const rule of deps.rules) {
            ...
            if (rule.maxTokensPerMinute) {
                ...
                const { allowed } = await rl.checkAndIncrement(key, rule.maxTokensPerMinute, WINDOW_MS, tenant, tokens);
                if (!allowed) {
                    return {
                        decision: {
            ...
            if (rule.maxUsdPerMinute) {
                ...
                const { allowed } = await rl.checkAndIncrement(usdKey, capMicro, WINDOW_MS, tenant, microUsd);
                if (!allowed) {
                    return {
                        decision: {
                            action: deps.resolveAction(rule.action),
...
//# sourceMappingURL=token-budget-strategy-legacy.js.map