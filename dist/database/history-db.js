...
import { decryptAuditArgsField, decryptField, encryptAuditArgsField, encryptField, getFieldEncryptionKey, } from '../utils/field-encryption.js';
...
function acquireLock(dbPath, readOnly = false) {
        ...
        try {
            ...
            if (!isNaN(existingPid) && existingPid > 0) {
                ...
                if (alive && existingPid !== process.pid) {
                    if (readOnly) {
                        Logger.info(`[HistoryDb] Opening ${dbPath} read-only (writer PID ${existingPid})`);
                    ...
                    Logger.info(`[HistoryDb] Opening ${dbPath} as secondary writer (primary PID ${existingPid})`);
...
export class HistoryDatabase {
    ...
    openedReadOnly;
    lockCleanup = null;
    PURGE_TTL_DAYS = resolveRetentionDays();
    purgeInterval = null;
    constructor(dbPathOrMemory, options) {
        ...
        const { cleanup, dbPath: effectivePath, readOnly: openedReadOnly, secondaryWriter } = acquireLock(requestedPath, readOnly);
        ...
        Logger.info(`[HistoryDb] Opened: ${this.dbPath} (${openedReadOnly ? 'read-only, ' : ''}WAL mode, PID ${process.pid})`);
    }
    async initialize() {
    ...
    applySqlCipherKeyIfConfigured() {
        const key = getFieldEncryptionKey();
        if (!key || this.dbPath === ':memory:')
            return;
        try {
    ...
    `);
        ...
        applyIndustryStandardMigration(this);
    }
    migrateQueryIndexes() {
        this.db.exec(`
    ...
    migrateTenantAuditColumns() {
        for (const table of ['cost_records', 'security_scans', 'health_checks']) {
            const cols = this.db.prepare(`PRAGMA table_info(${table})`).all();
            const names = new Set(cols.map((c) => c.name));
            if (!names.has('tenant_id')) {
                this.db.exec(`ALTER TABLE ${table} ADD COLUMN tenant_id TEXT NOT NULL DEFAULT 'default'`);
                this.db.exec(`CREATE INDEX IF NOT EXISTS idx_${table}_tenant ON ${table}(tenant_id, created_at DESC)`);
            }
        }
    }
    migrateCallRecordsColumns() {
        const cols = this.db.prepare('PRAGMA table_info(call_records)').all();
        const names = new Set(cols.map((c) => c.name));
    ...
    async getCallRecordsForServer(serverName, limit = 1000000, tenantId) {
        return monitorDbQuery('getCallRecordsForServer', () => {
            ...
            return rows.map((row) => this.mapCallRecordRow(row));
        });
    }
    /** Incremental sync — rows with id > afterId in ascending order. */
    async getCallRecordsAfterId(serverName, afterId, limit, tenantId) {
        return monitorDbQuery('getCallRecordsAfterId', () => {
            const rows = tenantId
                ? this.db
                    .prepare('SELECT * FROM call_records WHERE server_name = ? AND tenant_id = ? AND id > ? ORDER BY id ASC LIMIT ?')
                    .all(serverName, tenantId, afterId, limit)
                : this.db
                    .prepare('SELECT * FROM call_records WHERE server_name = ? AND id > ? ORDER BY id ASC LIMIT ?')
    ...
    async transaction(fn) {
        ...
        if (result instanceof Promise) {
            return Promise.reject(new Error('Async callbacks not supported in SQLite transactions — use transactionSync or synchronous operations'));
        }
        return this.transactionSync(() => result);
    }
    async flush() { }
    async addSecurityScan(serverName, score, cvesFound, details, tenantId = 'default') {
        this.db
            .prepare('INSERT INTO security_scans (server_name, score, cves_found, details, tenant_id) VALUES (?, ?, ?, ?, ?)')
            .run(serverName, score, cvesFound, JSON.stringify(details), tenantId);
    }
    async getLatestSecurityScan(serverName, tenantId) {
        const row = (tenantId
            ? this.db
                .prepare('SELECT * FROM security_scans WHERE server_name = ? AND tenant_id = ? ORDER BY id DESC LIMIT 1')
    ...
    async getSecurityScanHistory(serverName, limit = 10, tenantId) {
        return (tenantId
            ? this.db
                .prepare('SELECT * FROM security_scans WHERE server_name = ? AND tenant_id = ? ORDER BY id DESC LIMIT ?')
                .all(serverName, tenantId, limit)
            : this.db
    ...
    async getDistinctScannedServers(tenantId) {
        const rows = (tenantId
            ? this.db.prepare('SELECT DISTINCT server_name FROM security_scans WHERE tenant_id = ? ORDER BY server_name').all(tenantId)
            : this.db.prepare('SELECT DISTINCT server_name FROM security_scans ORDER BY server_name').all());
    ...
    async getDistinctActiveServers(tenantId) {
        ...
        const rows = this.db
            .prepare(`SELECT DISTINCT server_name FROM (
    ...
    async addCostRecord(serverName, tokensUsed, estimatedCostUSD, tenantId = 'default') {
        this.db
            .prepare('INSERT INTO cost_records (server_name, tokens_used, estimated_cost_usd, tenant_id) VALUES (?, ?, ?, ?)')
            .run(serverName, tokensUsed, estimatedCostUSD, tenantId);
    }
    async getLatestCostRecord(serverName, tenantId) {
        const row = (tenantId
            ? this.db
                .prepare('SELECT * FROM cost_records WHERE server_name = ? AND tenant_id = ? ORDER BY id DESC LIMIT 1')
                .get(serverName, tenantId)
            : this.db
    ...
    async getCostHistory(serverName, tenantId) {
        return (tenantId
            ? this.db
    ...
    async getTotalCost(serverName, tenantId) {
        if (serverName && tenantId) {
            const row = this.db
                .prepare('SELECT SUM(estimated_cost_usd) as total FROM cost_records WHERE server_name = ? AND tenant_id = ?')
                .get(serverName, tenantId);
    ...
    async addHealthCheck(serverName, latencyMs, success, toolCount, tenantId = 'default') {
        this.db
            .prepare('INSERT INTO health_checks (server_name, latency_ms, success, tool_count, tenant_id) VALUES (?, ?, ?, ?, ?)')
            .run(serverName, latencyMs, success ? 1 : 0, toolCount, tenantId);
    }
    async getLatestHealthCheck(serverName, tenantId) {
        const row = (tenantId
            ? this.db
                .prepare('SELECT * FROM health_checks WHERE server_name = ? AND tenant_id = ? ORDER BY id DESC LIMIT 1')
                .get(serverName, tenantId)
            : this.db
    ...
    async getRecentSuccessRate(serverName, tenantId) {
        const row = (tenantId
            ? this.db.prepare(`SELECT AVG(success) as avg
         FROM (
    ...
    getDistinctAuditTenants() {
        const rows = this.db
            .prepare(`SELECT DISTINCT tenant_id FROM (
    ...
    getDistinctServersForTenant(tenantId) {
        const rows = this.db
            .prepare(`SELECT DISTINCT server_name FROM (
