...
import { applyDifferentialPrivacyNoise, federatedPrivacyConfig, hashFederatedSignature, shouldShareFederatedDelta, } from './federated-privacy.js';
import { publishFederatedDeltaViaMesh, pullFederatedDeltasFromMesh } from './federated-mesh-bridge.js';
import { secureAggregateWeightVectors, scoreWithAggregatedWeights, FEDERATED_WEIGHT_DIM, } from './federated-weight-aggregation.js';
import { computeLocalGradient, fedAvgGradients, applyGradientToWeights, } from './federated-gradient-aggregation.js';
import { maskGradientForUpload, sumMaskedGradients, unmaskAggregatedGradients, } from './federated-masked-aggregation.js';
async function tryOnnxRuntimeScore(features, modelVersion) {
    if (process.env.MASTYF_AI_FEDERATED_ONNX === 'false')
        return null;
    try {
        ...
        const labels = ['benign', 'injection', 'exfil'];
...
export class FederatedLearningCoordinator {
    ...
    aggregatedContributors = 0;
    pendingRolloutApprovalId = null;
    rolloutStage = 'baseline';
    blockedSampleCount = 0;
    activeWeights = null;
    pendingGradients = [];
    constructor(approvalGate, bandit, store) {
    ...
    submitLocalDelta(params) {
        ...
        const noisySampleCount = Math.max(1, Math.round(applyDifferentialPrivacyNoise(params.sampleCount, gate.privacyBudgetEpsilon)));
        ...
        if (this.deltas.length > 100)
            this.deltas.splice(0, this.deltas.length - 100);
    ...
    recordBlockedFeatures(features, sampleCount = 1) {
        if (!this.isEnabled() || !features.length)
            return;
    ...
    recordBlockedSignature(signatureHash, features) {
        if (!this.isEnabled())
            return null;
        this.blockedSampleCount++;
        if (features?.length)
            this.recordBlockedFeatures(features, this.blockedSampleCount);
    ...
    shouldRouteToFederatedModel(requestId) {
        if (!this.isEnabled() || this.rolloutStage === 'baseline')
            return false;
        const bucket = parseInt(createHash('sha256').update(requestId).digest('hex').slice(0, 8), 16) % 100;
        if (this.rolloutStage === 'canary')
            return bucket < 10;
        if (this.rolloutStage === 'partial')
            return bucket < 50;
        return true;
    }
    getRolloutStage() {
    ...
    aggregateDeltas(minContributors = 3) {
        ...
        const { weights, contributorCount } = secureAggregateWeightVectors(this.deltas.map(d => ({ signatureHash: d.signatureHash, sampleCount: d.sampleCount })));
        ...
        if (this.pendingGradients.length > 0) {
            ...
            if (process.env.MASTYF_AI_FEDERATED_MPC === 'true' && participantIds.length > 1) {
                const masked = this.pendingGradients.map((g, i) => maskGradientForUpload(g.gradient, participantIds[i], participantIds, roundId));
        ...
        if (needsApproval && this.approvalGate) {
            approvalId = this.approvalGate.submit('federated-rollout', `Canary rollout for federated model ${newVersion}`, [{
    ...
    approvePendingRollout(requestId) {
        if (!this.approvalGate?.approve(requestId))
            return null;
    ...
    promoteRolloutStage() {
        ...
        if (prev === 'canary')
            this.rolloutStage = 'partial';
        else if (prev === 'partial')
            this.rolloutStage = 'full';
        else
            return null;
        const decision = {
    ...
    async runOnnxInference(features) {
        if (!this.isEnabled())
            return null;
        if (!features.length)
            return null;
        ...
        if (weights?.length) {
            const padded = [...salted];
            while (padded.length < FEDERATED_WEIGHT_DIM)
                padded.push(0);
            const score = scoreWithAggregatedWeights(padded.slice(0, FEDERATED_WEIGHT_DIM), weights);
            const label = score > 0.75 ? 'injection' : score > 0.45 ? 'exfil' : 'benign';
