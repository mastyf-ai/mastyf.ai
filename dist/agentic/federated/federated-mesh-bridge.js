...
export async function pullFederatedDeltasFromMesh(limit = 200) {
    ...
    for (const sig of catalog.signatures) {
        if (sig.category !== 'federated_learning')
            continue;
        ...
        try {
            ...
            if (parsed.type !== 'federated_learning' && !parsed.deltaId)
                continue;
            if (!parsed.deltaId || !parsed.signatureHash)
                continue;
            deltas.push({
...
export async function publishFederatedDeltaViaMesh(delta) {
    const relayUrl = process.env.MASTYF_AI_THREAT_MESH_RELAY_URL?.trim()
        ?? process.env.MASTYF_AI_FEDERATED_RELAY_URL?.trim();
    if (!relayUrl)
...
//# sourceMappingURL=federated-mesh-bridge.js.map