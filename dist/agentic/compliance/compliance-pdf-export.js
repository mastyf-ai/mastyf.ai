...
export async function writeComplianceEvidencePdf(bundle) {
    ...
    for (const signal of bundle.policySignals.slice(0, 20)) {
        pushWrapped(lines, `- ${signal.enabled ? 'enabled' : 'disabled'} ${signal.action ?? 'action-unset'}: `, `${signal.name}${signal.description ? ` - ${signal.description}` : ''}`);
    }
    if (bundle.policySignals.length === 0)
        lines.push('- No policy rules were readable from the configured policy path.');
    ...
    if (bundle.auditCounts.byServer.length === 0)
        lines.push('- No proxy call records found for this tenant.');
    ...
    if (bundle.auditCounts.securityScans.length === 0)
        lines.push('- No security scan records found for active servers.');
    lines.push('', 'Recent Blocked Calls');
    for (const call of bundle.auditCounts.recentBlocked.slice(0, 12)) {
        pushWrapped(lines, `- ${call.timestamp} ${call.serverName}.${call.toolName}: `, `${call.blockRule ?? 'rule-unavailable'}${call.blockReason ? ` - ${call.blockReason}` : ''}${call.argumentSnippet ? ` args=${call.argumentSnippet}` : ''}`);
    }
    if (bundle.auditCounts.recentBlocked.length === 0)
        lines.push('- No blocked calls were found in the current audit window.');
    ...
    for (const control of bundle.posture.controls) {
        ...
        if (control.gap)
            pushWrapped(lines, 'Gap: ', control.gap);
...
export function writePdfFromLines(lines, outputPath) {
...
//# sourceMappingURL=compliance-pdf-export.js.map