...
export class ControlMapper {
     ...
     */
    evaluate(framework, activePolicies, // List of active policy rule names
    blockedIncidents) {
     ...
     */
    evaluateControl(control, activePolicies, blockedIncidents) {
        // Check how many requirements are satisfied
        const satisfiedBy = [];
        const missing = [];
        for (const req of control.requirements) {
            const matchedPolicy = activePolicies.some(p => p.toLowerCase().includes(req.toLowerCase()) ||
                req.toLowerCase().includes(p.toLowerCase()));
            const matchedIncident = blockedIncidents.some(inc => inc.toLowerCase().includes(req.toLowerCase()) ||
        ...
        const incidentEvidence = blockedIncidents.filter(inc => control.requirements.some(req => inc.toLowerCase().includes(req.toLowerCase()) ||
            req.toLowerCase().includes(inc.toLowerCase())));
     ...
     */
    generateRecommendedPolicy(control, missing) {
        const lines = [
    ...
    buildSummary(framework, score, satisfied, total, gaps) {
