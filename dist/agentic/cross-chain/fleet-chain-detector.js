...
const onnxBoostCache = new Map();
async function refreshGraphOnnxBoost(sessionId, events) {
    if (!process.env.MASTYF_AI_FLEET_GRAPH_ONNX_MODEL?.trim())
        return;
    try {
        const { scoreGraphEventsWithOnnx } = await import('./graph-onnx-inference.js');
        const result = await scoreGraphEventsWithOnnx(events);
        if (result)
            onnxBoostCache.set(sessionId, result.score > 0.5 ? (result.score - 0.5) * 0.15 : 0);
    }
    catch {
        // best-effort
    }
...
export class FleetChainDetector {
    ...
    async mergeRedisEvents(sessionId) {
        try {
            ...
            for (const r of remote) {
                const dup = list.some(m => m.serverName === r.serverName
    ...
    hydrateSessionEvents(sessionId) {
        ...
        for (const e of cached) {
            const dup = merged.some(m => m.serverName === e.serverName
    ...
    detectCrossServerChain(sessionId, events) {
    ...
    async triggerIncidentPlaybook(alert) {
        try {
            const { ensureAgenticContainer } = await import('../../utils/agentic-container.js');
            const container = await ensureAgenticContainer();
            container?.incidentPlaybook.run('cross_mcp_chain', 'fleet-chain-detector', alert.confidence >= 0.75 ? 'critical' : 'high', 'shell_injection', { agentId: alert.agents[0], recentCalls: alert.tools.length });
        }
        catch {
            /* best-effort */
        }
    }
    async emitSiemAlert(alert) {
        try {
    ...
    toSessionGraph(sessionId, events) {
        return buildSessionChainGraph(sessionId, events.map(e => ({
            toolName: e.toolName,
            at: e.timestamp,
    ...
    getAlerts(limit = 50) {
        ...
        for (const a of this.alerts) {
            if (!merged.some(m => m.alertId === a.alertId))
                merged.push(a);
        }
        return merged.slice(-limit);
    }
    exportIrBundle(sessionId) {
...
//# sourceMappingURL=fleet-chain-detector.js.map