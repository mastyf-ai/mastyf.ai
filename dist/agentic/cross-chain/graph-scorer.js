...
function graphNeuralLayer(events, weightOverride) {
    if (events.length === 0)
        return 0;
    const n = events.length;
    const X = events.map(eventFeatureVector);
    ...
    for (let i = 0; i < n; i++) {
        ...
        h2 = sigmoid(h2 + H1[i]);
        if (X[i][0] > 0)
            readScore = Math.max(readScore, h2);
        if (X[i][2] > 0)
            exfilScore = Math.max(exfilScore, h2);
    }
    return readScore * exfilScore;
}
/** Boost chain confidence when argument snapshots show causal read→encode→exfil flow. */
export function scoreCausalGraphConfidence(events, baseConfidence) {
    if (events.length < 2)
    ...
    for (const e of events) {
        const argsStr = JSON.stringify(e.argumentsSnapshot ?? {});
        const tool = e.toolName.toLowerCase();
        if (/read|list|search|get/i.test(tool) || SENSITIVE_HINT.test(argsStr))
            hasSensitiveRead = true;
        if (/exec|bash|run|encode|python|node/i.test(tool) || ENCODE_HINT.test(argsStr))
            hasEncode = true;
        if (/http|webhook|upload|fetch|curl|post|send/i.test(tool) || EXFIL_HINT.test(argsStr))
            hasExfil = true;
    }
    let boost = 0;
    if (hasSensitiveRead && hasExfil)
        boost += 0.08;
    if (hasSensitiveRead && hasEncode && hasExfil)
        boost += 0.12;
    if (crossServer)
        boost += 0.05;
...
export function exportGraphFeatures(events) {
    return events.map(eventFeatureVector);
}
export function computeGraphNeuralScore(events, baseConfidence) {
...
export function trainGraphWeightsFromEvents(samples, epochs = 8, learningRate = 0.05) {
...
//# sourceMappingURL=graph-scorer.js.map