...
function shapeHash(toolName, argBytes) {
...
export class BehaviorFingerprintEngine {
    ...
    observe(obs) {
        ...
        if (obs.credentialIdentity) {
            ...
            if (!bound)
                this.credentialBindings.set(obs.agentId, obs.credentialIdentity);
        }
        return fp;
    }
    scoreAnomaly(agentId, obs) {
    ...
    listAnomalies(limit = 50) {
        return this.store?.listBehaviorAnomalies?.(limit) ?? [];
    }
}
export function resetBehaviorFingerprintEngineForTests() {
    // no-op singleton reset hook for tests
}
//# sourceMappingURL=behavior-fingerprint.js.map