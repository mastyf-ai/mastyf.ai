...
import { computeTransitiveTrust, loadTrustEdgesFromStore, resolveTrustAnchor, } from './reputation-web-of-trust.js';
import { mergeRatingsWithQuorum } from './reputation-quorum.js';
function hashServer(serverName, packageName) {
...
export class ReputationNetwork {
    ...
    mastyfAiScore;
    entries = new Map();
    constructor(store, mastyfAiScore) {
        this.store = store;
        this.mastyfAiScore = mastyfAiScore;
    }
    rateServer(params) {
        ...
        const merged = { ...defaults, ...(existing?.dimensions ?? {}), ...params.dimensions };
        const values = Object.values(merged);
        const weightedScore = existing
            ? Math.round((existing.consensusScore * (existing.raterCount ?? 1) + (values.reduce((a, b) => a + b, 0) / values.length) * raterWeight)
    ...
    applyByzantineQuorum(serverHash, fallback) {
        const votes = this.store?.listReputationRaterVotes?.(serverHash) ?? [];
        if (votes.length === 0)
            return fallback;
        const quorum = mergeRatingsWithQuorum(votes.map(v => ({
            raterId: v.raterId,
            dimensions: v.dimensions,
            raterWeight: v.raterWeight,
        })));
        if (!quorum.quorumMet || !quorum.dimensions)
            return fallback;
        return {
            ...fallback,
    ...
    queryServerReputation(serverName, packageName) {
    ...
    async queryWithNetwork(serverName, packageName) {
        const local = this.queryServerReputation(serverName, packageName);
        if (local && local.raterCount > 0)
            return local;
        ...
        try {
            ...
            if (body.attestationJws) {
                ...
                if (ingested.ok && ingested.entry)
                    return ingested.entry;
            }
            if (!body.consensusScore)
                return local;
    ...
    validateCertAgainstReputation(serverName, certLevel, packageName) {
        const rep = this.queryServerReputation(serverName, packageName);
        if (!rep)
            return { valid: true, reason: 'no_network_entry' };
    ...
    buildFromMastyfAiScore(serverName, score) {
        const byName = (n) => score.categories.find(c => c.name === n)?.score ?? 50;
        return this.rateServer({
            serverName,
            dimensions: {
    ...
    async publishToMeshRelay(serverName, packageName) {
        const entry = this.queryServerReputation(serverName, packageName);
        if (!entry)
            return { published: false, error: 'no_local_entry' };
        ...
        if (mesh.published)
            return { published: true, via: mesh.via };
        const relayUrl = process.env.MASTYF_AI_REPUTATION_RELAY_URL?.trim();
        if (!relayUrl)
            return { published: false, error: mesh.error ?? 'relay_not_configured' };
        try {
            ...
            if (!res.ok)
                return { published: false, error: `relay ${res.status}` };
...
//# sourceMappingURL=reputation-network.js.map