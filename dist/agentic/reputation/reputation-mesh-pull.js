...
export async function pullReputationEntriesFromMesh(network, limit = 200) {
    ...
    for (const sig of catalog.signatures) {
        if (sig.category !== 'reputation_network')
            continue;
    ...
    if (ingested > 0)
        Logger.info(`[ReputationMesh] Ingested ${ingested} mesh reputation entries`);
    return ingested;
}
/** Export quorum attestation bundle for a server (B1 decentralized sync). */
export function exportReputationAttestationBundle(network, serverName, packageName) {
...
//# sourceMappingURL=reputation-mesh-pull.js.map