...
export async function runDigitalTwinReplayHarness(params) {
    ...
    Logger.info(`[DigitalTwinReplay] ${params.serverName}: blocked ${attacksBlocked}/${attacksTotal} attacks, preserved ${workflowsPreserved}/${workflowsTotal} workflows, replayed ${capturedReplayed} captured calls${skipCorpus ? ' (captured-only)' : ''}`);
    return {
        serverName: params.serverName,
        attacksTotal,
        attacksBlocked,
