...
import { fleetChainBlockConfidenceThreshold, resolveGlobalSessionId, isEphemeralRequestSession, deriveAgentIdForFleetChain, } from '../utils/global-session-id.js';
function resolveSessionContext(ctx, legacyAgentId) {
 ...
 */
export async function hookAgenticObservation(container, serverName, toolName, args, sessionHash, _latencyMs, _success, agentId, credentialIdentity) {
    if (!container.behaviorCollector.isActive())
        return;
    try {
 ...
 */
export async function hookPromptInjectionCheck(container, serverName, toolName, args) {
    try {
        ...
        if (data.detected) {
            ...
            container.telemetry.recordDecision('proxy-pipeline', 'prompt-injection', {
            ...
            if (data.confidence > 0.7) {
                ...
                container.federatedLearning?.recordBlockedSignature(`${toolName}:${data.category ?? 'injection'}`);
                return {
                    blocked: true,
 ...
 */
export function hookThreatMeshContribution(container, blockedPattern, category, severity = 'high') {
    try {
...
export function recordAgenticAudit(container, params) {
    try {
        ...
        Logger.debug(`[AgenticProxyIntegration] Audit record error: ${err instanceof Error ? err.message : String(err)}`);
    }
}
/** Pre-forward hooks: sandbox tier, intent binding, collusion, reputation, injection scan. */
export async function runAgenticPreForwardHooks(container, serverName, toolName, args, sessionCtx, legacyAgentId) {
    ...
    const tierScope = { scopeType: 'server', scopeId: serverName };
    ...
    if (container.sandboxEnforcer.shouldShadow(tierScope)) {
        ...
        if (grant) {
            Logger.info(`[Sandbox] Shadow bypass via operator grant ${grant.grant_id} for ${toolName} on ${serverName}`);
        }
        else {
    ...
    const collusion = container.collusionDetector.record(agentId ?? globalSessionId, serverName, toolName, { sessionId: globalSessionId });
...
export async function runAgenticPostCallHooks(container, serverName, toolName, args, sessionHash, latencyMs, success, agentId, responseSize) {
...
export function runAgenticDeniedCallHooks(container, params) {
    recordAgenticAudit(container, params);
    if (params.blockRule && params.toolName) {
        hookThreatMeshContribution(container, `${params.toolName}:${params.blockRule}`, params.blockRule, 'high');
    }
}
//# sourceMappingURL=proxy-integration.js.map