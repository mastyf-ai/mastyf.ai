...
export class MCPCertifier {
    ...
    registry = new Map();
    mastyfAiScore;
    constructor(store, mastyfAiScore, reputationNetwork) {
        this.store = store;
    ...
    collectFromMastyfAiScore(input) {
    ...
    certify(serverName, packageName, version, results) {
        const checks = [
            ...
            { id: 'supply-chain', name: 'Trusted Publisher', passed: results.trustedPublisher, score: results.trustedPublisher ? 100 : 0, maxScore: 100, details: results.trustedPublisher ? 'Verified publisher' : 'Unknown publisher' },
            {
                ...
                maxScore: 100,
                details: (results.fuzzCriticalFailures ?? 0) === 0
        ...
        let level = totalScore >= 90 ? 'platinum' : totalScore >= 75 ? 'gold' : totalScore >= 60 ? 'silver' : 'bronze';
        if (this.reputationNetwork) {
            ...
            if (!netCheck.valid && level === 'platinum')
                level = 'gold';
            if (!netCheck.valid && level === 'gold')
                level = 'silver';
        }
    ...
    certifyFromScan(serverName, packageName, version, scan) {
    ...
    verifyCertification(serverName, attestationJws) {
        const cert = this.getCertification(serverName);
        if (!cert)
            return { valid: false, reason: 'not_certified' };
