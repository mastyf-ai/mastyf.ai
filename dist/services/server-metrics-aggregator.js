 ...
 */
import { calculateServerThreatScore } from './server-threat-scorer.js';
...
export class ServerMetricsAggregator {
    ...
    recordCall(params) {
        ...
        if (decision === 'block') {
            ...
            const isCritical = (rule && (rule.includes('injection') || rule.includes('exfil') || rule.includes('reverse-shell'))) || false;
        ...
        toolMeta.totalCalls++;
        if (decision === 'block')
            toolMeta.blockedCalls++;
        toolMeta.lastUsed = now;
    }
    getServerSummary(serverName) {
        const data = this.servers.get(serverName);
        if (!data)
            return null;
        const tools = Array.from(data.tools.values());
        const highRiskToolCount = tools.filter((t) => t.riskTier === 'high' || t.riskTier === 'critical').length;
        const oneHourAgo = Date.now() - 3600000;
        const recentThreats1h = data.threats.filter((t) => new Date(t.timestamp).getTime() >= oneHourAgo).length;
        const criticalThreatCount = data.threats.filter((t) => t.severity === 'critical').length;
        ...
        let status = 'healthy';
        if (threatScore.tier === 'danger')
            status = 'unhealthy';
        else if (threatScore.tier === 'warning')
            status = 'degraded';
        return {
            name: serverName,
            status,
...
//# sourceMappingURL=server-metrics-aggregator.js.map