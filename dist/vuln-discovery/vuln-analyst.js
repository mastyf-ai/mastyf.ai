...
function minSeverityOk(finding) {
    const min = (process.env.MASTYF_AI_VULN_ANALYSIS_MIN_SEVERITY || 'MEDIUM').toUpperCase();
    const order = ['CRITICAL', 'HIGH', 'MEDIUM', 'LOW', 'INFO'];
    return order.indexOf(finding.severity) <= order.indexOf(min);
}
export function isVulnAnalysisEnabled() {
    if (process.env.MASTYF_AI_VULN_ANALYSIS_ENABLED === 'false')
        return false;
    if (process.env.MASTYF_AI_VULN_DISCOVERY_ENABLED === 'true')
        return true;
    return process.env.MASTYF_AI_VULN_ANALYSIS_ENABLED === 'true';
}
export function buildContextPack(finding) {
...
function citationsFromPack(pack) {
    const citations = [
        {
...
function formatFactsCorpus(pack) {
...
function templateReport(finding, pack) {
    const sections = {
        executiveSummary: `Mastyf.ai detected a ${finding.severity} ${finding.class} issue on ` +
        ...
        technicalDeepDive: `${finding.description}\n\nScanner: ${finding.evidence.scanner}\n` +
        ...
        mastyfRecommendations: 'Open Threat Lab with this finding; consider response-injection scanner and corpus fixture if applicable.',
        disclosureGuidance: 'If validated, prepare vendor report with repro steps. Do not invent CVE IDs.',
    };
...
function renderMarkdown(finding, sections, citations) {
    const lines = [
...
function validateFactBullets(facts, citeIds) {
    ...
    for (const line of lines) {
        const m = line.match(/\[([^\]]+)\]/);
        if (m && citeIds.has(m[1]))
            ok++;
    }
    return ok >= Math.ceil(lines.length * 0.5);
}
function rejectFabricatedCves(text, allowed) {
    return text.replace(/CVE-\d{4}-\d+/gi, (id) => {
        if (allowed.has(id.toUpperCase()) || allowed.has(id))
            return id;
        return `${id} [UNVERIFIED—removed]`;
    });
}
async function llmGenerate(system, user, llm) {
    const assistant = llm
        || new LlmAssistant({
            hotPath: false,
...
export function allowVulnAnalysisTemplateFallback() {
...
export function ensureVulnAnalysisLlmEnabled() {
...
async function requireLlmAssistant() {
    ensureVulnAnalysisLlmEnabled();
    if (process.env.MASTYF_AI_LLM_ENABLED === 'false') {
        throw new VulnAnalysisLlmUnavailableError('LLM is required for Vuln Discovery deep analysis (MASTYF_AI_LLM_ENABLED=false). '
            + 'Set MASTYF_AI_LLM_ENABLED=true, or set MASTYF_AI_VULN_ANALYSIS_ALLOW_TEMPLATE=true for offline templates.');
    }
    const model = process.env.MASTYF_AI_VULN_ANALYSIS_MODEL
    ...
    if (!llm.isAvailable()) {
        throw new VulnAnalysisLlmUnavailableError('LLM is required but disabled in config. Set MASTYF_AI_LLM_ENABLED=true.');
    }
    const health = await llm.healthCheckDetailed();
    if (!health.ok) {
        throw new VulnAnalysisLlmUnavailableError(`LLM is required for Vuln Discovery analysis but unavailable at ${health.endpoint}`
...
function auditAnalysis(event) {
    const dir = ensureVulnStoreDir();
    appendFileSync(join(dir, 'vuln-analysis-audit.jsonl'), JSON.stringify({ ts: new Date().toISOString(), ...event }) + '\n');
}
...
export function loadReport(findingId) {
...
export function approveAnalysisReport(findingId) {
 ...
 */
export async function analyzeFinding(findingId, opts) {
    ...
    const allowedCves = new Set([...(pack.publishedIntel || [])]
        .map((p) => p.id.toUpperCase())
        .filter((id) => id.startsWith('CVE-')));
    if (finding.relatedCve)
        allowedCves.add(finding.relatedCve.toUpperCase());
    ...
    let provider = 'none';
    if (passes >= 1) {
        const pass1 = await llmGenerate('You extract security finding facts. List ONLY facts present in citations. ' +
            'Each bullet MUST include a [citation-id]. No inference. No new CVE IDs.', `Citations:\n${corpus}\n\nList fact bullets:`, llm);
        if (pass1) {
            facts = pass1.text;
            model = pass1.model;
            provider = 'ollama';
    ...
    if (!facts) {
        if (!allowVulnAnalysisTemplateFallback()) {
            throw new VulnAnalysisLlmUnavailableError('LLM is required for Vuln Discovery deep analysis but returned no usable output. '
    ...
    if (passes >= 2) {
        const pass2 = await llmGenerate('You are a senior security researcher writing an in-depth vulnerability analysis for an MCP/AI agent stack. ' +
            ...
            + 'Do not claim RCE, data exfiltration, or privilege escalation without quoting proof from the facts.', `Finding: ${finding.title}\nClass: ${finding.class}\nSeverity: ${finding.severity}\n\nFacts:\n${facts}\n\nWrite the sections:`, llm);
        if (pass2) {
            model = pass2.model;
    ...
    // Pass 3 — mitigations
    if (passes >= 3) {
        const pass3 = await llmGenerate('You recommend mitigations for MCP/AI proxy security (mastyf.ai). ' +
            'Write sections:\n## Recommended Mitigations\n## Mastyf.ai Recommendations\n## Disclosure Guidance\n' +
            'Be specific about policy rules, response scanning, and vendor disclosure. No invented CVE IDs.', `Finding: ${finding.title}\nFacts:\n${facts}\nDeep dive:\n${sections.technicalDeepDive.slice(0, 2000)}\n\nWrite mitigations:`, llm);
        if (pass3) {
    ...
    const exec = await llmGenerate('Explain this vulnerability to a non-technical operator in 2-3 short paragraphs. Use only cited facts. No jargon dump.', `Title: ${finding.title}\nSeverity: ${finding.severity}\nFacts:\n${facts}\n\nExecutive summary:`, llm);
    sections.executiveSummary =
        exec?.text ||
    ...
    if (report.source !== 'llm' && !allowVulnAnalysisTemplateFallback()) {
        throw new VulnAnalysisLlmUnavailableError('LLM is required for Vuln Discovery analysis but the report was not produced by the LLM. '
...
export async function onFindingValidated(findingId) {
    if (process.env.MASTYF_AI_VULN_ANALYSIS_AUTO_ON_VALIDATE === 'false')
        return null;
    return analyzeFinding(findingId);
}
//# sourceMappingURL=vuln-analyst.js.map