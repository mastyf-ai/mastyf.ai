...
function argDepth(v, d = 0) {
    if (d > 10)
        return d;
    if (v && typeof v === 'object') {
        return Math.max(d, ...Object.values(v).map((x) => argDepth(x, d + 1)));
    }
    return d;
}
function suspiciousChars(s) {
    return /[\u0000\u200b\u202e\uFEFF]|%2e%2e|\\x[0-9a-f]{2}/i.test(s) ? 1 : 0;
}
/** Lightweight local reconstruction-error proxy when core autoencoder unavailable. */
export function scoreBehavioralAnomaly(input) {
    ...
    const score = Math.min(1, entropy * 0.35 + depth * 0.25 + len * 0.15 + sus * 0.2 + (1 - timeNorm) * 0.05);
    const enabled = process.env.MASTYF_AI_AUTOENCODER_ENABLED === 'true';
    return { anomaly: enabled && score >= threshold, score, threshold };
}
export async function evaluateBehavioralAndRecord(input) {
    const enabled = process.env.MASTYF_AI_AUTOENCODER_ENABLED === 'true';
    if (!enabled)
        return null;
    // Prefer core autoencoder when available
    try {
        const { extractAutoencoderFeatures, detectAnomaly, trainOnBenign, } = await import('@mastyf_ai/core');
        const features = extractAutoencoderFeatures(input.toolName, (input.arguments && typeof input.arguments === 'object'
            ? input.arguments
            : { value: input.arguments }), input.serverName);
        ...
        if (result.anomaly || result.reconstructionError >= result.threshold) {
            const finding = persistBehavioral(input, result.reconstructionError, result.threshold, 'core-autoencoder');
...
function persistBehavioral(input, score, threshold, scanner) {
    const partial = {
        class: 'behavioral',
        severity: 'MEDIUM',
        status: 'candidate',
