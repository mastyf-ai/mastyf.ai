 ...
 */
const SOFT_DENY_RE = /access\s+denied|path\s+outside\s+allowed|outside\s+the\s+allowed|not\s+in\s+.*allowed|permission\s+denied|eacces|enoent|forbidden|unauthorized|invalid\s+path|path\s+traversal\s+blocked|blocked\s+by\s+mastyf|is\s+not\s+allowed/i;
const EXPLOIT_CONTENT_RE = /(?:root:[x*]:0:0:|-----BEGIN (?:RSA )?PRIVATE KEY-----|AKIA[0-9A-Z]{16}|ghp_[A-Za-z0-9]{36}|sk-[A-Za-z0-9]{20,}|ignore\s+(?:all\s+)?(?:previous|prior|above)\s+instructions|<script[\s>]|javascript:\s*)/i;
...
export function resultLooksLikeMcpError(result) {
    if (!result || typeof result !== 'object')
        return false;
    const r = result;
    if (r.isError === true || r.is_error === true)
        return true;
    return false;
}
export function isSoftDenyText(text) {
    return SOFT_DENY_RE.test(text);
}
/** Response is mostly an echo of the request args (common benign FP). */
export function isArgsEchoOnly(args, responseText) {
    if (!responseText || responseText.length < 2)
        return true;
    try {
        const argsJson = JSON.stringify(args);
        if (responseText === argsJson)
            return true;
...
export function isMaliciousArgs(args) {
...
//# sourceMappingURL=effect-classifier.js.map