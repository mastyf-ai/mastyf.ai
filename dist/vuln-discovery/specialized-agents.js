...
import { generateFuzzPayloads, findingFromFuzzResult } from './mcp-tool-fuzzer.js';
...
export function coverageAgent(limit = 8) {
...
export function prioritizerAgent(limit = 20) {
    const order = ['live-traffic-tap', 'mcp-tool-fuzzer', 'response-injection-scanner'];
    const candidates = listFindings({ status: 'candidate', minSeverity: 'HIGH' }).filter((f) => f.evidence.scanner === 'live-traffic-tap'
...
export function benignArgsFromTool(tool) {
