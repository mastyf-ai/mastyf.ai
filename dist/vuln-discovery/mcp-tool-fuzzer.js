...
import { EXPLOIT_EFFECT_DECISION, EXPLOIT_EFFECT_RULE, classifyExploitEffect, hasProvenExploitEffect, isMaliciousArgs, } from './effect-classifier.js';
import { recordPrecisionEvent } from './precision-metrics.js';
function depthLimit(d) {
    if (d === 'shallow')
        return 3;
    if (d === 'deep')
        return 12;
...
export function generateFuzzPayloads(schema, depth = 'medium') {
 ...
 */
export function findingFromFuzzResult(serverName, result) {
    ...
    if (!result.blockedByProxy && result.ok) {
        ...
        const finding = persist({
            ...
            description: `Malicious fuzz payload produced exploit effect (${effect.reason}). `
...
export function getFuzzDepthFromEnv() {
    const d = (process.env.MASTYF_AI_VULN_DISCOVERY_FUZZ_DEPTH || 'medium').toLowerCase();
    if (d === 'shallow' || d === 'deep')
        return d;
    return 'medium';
}
//# sourceMappingURL=mcp-tool-fuzzer.js.map