...
import { EXPLOIT_EFFECT_DECISION, EXPLOIT_EFFECT_RULE, hasProvenExploitEffect, isSoftDenyText, stringifyResult, classifyExploitEffect, } from './effect-classifier.js';
function evidenceHasExploitEffect(finding) {
    if (finding.evidence.rule === EXPLOIT_EFFECT_RULE)
        return true;
    if (finding.evidence.proxyDecision === EXPLOIT_EFFECT_DECISION)
        return true;
    ...
    const text = stringifyResult(finding.evidence.response);
    const args = finding.evidence.payloads?.[0] || {};
    return hasProvenExploitEffect({
        args,
        responseText: text,
        result: finding.evidence.response,
 ...
 */
export async function reproFinding(findingId, opts) {
...
export async function reproNovelCandidates(opts) {
    ...
    const candidates = listFindings({ status: 'candidate', minSeverity: 'HIGH' })
        .filter((f) => f.evidence.scanner === 'mcp-tool-fuzzer'
...
//# sourceMappingURL=repro-agent.js.map