...
function upsertFromPartial(partial) {
...
export function packageAppearsInCveText(pkg, text) {
    const bare = pkg.includes('/') ? pkg.slice(pkg.lastIndexOf('/') + 1) : pkg;
    if (!bare || bare.length < 2 || /^[\d.]+$/.test(bare))
        return false;
 ...
 */
export function shouldUpsertCveFinding(pkg, cve, osvIds) {
    if (cve.id.startsWith('GHSA-') || cve.source === 'osv')
        return true;
    if (osvIds.has(cve.id))
        return true;
...
function cveToFinding(serverName, pkg, cve) {
...
function auditToFinding(serverName, vuln) {
 ...
 */
export async function scanServerSupplyChain(server, opts) {
    const findings = [];
    const errors = [];
    ...
    try {
        ...
        for (const { packageName, result } of perPkg) {
            const osvIds = new Set(result.findings
                .filter((f) => f.source === 'osv' || f.id.startsWith('GHSA-'))
                .map((f) => f.id));
            for (const cve of result.findings) {
                if (!shouldUpsertCveFinding(packageName, cve, osvIds))
                    continue;
    ...
    if (!opts?.skipTransitiveTree) {
        ...
        if (root) {
            try {
                ...
                const osvByPkg = new Map();
                for (const { packageName, cve } of treeCves) {
                    if (cve.source === 'osv' || cve.id.startsWith('GHSA-')) {
                        if (!osvByPkg.has(packageName))
                            osvByPkg.set(packageName, new Set());
    ...
    try {
        ...
        if (sbom) {
            ...
            if (sbomDiff.added.length > 10) {
                findings.push(upsertFromPartial({
                    class: 'config',
                    severity: 'INFO',
                    status: 'candidate',
    ...
    Logger.info(`[vuln:supply-chain] ${server.name}: ${findings.length} findings (cve=${cveCount} audit=${auditCount})`);
    ...
    if (process.env.MASTYF_AI_THREAT_RESEARCH_AUTO === 'true') {
        try {
            const { enqueueThreatResearch, buildDependencyAnomalyEvent } = await import('../ai/threat-research-pipeline.js');
            for (const f of findings) {
                if (f.relatedCve)
                    continue;
                if (f.severity !== 'CRITICAL' && f.severity !== 'HIGH')
                    continue;
                if (f.class !== 'dependency')
                    continue;
                enqueueThreatResearch(buildDependencyAnomalyEvent({
                    id: f.id,
                    class: f.class,
                    severity: f.severity,
                    title: f.title,
...
//# sourceMappingURL=supply-chain-scanner.js.map