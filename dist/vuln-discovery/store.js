...
export function createFindingId(fingerprint) {
...
export function upsertFinding(finding) {
    ensureDir();
    appendFileSync(findingsPath(), JSON.stringify(finding) + '\n');
    return finding;
}
export function updateFindingStatus(id, status, extra) {
...
export function listFindings(opts) {
    ...
    if (opts?.status)
        all = all.filter((f) => f.status === opts.status);
    if (opts?.class)
        all = all.filter((f) => f.class === opts.class);
    if (opts?.minSeverity) {
...
export function compactFindingsStore() {
