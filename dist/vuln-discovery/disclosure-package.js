...
`;
function isThinReport(report) {
    if (!report)
        return true;
    if (report.status === 'stale')
        return true;
    if (report.source === 'template-fallback')
        return true;
...
async function enrichThreatLabLinks(findingId) {
    try {
        ...
        if (!cand)
            return {};
        const corpus = cand.corpusCandidate;
        return {
            threatLabCandidateId: cand.id,
            acceptedPolicyRuleId: cand.reviewStatus === 'accepted'
 ...
 */
export async function buildDisclosurePackage(findingId, opts) {
    ...
    if (opts?.requireFinal && !vendorReady) {
        throw new Error(`Analysis is ${report.status}; approve analysis before vendor-ready export`);
    }
 ...
 */
export async function prepareDisclosurePackage(findingId, opts) {
    const finding = getFinding(findingId);
    if (!finding) {
        throw new Error(`Finding not found: ${findingId}`);
    }
...
export function readDisclosurePackageZip(findingId) {
    const zipPath = join(vulnDisclosureDir(findingId), 'disclosure-package.zip');
    if (!existsSync(zipPath))
        return null;
    return readFileSync(zipPath);
}
export function loadDisclosurePackageMeta(findingId) {
...
//# sourceMappingURL=disclosure-package.js.map