...
import { EXPLOIT_EFFECT_DECISION, EXPLOIT_EFFECT_RULE, hasProvenExploitEffect, isMaliciousArgs, stringifyResult, classifyExploitEffect, } from './effect-classifier.js';
import { recordLiveAllow, recordMaliciousShaped, recordSoftDenySeen, } from './live-traffic-stats.js';
import { recordPrecisionEvent } from './precision-metrics.js';
export function isLiveTrafficTapEnabled() {
    if (process.env.MASTYF_AI_VULN_LIVE_TAP === 'false')
        return false;
    if (process.env.MASTYF_AI_VULN_LIVE_TAP === 'true')
        return true;
...
export function tapAllowedToolCall(input) {
    ...
    if (!hasProvenExploitEffect({ args, result: input.result, responseText })) {
        ...
        Logger.debug(`[live-tap] skip ${input.serverName}/${input.toolName}: ${effect.reason}`);
        return null;
    }
    ...
    const partial = {
        ...
        description: `Incoming traffic allowed malicious args and upstream returned exploit effect (${effect.reason}). `
...
//# sourceMappingURL=live-traffic-tap.js.map