...
export function buildAgentStackGraph(servers, opts) {
    const nodes = [];
    const edges = [];
    const seen = new Set();
    const addNode = (n) => {
...
export function sliceGraphAround(graph, name) {
    const related = new Set();
    for (const n of graph.nodes) {
        if (n.name.includes(name) || n.id.includes(name))
            related.add(n.id);
    }
...
//# sourceMappingURL=stack-graph.js.map