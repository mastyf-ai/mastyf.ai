...
import { runVulnDiscovery } from './engine.js';
...
async function scoutAgent(opts) {
    const runOpts = {
...
function correlatorAgent(servers, findings) {
    try {
        ...
        Logger.debug(`[vuln-agentic:correlator] skipped: ${err instanceof Error ? err.message : String(err)}`);
    }
}
/** NoiseRejecter — auto-reject numeric targets / ancient NVD spam before validation. */
function noiseRejecterAgent(findings) {
    let rejected = 0;
    const seen = new Set();
...
async function reporterAgent(minSeverity) {
    ...
    for (const f of validated) {
        ...
        try {
            ...
            Logger.warn(`[vuln-agentic:reporter] ${f.id}: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    return n;
}
/** BlockProposal — enqueue Threat Lab candidates (human Accept). */
async function blockProposalAgent(tenantId, useLlm = true) {
    ...
    for (const f of validated) {
        try {
            ...
            Logger.warn(`[vuln-agentic:block] ${f.id}: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    return n;
}
 ...
 */
export async function runAgenticVulnDiscovery(opts) {
    const errors = [];
    ...
    try {
        ...
        Logger.debug(`[vuln-agentic:coverage] skipped: ${err instanceof Error ? err.message : String(err)}`);
    }
    ...
    try {
        ...
        Logger.warn(`[vuln-agentic:repro] skipped: ${err instanceof Error ? err.message : String(err)}`);
    }
    let validated = 0;
    ...
    Logger.info(`[vuln-agentic] done scout=${scout.findings.length} validated=${validated} rejected=${rejected} reports=${reports} blocks=${blockProposals}`);
    auditProbe({
        action: 'agentic-run',
...
export async function agenticPromoteFinding(findingId, opts) {
    const result = validateFinding(findingId, { llmConfirmation: true });
    if (!result)
        return { validated: false, reason: 'not found' };
    if (!result.promoted)
        return { validated: false, reason: result.reason };
