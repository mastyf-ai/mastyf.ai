...
const STRONG_SCANNERS = ['semgrep', 'npm-audit-transitive', 'response-injection-scanner'];
 ...
 */
export function isAdvisoryDependencyFinding(finding) {
    if (finding.class !== 'dependency')
        return false;
    const scanner = finding.evidence.scanner;
    return (scanner === 'cve-checker'
...
export function isRuntimeNovelFinding(finding) {
    if (RUNTIME_NOVEL_SCANNERS.has(finding.evidence.scanner))
        return true;
    return (finding.class === 'injection'
        || finding.class === 'implementation'
        || finding.class === 'protocol'
        || finding.class === 'auth');
}
export function discoveryLane(finding) {
    if (isAdvisoryDependencyFinding(finding))
        return 'advisory';
    if (isRuntimeNovelFinding(finding))
        return 'novel-runtime';
    return 'other';
}
/**
 ...
 */
export function isNovelOrPreAdvisory(finding) {
    if (finding.title.startsWith('Pre-advisory'))
        return true;
    if (finding.class === 'dependency' && !finding.relatedCve)
        return true;
    return false;
}
/**
 ...
 */
export function requiresHardPromotion(finding) {
    if (isRuntimeNovelFinding(finding))
        return true;
    if (isNovelOrPreAdvisory(finding))
        return true;
    return false;
}
export function parseCveYear(idOrTitle) {
 ...
 */
export function isNoiseFinding(finding) {
    ...
    if (finding.evidence.scanner === 'mcp-tool-fuzzer'
        ...
        if (finding.evidence.proxyDecision === 'allow'
    ...
    if (finding.class === 'dependency' && finding.evidence.scanner === 'cve-checker') {
        ...
        if (finding.relatedCve?.startsWith('CVE-')
            && name.length >= 2
...
function deriveScannerAgreement(finding) {
    const scanner = finding.evidence.scanner;
    const high = finding.severity === 'CRITICAL' || finding.severity === 'HIGH';
    if (STRONG_SCANNERS.includes(scanner) && high) {
        return true;
    }
...
function deriveReproSuccess(finding) {
    return (finding.evidence.reproSteps.length >= 2
 ...
 */
export function validateFinding(id, signals = {}) {
    ...
    if (isRuntimeNovelFinding(finding)) {
        ...
        if (derived.reproSuccess === undefined) {
            const effectOk = finding.evidence.rule === 'exploit-effect'
        ...
        if (derived.scannerAgreement === undefined)
            derived.scannerAgreement = false;
        if (derived.reproSuccess === undefined)
            derived.reproSuccess = false;
    }
    else {
        if (derived.scannerAgreement === undefined) {
...
export function rejectFinding(id, reason) {
...
export function markDisclosed(id, externalId) {
    const finding = getFinding(id);
    if (!finding)
        return undefined;
    if (finding.severity === 'CRITICAL' || finding.severity === 'HIGH') {
        ...
        if (!report || report.status !== 'final') {
            throw new Error('CRITICAL/HIGH findings require an approved (final) analysis report before disclosure');
        }
    }
    return updateFindingStatus(id, 'disclosed', {
...
export function autoValidateEligible() {
...
export function deletePoisonedNvdCaches() {
 ...
 */
export function purgeNoiseFindings() {
