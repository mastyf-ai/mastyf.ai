...
function checkScanStrict(reports) {
    if (process.env['MASTYF_AI_SCAN_STRICT'] !== 'true')
        return;
    const issues = [];
...
policyCmd
    ...
    try {
        ...
        const format = ['json', 'signed', 'tarball'].includes(opts.format) ? opts.format : 'json';
...
program
    ...
    .option('--project-root <path>', 'MCP Mastyf AI package root (dist/cli.js)', resolveMastyfAiInstallRoot())
...
program
    ...
    .action(async (opts) => {
    if (opts.policy)
        process.env.MASTYF_AI_POLICY_PATH = opts.policy;
...
program
    ...
    if (opts.unsafeNoTls) {
        ...
        console.error(chalk.yellow.bold('⚠ --unsafe-no-tls: upstream MCP traffic may use cleartext HTTP (development only)'));
    }
    ...
    if (!opts.gateway && !ingressMode && stdioServerCount > 1) {
        ...
        console.error(chalk.red('Multiple stdio MCP servers in one proxy process are not supported.\n' +
        ...
        try {
            ...
            if (opts.blockingMode && ['audit', 'warn', 'block'].includes(opts.blockingMode) && policyEngine) {
                if (process.env['MASTYF_AI_DISALLOW_MODE_OVERRIDE'] === 'true') {
                    console.error(chalk.yellow(`--blocking-mode ignored (MASTYF_AI_DISALLOW_MODE_OVERRIDE=true). Using policy file mode: ${policyEngine.getMode()}`));
                }
                else {
                    const { load } = await import('js-yaml');
                    ...
                    console.error(chalk.yellow(`Policy mode overridden in memory (file on disk unchanged): ${opts.blockingMode}`));
                }
            }
    ...
    void (async () => {
        ...
        const reload = async () => {
            ...
            try {
                ...
                if (!opts.gateway && stdioCount > 1) {
                    console.error(chalk.yellow(`Server reload skipped: ${stdioCount} stdio servers — use mastyf-ai start (Fleet Hub) for multi-server setups.`));
                    return;
                }
        ...
        watch(uiConfigDir, { awaitWriteFinish: { stabilityThreshold: 300, pollInterval: 100 } }).on('change', (changedPath) => {
            if (String(changedPath).endsWith('servers.json'))
                void reload();
        });
    ...
    if (isAiLearningEnabled()) {
        ...
        initializeAiEngine(db, servers)
            ...
            try {
                ...
                if (warmup.seeded > 0) {
                    console.error(chalk.dim(`[learning-warmup] ${warmup.seeded} corpus samples → ${warmup.semanticRecords} semantic audit(s)`));
                }
            }
            catch (err) {
                console.error(chalk.yellow(`Learning warmup warning: ${err instanceof Error ? err.message : String(err)}`));
            }
        })
            .catch((err) => {
    ...
    try {
        const cpClient = createControlPlaneClient({
            onPolicyUpdate: (yaml, _version) => {
                if (policyWatcher) {
                    policyWatcher.reloadFromYamlString(yaml);
                }
            },
            onLicenseUpdate: (tier, _features, maxInstances) => {
            ...
            onAuditSnapshot: async () => {
                ...
                for (const r of blocked) {
                    ...
                    if (r.block_rule)
                        ruleCounts[r.block_rule] = (ruleCounts[r.block_rule] || 0) + 1;
                }
                return {
...
vulnCmd
    .command('purge-noise')
    .description('Reject numeric/ancient CVE noise findings and delete poisoned nvd_* disk caches under ~/.mastyf-ai/cve-cache')
    .action(async () => {
    const { runVulnCli } = await import('./cli/vuln.js');
...
program
    ...
    .action(async (opts) => {
    if (opts.runHarness)
        process.env.MASTYF_AI_BENCH_RUN_HARNESS = 'true';
