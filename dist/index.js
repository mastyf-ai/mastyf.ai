...
import { buildObservedTools, computeMeasuredPerformance, filterRecordsByWindow, } from './utils/real-metrics.js';
...
async function getRecentServerRecords(serverName, tenantId, windowDays = DEFAULT_METRIC_WINDOW_DAYS) {
...
function noTrafficMessage(serverName, windowDays = DEFAULT_METRIC_WINDOW_DAYS) {
...
const server = new Server({ name: 'mastyf-ai', version: readPackageVersion() }, { capabilities: { tools: {}, resources: {}, prompts: {}, logging: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
        ...
        case 'start_behavior_observation': {
            const window = container.behaviorCollector.startWindow(args?.windowId);
        ...
        case 'stop_behavior_observation': {
            ...
            if (!window)
                return { content: [{ type: 'text', text: 'No active observation window to stop.' }] };
        ...
        case 'generate_policy_from_observations': {
            ...
            if (windows.length === 0)
                return { content: [{ type: 'text', text: 'No observation data available. Start an observation window first using start_behavior_observation.' }] };
        ...
        case 'suggest_policy_improvements': {
            const existingYaml = args?.existingPolicyYaml || null;
            const windows = container.behaviorCollector.getHistory();
            if (windows.length === 0)
                return { content: [{ type: 'text', text: 'No observation data available.' }] };
        ...
        case 'observation_status': {
            ...
            if (!summary)
                return { content: [{ type: 'text', text: 'No active observation window.' }] };
        ...
        case 'scan_prompt_injection': {
            ...
            const serverName = args?.serverName || 'unknown';
        ...
        case 'predict_threats': {
            ...
            if (targetServers.length === 0)
                return { content: [{ type: 'text', text: 'No matching servers found.' }] };
        ...
        case 'threat_forecast_for_server': {
            const srv = servers.find(s => s.name === args?.serverName);
            if (!srv)
                return { content: [{ type: 'text', text: `Server "${args?.serverName}" not found.` }] };
        ...
        case 'detect_drift': {
            const sn = args?.serverName || servers[0]?.name;
            if (!sn)
                return { content: [{ type: 'text', text: 'No servers available for drift detection.' }] };
            const baseline = container.driftDetector.getLatestBaseline(sn);
            if (!baseline)
                return { content: [{ type: 'text', text: `No baseline captured for "${sn}". Use capture_baseline first.` }] };
        ...
        case 'capture_baseline': {
            const sn = args?.serverName;
            const srv = servers.find(s => s.name === sn);
            if (!srv)
                return { content: [{ type: 'text', text: `Server "${sn}" not found.` }] };
            const records = await getRecentServerRecords(sn, tenantId);
        ...
        case 'rollback_server_config': {
            const sn = args?.serverName;
            const baseline = container.driftDetector.getLatestBaseline(sn);
            if (!baseline?.configSnapshot)
                return { content: [{ type: 'text', text: `No rollback baseline available for "${sn}".` }] };
        ...
        case 'generate_compliance_evidence': {
            ...
            const posture = container.controlMapper.evaluate(framework, [], []);
        ...
        case 'run_self_assessment': {
            const count = args?.attackCount || 50;
        ...
        case 'schedule_red_team': {
            const interval = args?.intervalHours || 24;
        ...
        case 'ab_test_policy': {
            const proposed = args?.proposedPolicyYaml || '';
        ...
        case 'contribute_threat_signature': {
            ...
            const sig = container.threatMeshNode.submitObservation(pattern, category, severity);
        ...
        case 'deploy_honeypot': {
            const hpName = args?.name;
            const template = args?.template || 'decoy-api-server';
            ...
            const tools = container.honeypotManager.getTemplateTools(template);
        ...
        case 'honeypot_report': {
            const hpId = args?.honeypotId;
            if (hpId) {
                const hp = container.honeypotManager.get(hpId);
                if (!hp)
                    return { content: [{ type: 'text', text: `Honeypot "${hpId}" not found.` }] };
        ...
        case 'negotiate_agent_trust': {
            const remoteId = args?.remoteAgentId;
            const requestedTools = args?.requestedTools || [];
            const maxMin = args?.maxSessionMinutes || 30;
            const result = container.trustProtocol.negotiate({ agentId: 'mastyf-ai-local', mastyfAiInstance: 'local', capabilities: ['scan', 'audit', 'protect'] }, { agentId: remoteId, mastyfAiInstance: 'remote', capabilities: requestedTools }, { requestedTools, scope: {}, maxSessionMinutes: maxMin });
        ...
        case 'compute_trust_score': {
            const sn = args?.serverName || servers[0]?.name || 'unknown';
            const score = container.mastyfAiScore.compute({
                serverName: sn,
                cveCount: args?.cveCount || 0,
                maxCvss: args?.maxCvss || 0,
                newestCveAgeDays: 0,
                authMethod: args?.authMethod || 'none',
                transport: args?.transport || (args?.transport ? args.transport : 'stdio'),
                highRiskToolCount: 0,
        ...
        case 'scan_response_dlp': {
            const respText = args?.responseText || '';
            const tool = args?.toolName || 'unknown';
            const srv = args?.serverName || 'unknown';
        ...
        case 'certify_server': {
            const sn = args?.serverName || servers[0]?.name || 'unknown';
            const pkg = args?.packageName || servers[0]?.packageName || sn;
            const ver = args?.version || 'latest';
            const srv = servers.find((s) => s.name === sn) ?? servers[0];
            let cert;
            if (args?.trustScore != null) {
                const trustScore = args.trustScore || 50;
                const complianceScore = args.complianceScore || 0;
                const cveFree = args.cveFree !== false;
                const authMethod = args.authMethod || 'none';
                const transport = args.transport || (srv?.transport || 'stdio');
                ...
                cert = container.certifier.certifyFromScan(sn, pkg, ver, {
                    ...
                    maxCvss: scan.cves.reduce((m, c) => Math.max(m, c.cvssScore ?? 0), 0),
                    authMethod: 'none',
        ...
        case 'verify_certification': {
            const sn = args?.serverName || '';
            const v = container.certifier.verifyCertification(sn, args?.attestationJws);
        ...
        case 'declare_intent': {
            ...
            const binding = container.intentEngine.declareIntent(sessionId, intent, allowedTools, {
                ...
                ttlMs: (args?.ttlMinutes || 30) * 60_000,
            });
        ...
        case 'run_protocol_fuzzer': {
            const blockFn = (method, params) => ({ blocked: false });
            const sn = args?.serverName || servers[0]?.name || 'local';
        ...
        case 'check_sla': {
            const sn = args?.serverName || servers[0]?.name || 'unknown';
            const tn = args?.toolName || 'read_file';
            const records = (await getRecentServerRecords(sn, tenantId))
        ...
        case 'run_incident_playbook': {
            const trigger = args?.trigger || 'test';
            const playbook = args?.playbook || 'prompt_injection';
            const severity = args?.severity || 'high';
            const report = container.incidentPlaybook.run(trigger, 'dashboard', severity, playbook);
        ...
        case 'harden_config': {
            ...
            const srv = servers.find(s => s.name === sn) || servers[0];
            if (!srv)
                return { content: [{ type: 'text', text: 'No server found.' }] };
        ...
        case 'policy_to_natural_language': {
            ...
            const summary = yaml
                ...
                : await explainPolicyFile(args?.policyPath);
        ...
        case 'query_server_reputation': {
            ...
            const entry = container.reputationNetwork.queryServerReputation(sn, args?.packageName);
            if (!entry) {
        ...
        case 'tune_policy_rule': {
            const serverType = args?.serverType || 'filesystem';
            const agentTier = args?.agentTier || 'standard';
            const ruleCategory = args?.ruleCategory || 'shell_injection';
        ...
        case 'adapt_threshold': {
            const parameter = args?.parameter || 'rateLimit';
            const blockRate = args?.blockRate || 0.3;
            const fpRate = args?.fpRate || 0.05;
            const callVolume = args?.callVolume || 0.5;
            const state = { blockRate, fpRate, callVolume };
            const decision = container.sarsaThresholds.decide(parameter, state);
        ...
        case 'select_fuzz_strategy': {
            ...
            if (args?.observeReward !== undefined) {
                container.reinforceFuzzer.observe(args.observeReward);
            }
