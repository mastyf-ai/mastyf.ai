 ...
 */
function attemptDecodePass(input) {
    ...
    if (/\\x[0-9a-fA-F]{2}/.test(input)) {
        const decoded = input.replace(/\\x([0-9a-fA-F]{2})/g, (_, hex) => String.fromCharCode(parseInt(hex, 16)));
        if (decoded !== input) {
            return {
                changed: true,
                output: decoded,
    ...
    if (/\\u[0-9a-fA-F]{4}/.test(input)) {
        const decoded = input.replace(/\\u([0-9a-fA-F]{4})/g, (_, uHex) => String.fromCharCode(parseInt(uHex, 16)));
        if (decoded !== input) {
            return {
                changed: true,
                output: decoded,
 ...
 */
export function multiStageDecode(input, maxPasses = 5) {
    const stages = [];
    let current = input;
...
//# sourceMappingURL=multi-stage-decoder.js.map