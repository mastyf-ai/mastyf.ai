 ...
 */
export function scanToolCallArguments(args, options) {
...
function matchInjectionPatterns(decodedBody, seen, criticalOnly = false) {
    const findings = [];
    const baseBodies = injectionMatchVariants(decodedBody);
    for (const pattern of getPatterns()) {
        if (criticalOnly && pattern.severity !== 'critical')
            continue;
...
export function detectPromptInjection(_toolName, responseBody) {
    const seen = new Set();
