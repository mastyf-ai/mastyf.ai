...
export function verifyChain(fromDate) {
    const dir = receiptDir();
    if (!existsSync(dir))
        return { valid: true, chainLength: 0, tampered: false };
    const allReceipts = [];
    ...
    for (const file of files) {
        ...
        for (const line of lines) {
            try {
                allReceipts.push(JSON.parse(line));
            }
...
export function exportReceipts(format) {
    const dir = receiptDir();
    if (!existsSync(dir))
        return '';
    const all = [];
