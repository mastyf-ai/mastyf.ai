...
import { getServerRegistry } from './server-registry.js';
import { buildChartMeta } from './chart-meta.js';
import { bucketGranularityForWindow, fillTimeSeries, generateTimeBuckets, parseRecordTimestamp, parseWindowDays, windowRangeMs, windowToLabel, } from './time-buckets.js';
...
function mapTransport(t) {
    ...
    if (lower.includes('mtls'))
        return 'mTLS';
    if (lower.includes('https'))
        return 'https';
    if (lower.includes('http') || lower.includes('sse') || lower.includes('ws'))
        return 'http';
    return 'stdio';
}
function computeServerTrust(container, server) {
...
export async function buildAgenticDashboardSummary(db, container, tenantId, windowDaysInput) {
    ...
    const frameworks = container
        ? ['soc2', 'hipaa', 'pci-dss', 'fedramp', 'iso27001'].map((f) => container.controlMapper.evaluate(f, [], []))
        : [];
    const complianceOverall = frameworks.length > 0
    ...
    if (container)
        dataSources.push('agentic-container');
...
//# sourceMappingURL=agentic-dashboard-summary.js.map