...
function measuredOverview(summary) {
    const bullets = [];
    bullets.push(`${summary.totalRequests.toLocaleString()} measured proxy calls across ${summary.activeServers} server(s)${summary.passRatePct != null ? `; pass rate ${summary.passRatePct}%` : ''}.`);
    if (summary.budgetUsd != null && summary.budgetUtilizationPct != null) {
        bullets.push(`Budget utilization ${summary.budgetUtilizationPct}% of $${summary.budgetUsd.toFixed(2)} daily cap (measured spend only).`);
    }
    if (summary.topToolsByCalls[0]) {
        bullets.push(`Highest-volume tool: ${summary.topToolsByCalls[0].tool} (${summary.topToolsByCalls[0].calls} calls).`);
    }
    return bullets.slice(0, 5);
}
function measuredCost(summary) {
    if (summary.totalCostUsd <= 0)
        return [];
    const bullets = [`Total measured spend: $${summary.totalCostUsd.toFixed(4)}.`];
    ...
    if (summary.budgetUsd != null) {
        bullets.push(summary.budgetUtilizationPct != null && summary.budgetUtilizationPct >= 100
...
function measuredSecurity(overallScore, activeThreats) {
    const bullets = [];
...
function measuredAudit(blocked, total, heatmapTop) {
    if (total === 0)
        return [];
    ...
    if (heatmapTop)
        bullets.push(`Top block pattern: ${heatmapTop}.`);
    return bullets.slice(0, 4);
}
function measuredBullets(scope, summary, securityScore, activeThreats, auditBlocked, auditTotal, heatmapTop) {
    switch (scope) {
        case 'cost':
        ...
        case 'audit':
            return measuredAudit(auditBlocked, auditTotal, heatmapTop ? `${heatmapTop.rule} on ${heatmapTop.tool} (${heatmapTop.count}×)` : '');
        case 'ai':
...
function hasMeasuredDataForScope(scope, summary, securityScore, activeThreats, auditBlocked, auditTotal) {
    switch (scope) {
        case 'cost':
...
function isInsightsLlmEnabled() {
    return process.env.MASTYF_AI_INSIGHTS_LLM === 'true';
}
function buildRagCorpus(scope, summary, bullets, heatmapTop) {
    const citations = [];
    citations.push({
...
async function buildLlmNarrative(scope, citations) {
    ...
    const result = await llm.generate(`You write board-ready MCP security briefings. Use ONLY the cited facts below. Every sentence must include at least one citation ID in brackets like [exec:totalRequests]. No invented statistics.`, `Scope: ${scope}\n\nFacts:\n${corpus}\n\nWrite 2-3 sentences for executives:`);
    if (!result?.text)
        return null;
...
export async function buildDashboardInsights(db, tenantId, scope, opts) {
    ...
    const bullets = measuredBullets(scope, summary, securityScore, activeThreats, auditBlocked, auditTotal, heatmapTop);
    const emptyPayload = {
        scope,
...
export function formatInsightsBriefingMarkdown(payload) {
    const lines = [
    ...
    if (payload.windowDays != null)
        lines.push(`- **Window:** ${payload.windowDays} days`);
    lines.push('');
...
export function buildDeterministicInsightsOnly(scope, summary, opts) {
    const securityScore = opts?.securityScore ?? null;
    const activeThreats = opts?.activeThreats ?? 0;
    const auditBlocked = opts?.auditBlocked ?? 0;
    const auditTotal = opts?.auditTotal ?? 0;
