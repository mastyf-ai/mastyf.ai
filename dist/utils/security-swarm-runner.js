...
function analysisPath(tenantId) {
    return join(swarmDir(tenantId), 'analysis.txt');
}
function loadJobFile(tenantId) {
...
function readLogTail(tenantId, maxLines = 50) {
...
function markSwarmJobFailed(tenantId, job, error, exitCode = 1) {
    const next = {
        ...job,
        state: 'failed',
 ...
 */
export function reconcileStaleSwarmJob(tenantId = DEFAULT_TENANT_ID) {
    const job = loadJobFile(tenantId);
    if (!job || job.state !== 'running')
        return false;
    const pid = job.pid != null ? Number(job.pid) : null;
    if (pid && !isProcessAlive(pid)) {
        markSwarmJobFailed(tenantId, job, 'Analysis process exited unexpectedly (orphaned job — re-run Security Swarm)');
        return true;
    }
    const logPath = join(swarmDir(tenantId), 'job.log');
    let logMtime = 0;
    ...
    if (logStale || ageStale) {
        markSwarmJobFailed(tenantId, job, logStale
...
function getWatcherState(tenantId) {
...
export function getSwarmJobStatus(tenantId = DEFAULT_TENANT_ID) {
...
export function isSwarmJobRunning(tenantId = DEFAULT_TENANT_ID) {
    reconcileStaleSwarmJob(tenantId);
    const job = loadJobFile(tenantId);
    return job?.state === 'running';
}
function checkIncrementalArtifacts(tenantId) {
    ...
    for (const name of WATCHED_ARTIFACTS) {
        ...
        try {
            ...
            if (state.artifactMtime.get(key) === m)
                continue;
...
function broadcastSwarmJob(tenantId, job) {
    if (!job)
        return;
    const state = getWatcherState(tenantId);
    ...
    if (jobState !== state.lastBroadcastState && (jobState === 'done' || jobState === 'failed')) {
        ...
        if (watchedTenants.size === 0)
            stopSwarmJobWatcher();
    }
}
function tickSwarmJobWatcher(tenantId) {
    reconcileStaleSwarmJob(tenantId);
    const job = loadJobFile(tenantId);
    if (!job || job.state !== 'running') {
        if (job)
...
function tickAllWatchers() {
    for (const tenantId of [...watchedTenants]) {
        tickSwarmJobWatcher(tenantId);
    }
}
export function startSwarmJobWatcher(tenantId = DEFAULT_TENANT_ID) {
...
export function stopSwarmJobWatcher() {
    if (jobWatchTimer) {
...
export function startSwarmAnalysis(opts = {}) {
    ...
    writeFileSync(join(swarmOut, 'job.json'), JSON.stringify({
        jobId,
        state: 'running',
        phase: 'preflight',
    ...
    if (existingAfterSpawn?.jobId === jobId && child.pid) {
        writeFileSync(join(swarmOut, 'job.json'), JSON.stringify({ ...existingAfterSpawn, pid: child.pid }, null, 2));
    }
    ...
    child.once('exit', (code, signal) => {
        setTimeout(() => {
            ...
            if (!existingJob || existingJob.state !== 'running')
                return;
            if (existingJob.jobId !== jobId)
                return;
            markSwarmJobFailed(tenantId, existingJob, code === 0
                ? 'Analysis process ended without writing final job state — re-run Security Swarm'
                : `Analysis process exited ${code ?? signal ?? 'unknown'} — check job.log and gate-pro.mjs / pnpm build`, code ?? 1);
            tickSwarmJobWatcher(tenantId);
        }, 2500);
    });
...
export function readAnalysisReport(tenantId = DEFAULT_TENANT_ID) {
...
//# sourceMappingURL=security-swarm-runner.js.map