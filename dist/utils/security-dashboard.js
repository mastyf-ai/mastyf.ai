import { getAllActiveServerNames, parseSecurityScanDetails, summarizeRecords, } from './db-aggregate.js';
...
function ruleToThreatType(rule, reason) {
    ...
    if (r.includes('privilege') || r.includes('escalation'))
...
function severityFromRule(rule) {
    ...
    if (r.includes('semantic') || r.includes('secret') || r.includes('injection'))
        return 'critical';
    if (r.includes('shell') || r.includes('path') || r.includes('sql'))
        return 'high';
    if (r.includes('auth') || r.includes('brute'))
        return 'high';
    if (r.includes('warn'))
        return 'medium';
    return 'medium';
}
function pseudoSource(serverName, toolName) {
...
export function threatDisplayFingerprint(row) {
    return `${row.type}:${row.source}`;
}
function buildThreatsFromRecords(blocked) {
...
async function buildThreatsFromSemantic(tenantId) {
...
async function gatherThreatCandidates(db, tenantId, windowDays) {
...
export async function listMonitorThreatCandidates(db, tenantId, windowDaysInput) {
    if (!db)
        return [];
...
export function filterRelatedMonitorThreats(candidates, anchor) {
...
export async function listRelatedMonitorThreatsForQuarantine(db, tenantId, anchor, windowDaysInput = DEFAULT_SECURITY_MONITOR_WINDOW) {
...
export function collectBulkQuarantineTargets(candidates, visibleThreats) {
    const visibleHigh = visibleThreats.filter((t) => t.severity === 'critical' || t.severity === 'high');
    ...
    for (const row of candidates) {
        if (row.severity !== 'critical' && row.severity !== 'high')
            continue;
        if (!targetFingerprints.has(threatDisplayFingerprint(row)))
            continue;
        targetsByKey.set(row.threatKey, row);
    }
    return [...targetsByKey.values()];
}
export async function listBulkQuarantineTargets(db, tenantId, windowDaysInput = DEFAULT_SECURITY_MONITOR_WINDOW, policyMode) {
    const windowDays = parseWindowDays(windowDaysInput);
...
export function filterVisibleMonitorThreats(candidates, quarantine) {
    return applyThreatVisibilityFilter(candidates, quarantine);
}
function applyThreatVisibilityFilter(candidates, quarantine) {
    const suppressedKeys = quarantine.quarantinedKeys();
    const suppressedFingerprints = new Set(quarantine.list(365).map((e) => threatDisplayFingerprint(e)));
    const seen = new Set();
    const threats = [];
    for (const t of candidates) {
        if (suppressedKeys.has(t.threatKey))
            continue;
        ...
        threats.push(t);
        if (threats.length >= 12)
            break;
    }
    return threats;
}
function p50Latency(records) {
    const lat = records.map((r) => r.durationMs || 0).filter((n) => n > 0).sort((a, b) => a - b);
    if (!lat.length)
        return null;
    return lat[Math.floor(lat.length / 2)] ?? null;
}
export async function buildSecurityDashboard(db, tenantId, windowDaysInput, opts) {
    const windowDays = parseWindowDays(windowDaysInput);
    ...
    if (scanned > 0)
        manifestScore = Math.round(scoreSum / scanned);
    ...
    const authIntegrity = sum.total > 0 ? Math.round((sum.passed / sum.total) * 1000) / 10 : 100;
    const quarantinedEntries = quarantine.list(365);
    const quarantinedCount = quarantinedEntries.length;
    const quarantinedGroups = new Set(quarantinedEntries.map((e) => threatDisplayFingerprint(e))).size;
    const summaryLines = [
        sum.blocked > 0
