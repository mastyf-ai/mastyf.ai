...
import { ensureThreatLabLlmReady, threatLabEnabled, threatLabMode, threatLabMaxCandidates, threatLabSemanticEnabled, } from '../ai/threat-lab.js';
import { getThreatResearchConfig, getThreatResearchQueueStatus, threatResearchAutoEnabled, } from '../ai/threat-research-pipeline.js';
import { countProcessedFingerprints } from '../ai/auto-corpus-writer.js';
import { readThreatLabCandidates, readThreatLabCandidatesUngated, readAutoCorpusManifest, } from './swarm-artifacts.js';
...
async function cachedLlmHealth() {
    const now = Date.now();
...
function aggregateThreatLab(candidates) {
    const reviewStatuses = candidates.map((c) => c.reviewStatus || 'pending');
    const sources = candidates.map((c) => c.provenance?.source || 'unknown');
...
function aggregateAutoCorpus(entries) {
    const now = Date.now();
    const dayAgo = now - 24 * 60 * 60 * 1000;
    const last24h = entries.filter((e) => Date.parse(e.timestamp) >= dayAgo).length;
...
export async function buildThreatDiscoveryStatus(tenantId) {
    ...
    const hasSwarm = getLicenseClient().hasFeature('swarm')
        || isCiLicenseBypass()
        || isCiTokenCached();
...
export function resetThreatDiscoveryStatusCacheForTests() {
    llmCache = null;
}
//# sourceMappingURL=threat-discovery-status.js.map