...
function classifySkipReason(line) {
    const lower = line.toLowerCase();
    if (lower.includes('duplicate fingerprint'))
        return 'duplicate';
    if (lower.includes('fixture write skipped'))
        return 'duplicate';
    if (lower.includes('below min confidence'))
        return 'belowMinConfidence';
    if (lower.includes('replay smoke test failed') || lower.includes('not blocked by current policy')) {
        return 'replayFailed';
    }
    if (lower.includes('llm unavailable') || lower.includes('llm disabled'))
...
export function parseAutoResearchLogTail(logTail) {
    const lines = String(logTail || '')
        .split('\n')
        .map((line) => line.trim())
        .filter(Boolean);
    const summaryLine = [...lines].reverse().find((line) => WRITE_SUMMARY_RE.test(line) || line.includes('[auto-threat-research] wrote')) || null;
...
//# sourceMappingURL=parse-auto-research-log.js.map