...
export function isLikelyPackageArg(arg) {
 ...
 */
export function extractPackagesFromServer(server) {
    const found = new Set();
    ...
    if (command === 'npx' || command.endsWith('/npx')) {
        for (const arg of args) {
            ...
            if (arg.startsWith('@') && arg.includes('/'))
                found.add(arg);
            else if (BARE_NPM.test(arg) && arg.length >= 2)
                found.add(arg);
        }
    }
    if (command === 'uvx' || command === 'uv' || command.includes('python')) {
        for (const arg of args) {
            if (!isLikelyPackageArg(arg))
                continue;
            if (!arg.includes('/') && arg.length > 1 && !/^[\d.]+$/.test(arg))
                found.add(arg);
        }
    }
    return [...found];
...
export function findLockfileNearPath(startPath, maxUp = 4) {
...
export function resolvePackageRootFromServer(server) {
