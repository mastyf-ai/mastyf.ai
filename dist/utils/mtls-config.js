...
export function getActiveSpiffeId() {
    const fromEnv = process.env['MASTYF_AI_SPIFFE_ID']?.trim();
    if (fromEnv?.startsWith('spiffe://'))
        return fromEnv;
...
export function resetSpiffeSvidCacheForTests() {
    spiffeLoaded = false;
 ...
 */
export async function fetchSpiffeSvidFromWorkloadApi() {
    const socketPath = process.env['MASTYF_AI_SPIFFE_SOCKET_PATH']?.trim();
    if (!socketPath || spiffeLoaded)
        return spiffeLoaded;
...
function writeTempPem(kind, pemBody) {
 ...
 */
export function resolveMtlsEnvFromMounts() {
