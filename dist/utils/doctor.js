...
import { validateSignedPolicyYaml } from '../policy/policy-signature.js';
...
function validatePolicyAtPath(resolvedPolicy) {
    let issues = 0;
    try {
...
export function runDoctor(opts = {}) {
    ...
    if (process.env.MASTYF_AI_ENTERPRISE_MODE === 'true' && !isFieldEncryptionEnabled()) {
        console.log(chalk.yellow('  Enterprise mode: MASTYF_AI_DB_ENCRYPTION_KEY unset — audit block_reason/args stored in plaintext'));
        issues++;
    }
