 ...
 */
function getCostEfficiency(costs) {
    ...
    if (avgModelCost <= 5.0)
        return 50;
    if (avgModelCost <= 15.0)
        return 30;
    if (avgModelCost <= 40.0)
        return 15;
    return 5;
}
export function calculateOverallScore(security, health, costs) {
    ...
    if (costs && costs.length > 0) {
        ...
        if (security.length === 0 && health.length === 0)
            return Math.round(costEff);
        if (security.length === 0)
            return Math.round((healthAvg * 0.5) + (costEff * 0.5));
        if (health.length === 0)
            return Math.round((secAvg * 0.6) + (costEff * 0.4));
