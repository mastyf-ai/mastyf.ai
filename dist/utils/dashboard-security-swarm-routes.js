...
export async function handleDashboardSecuritySwarmRoutes(params) {
    const { url, method, req, res, requestTenantId, policyWatcher, writeJson, readBody, setCors, assertFeature, } = params;
    if (!url.startsWith('/api/security-swarm/'))
        return false;
    if (!assertFeature(url, 'swarm', res, setCors))
        return true;
    if (url === '/api/security-swarm/run' && method === 'POST') {
        ...
        void import('./dashboard-log-writer.js').then(({ writeLogEntry }) => {
            writeLogEntry(requestTenantId, 'info', 'swarm', `Swarm analysis ${body.full ? 'full' : 'incremental'} analysis started`, {
                source: 'swarm',
                details: `Job ${result.jobId} started`,
                metadata: { jobId: result.jobId, full: !!body.full, startedAt: result.startedAt },
            });
        });
        return true;
    }
    ...
    if (url === '/api/security-swarm/job-log' && method === 'GET') {
        ...
        const steps = readSwarmJsonFile('steps.json', requestTenantId);
    ...
    if (url === '/api/security-swarm/report' ||
        ...
        if (url.endsWith('/download')) {
            res.setHeader('Content-Disposition', 'attachment; filename="mastyf-ai-swarm-analysis.txt"');
        }
        res.end(report.text);
        return true;
    }
    ...
    if (url === '/api/security-swarm/threat-lab-candidates/accept' && method === 'POST') {
        ...
        const policyRule = candidate.policyRule;
        if (!policyRule?.name) {
            writeJson(res, 400, {
                ok: false,
        ...
        const result = await applySuggestionToPolicy(policyRule, policyPath, policyWatcher ?? null, { tenantId: requestTenantId });
...
//# sourceMappingURL=dashboard-security-swarm-routes.js.map