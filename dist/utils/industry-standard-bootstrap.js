...
import { ingestFleetHeartbeatIntoObservatory, ingestMastyfAiBenchIntoObservatory, ingestMtxCatalogIntoObservatory, ingestCloudObservatoryRelay, } from '../agentic/observatory/observatory-ingest.js';
...
export function registerIndustryStandardTasks(container) {
    ...
    if (process.env.MASTYF_AI_THREAT_MESH_ENABLED === 'true') {
        ...
        agenticScheduler.register('mtx-relay-sync', 'MTX Threat Mesh Relay Sync', interval, async () => {
            ...
            Logger.info(`[IndustryStandard] MTX relay sync: published=${result.published} pulled=${result.pulled} connected=${result.relayConnected}`);
            resetThreatIntelGuardCache();
        });
    }
    if (process.env.MASTYF_AI_COMPLIANCE_CRON !== 'false') {
        ...
        agenticScheduler.register('compliance-evidence', 'Compliance Evidence Collection', cron, async () => {
            ...
            for (const fw of frameworks) {
                ...
                if (!['soc2', 'hipaa', 'pci-dss', 'fedramp', 'iso27001'].includes(framework))
                    continue;
    ...
    agenticScheduler.register('observatory-ingest', 'Ecosystem Observatory Telemetry Ingest', observatoryInterval, async () => {
        ...
        ingestMtxCatalogIntoObservatory(ecosystemObservatory, mtxHashes.map(hash => ({ category: 'mtx-signature', severity: hash.slice(0, 8) })));
...
//# sourceMappingURL=industry-standard-bootstrap.js.map