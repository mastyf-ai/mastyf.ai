...
import { readAutoCorpusManifest, readThreatLabCandidates, readThreatLabCandidatesUngated, } from './swarm-artifacts.js';
...
function aggregateCorpus(entries) {
    const now = Date.now();
...
function parseThreatLabWritten(logTail) {
    const match = logTail.match(/wrote\s+(\d+)\s+authentic/i);
    return match ? Number(match[1]) : null;
}
function aggregateLearning(events) {
...
export async function buildThreatAutomationSummary(tenantId) {
...
//# sourceMappingURL=threat-automation-summary.js.map