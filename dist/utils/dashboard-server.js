...
import { signPolicyYaml, validateSignedPolicyYaml, } from '../policy/policy-signature.js';
...
import { resolveDashboardBindHost, shouldRefuseUnauthenticatedDashboardBind, } from './dashboard-bind.js';
...
import { DashboardAuth, SESSION_COOKIE_NAME, } from '../auth/dashboard-auth.js';
import { assertTenantAdminScope, canAccessRoute, } from '../auth/dashboard-rbac.js';
...
import { getLicenseClient, isLicenseEnforcementEnabled, loadLicenseClientConfig, } from '../license/license-client.js';
import { isCiLicenseBypass, isOpenCoreEnabled, } from '../license/feature-tiers.js';
...
import { getAllActiveServerNames, loadAllCallRecords, securityRowFromScan, summarizeRecords, } from './db-aggregate.js';
...
import { buildDashboardInsights } from './dashboard-insights.js';
...
import { listFederatedRegions, resolveFederatedChartDb, } from './federated-data-source.js';
import { buildObservedTools, computeMeasuredPerformance, } from './real-metrics.js';
...
function deployDir() {
    const fromEnv = process.env['MASTYF_AI_DEPLOY_DIR'];
    const candidates = [
...
function scanTimestampMs(value) {
    if (!value)
        return 0;
...
function latestScanTimestamp(...candidates) {
    let best = null;
    let bestMs = 0;
...
function serveDashboardAsset(spaRoot, relPath, res, method = 'GET') {
...
function writeSpaNotFound(res) {
    res.writeHead(404, { 'Content-Type': 'application/json; charset=utf-8' });
    res.end(JSON.stringify({ error: 'Static asset not found' }));
}
function tryServeDashboardSpa(url, res, opts) {
    const dir = deployDir();
    if (!dir)
        return false;
    ...
    if (url.startsWith('/_next/')) {
        ...
        if (!ok && opts?.notFoundOnMiss)
            writeSpaNotFound(res);
        return ok || !!opts?.notFoundOnMiss;
    }
    if (url === '/favicon.ico') {
        if (serveDashboardAsset(spaRoot, 'favicon.ico', res, method))
            return true;
    ...
    if (url === '/logo.png' || url === '/logo.jpeg' || url === '/logo.jpg' || url === '/icon.png') {
        const rel = url.slice(1);
        if (serveDashboardAsset(spaRoot, rel, res, method))
            return true;
        if (spaRoot !== legacyRoot && serveDashboardAsset(legacyRoot, rel, res, method))
            return true;
    }
    if (!url.startsWith('/dashboard-spa/'))
        return false;
...
function tryServeSwarmArtifact(url, res, method = 'GET') {
    let root = null;
    let rel = null;
    ...
    if (!existsSync(filePath))
        return false;
    if (!isSwarmArtifactVisibleForSession(filePath, tenantId))
        return false;
    const mime = SPA_MIME[extname(filePath)] || 'application/octet-stream';
...
function tryServeDocsAsset(url, res, method = 'GET') {
...
function loopbackDashboardOrigins() {
    const ports = new Set(['4000', '3000', '14000']);
    const extra = String(process.env['DASHBOARD_PORT'] || process.env['MASTYF_AI_PORT'] || '').trim();
    if (/^\d+$/.test(extra))
        ports.add(extra);
    const origins = [];
...
let dashboardAiEnginePromise = null;
function touchDashboardAiEngine() {
    if (!runtimeHistoryDb)
...
export function setDashboardDataSource(historyDb) {
...
function parseRegionParam(q) {
    const region = q.get('region')?.trim();
    return region || undefined;
}
async function resolveChartContext(tenantId, windowDays, region) {
    return resolveFederatedChartDb(runtimeHistoryDb, tenantId, windowDays, region);
}
function mergeFedMeta(meta, fed) {
    return {
        ...(meta || {}),
...
export async function startDashboardServer(port = 4000, policyWatcher, dashboardAuth) {
    ...
    if (process.env['DASHBOARD_AUTH_DISABLED'] === 'true'
        ...
        Logger.error('[license] DASHBOARD_AUTH_DISABLED is not allowed when cloud license enforcement is enabled');
    }
    ...
    if (dashboardEnabled
        ...
        Logger.error('[license] DASHBOARD_ENABLED requires a valid cloud API key when MASTYF_AI_REQUIRE_LICENSE=true');
        dashboardEnabled = false;
    }
    ...
    if (dashboardEnabled && deployRoot && !isReactDashboardBuilt(deployRoot)) {
        Logger.warn('[dashboard] React dashboard not built — run pnpm dashboard:build (serving legacy static shell until out/ exists)');
    }
    ...
    function isLicenseExemptPath(path) {
        return (path === '/api/license/status'
            || path === '/api/auth/cloud-exchange'
            || path === '/api/auth/csrf'
            || path === '/login');
    }
    function assertLicensedApi(path, res, setCors) {
        if (isLicenseExemptPath(path))
            return true;
        if (isCiLicenseBypass() || isCiTokenCached())
            return true;
        if (!isLicenseEnforcementEnabled() && licenseClient.isLicensed())
            return true;
    ...
    function assertFeature(_path, feature, res, setCors) {
        if (licenseClient.hasFeature(feature))
            return true;
        setCors();
        writeJson(res, 402, {
    ...
    if (authRequired && auth.isConfigured()) {
        ...
        Logger.error('[dashboard] Dashboard authentication required but DASHBOARD_API_KEY or DASHBOARD_JWT_SECRET is missing — all API requests will be rejected until configured');
    }
    else {
        Logger.warn('[dashboard] Dashboard API is UNauthenticated (DASHBOARD_AUTH_DISABLED=true) — do not expose to a network');
    }
    ...
    function writeCloudExchangeError(req, res, status, message) {
        if (prefersHtml(req)) {
    ...
    function appendThreatIntelActionAudit(action) {
        try {
    ...
    function isAuthSubsystemPath(pathname) {
        ...
        if (pathname === '/api/auth/cloud-exchange')
            return false;
    ...
    if (!AUTH_DISABLED) {
        ...
        authApiApp.use((req, res, next) => {
            if (req.path === '/api/auth/status' && req.method === 'GET') {
                ...
                res.json = ((body) => originalJson({ ...licenseStatusPayload(), ...body }));
            }
            next();
        });
        ...
        registerFederationRoutes(authApiApp, {
                ...
                createFromIdp: async (data) => {
                    ...
                    try {
                        ...
                        if (err.message?.includes('UNIQUE') || err.message?.includes('unique')) {
                            return (await userStore.findByUsernameOrEmail(email)) || { id: '' };
                        }
                        throw err;
                    }
                },
                updateIdpTokens: async (_userId, _accessToken, _refreshToken, expiresAt) => {
                    ...
                    if (expiresAt) {
                        try {
                            (await import('./persistence-store.js')).getPersistenceStore().saveCustomHook(`sso_expiry_${_userId}`, String(expiresAt), 'error', 0);
                        }
                        catch { }
                    }
                },
            ...
            sessionStore: {
                create: async (userId, ipAddress, userAgent, tenantId) => {
        ...
        Logger.warn('[dashboard] MASTYF_AI_AUTH_DISABLED=true — dashboard login/RBAC is DISABLED. Do not use in production.');
    ...
    const loginRateLimiter = new LRUCache({
        max: 500,
        ttl: 60000,
        updateAgeOnGet: false,
    });
    const apiRateLimiter = new LRUCache({
        max: 10000,
        ttl: 60000,
        updateAgeOnGet: false,
    });
    const dashboardApiRateLimit = () => {
        if (process.env.DASHBOARD_AUTH_DISABLED === 'true') {
            return 5000;
        }
    ...
    function isDashboardRateLimitExempt(path, method) {
        if (method !== 'GET')
            return false;
    ...
    async function checkDashboardApiRateLimit(ip, tenantId) {
        ...
        try {
            ...
            if (url === '/api/auth/cloud-exchange' && method === 'GET') {
                ...
                if (!auth.hasJwtSessionAuth()) {
                    writeCloudExchangeError(req, res, 503, `Set DASHBOARD_JWT_SECRET or MASTYF_AI_CLOUD_JWT_SECRET on this Mastyf AI host (same value as cloud AUTH_SECRET). The cloud console at ${DEFAULT_CLOUD_CONSOLE_URL} does not need this — only self-hosted SSO.`);
                    return;
                }
                const controlPlaneUrl = loadLicenseClientConfig().controlPlaneUrl;
                if (!controlPlaneUrl) {
                    writeCloudExchangeError(req, res, 503, `MASTYF_AI_CONTROL_PLANE_URL not configured (set to ${DEFAULT_CLOUD_CONSOLE_URL})`);
                    return;
                }
                ...
                if (!cloudPayload) {
                    writeCloudExchangeError(req, res, 401, 'Invalid cloud session token — MASTYF_AI_CLOUD_JWT_SECRET must match cloud AUTH_SECRET');
                    return;
                }
                try {
                    const token = auth.createCloudSession(exchanged.tenantSlug ?? cloudPayload.tenantSlug, cloudPayload.identity, mapCloudRoles(cloudPayload.roles));
                    const csrfToken = auth.isCsrfEnforced() ? auth.issueCsrfToken() : undefined;
                    const cookies = [auth.sessionSetCookieHeader(token)];
                    if (csrfToken)
                        cookies.push(auth.csrfSetCookieHeader(csrfToken));
            ...
            if (url === '/login' && method === 'GET') {
                ...
                if (auth.isEnabled() && auth.hasJwtSessionAuth()) {
                    ...
                    if (csrfToken)
                        headers['Set-Cookie'] = auth.csrfSetCookieHeader(csrfToken);
                    res.writeHead(200, headers);
            ...
            if (url === '/api/login' && method === 'POST') {
                ...
                if (auth.isCsrfEnforced()) {
                    const csrfHeaders = auth.csrfHeadersFromForm(req.headers, body['_csrf']);
                ...
                const reqCookies = auth.parseCookies(typeof req.headers.cookie === 'string' ? req.headers.cookie : undefined);
            ...
            const publicUnauth = method === 'GET' && (url === '/api/health' || url === '/api/gateway/bff-info');
            ...
            const tenantScope = assertTenantAdminScope(roles, authResult.sessionTenantId, requestTenantId);
            if (!tenantScope.ok) {
                setCors();
            ...
            if (url === '/api/policy' && method === 'GET') {
                ...
                if (existsSync(sigPath)) {
                    try {
                        signature = JSON.parse(readFileSync(sigPath, 'utf-8'));
                    }
                    catch {
                        signature = null;
                    }
            ...
            if (url === '/api/policy/reload' && method === 'POST') {
                setCors();
                writeJson(res, 200, { status: 'ok', message: 'Policy watcher auto-detects changes' });
                return;
            }
            ...
            if (url === '/api/policy/copilot/replay' && method === 'POST') {
                ...
                const replay = await replayDraftRuleAsync(rule, {
                    tenantId: requestTenantId,
                });
            ...
            if (url === '/api/policy/copilot/counterfactual' && method === 'POST') {
                ...
                const draftRule = body.rule && typeof body.rule === 'object' && 'name' in body.rule && 'action' in body.rule
                    ? body.rule
                    : undefined;
            ...
            }
            {
                const handledNew = await handleNewApiRoutes(url, method, req, res, setCors, {
                    policyEngine: policyWatcher?.get(),
                    tenantId: requestTenantId,
                    db: runtimeHistoryDb,
                });
                if (handledNew)
            ...
            if (url === '/api/incidents/investigate' && method === 'POST') {
                ...
                const triggerType = triggerTypeRaw === 'semantic_flag' ||
            ...
            if (url === '/api/fleet/rug-pull-events' && method === 'PATCH') {
                ...
                const u = new URL(req.url || url, 'http://localhost');
                const body = await readBody(req);
                const eventId = (u.searchParams.get('id') || body.id || '');
            ...
            if (url === '/api/fleet/rug-pull-status' && method === 'GET') {
                ...
                let watched = fleet?.servers?.length ?? 0;
                if (!watched) {
                    try {
                        const fed = await resolveChartContext(requestTenantId, 7);
                        if (fed.db) {
                            const srvs = await getAllActiveServerNames(fed.db, requestTenantId);
                            watched = srvs.filter((s) => !s.includes('..') && !s.includes('passwd') && !s.includes('PROBE')).length;
                        }
                    }
                    catch {
                        // fallback
                ...
                writeJson(res, 200, {
                    ...status,
                    servers_watched: watched,
                    watched,
                    totalServers: watched,
                    open_alerts: status.unreviewed ?? 0,
                    openAlerts: status.unreviewed ?? 0,
                    alerts: status.unreviewed ?? 0,
                });
                return;
            }
            ...
            if (url === '/api/ai/auto-corpus/manifest' && method === 'GET') {
                ...
                if (statusFilter) {
                    entries = entries.filter((e) => statusFilter === 'pending'
            ...
            if (url === '/api/ai/auto-corpus/approve' && method === 'POST') {
                setCors();
                const body = await readBody(req);
                const advId = (body.advId || '');
                if (!advId) {
                    writeJson(res, 400, { error: 'advId required' });
                    return;
                }
            ...
            if (url === '/api/audit/heatmap' && method === 'GET') {
                ...
                try {
                    ...
                    writeJson(res, 200, available({ ...bundle, meta: mergeFedMeta(bundle.meta, fed) }));
                }
                catch {
            ...
            if (url === '/metrics') {
                ...
                try {
                    ...
                    if (!mr.ok)
                        throw new Error(`status ${mr.status}`);
                    applyCors(req, res);
                    res.writeHead(200, { 'Content-Type': 'text/plain; version=0.0.4; charset=utf-8' });
                    res.end(await mr.text());
                }
                catch {
                    writeJson(res, 200, { error: 'Metrics unavailable' });
                }
                return;
            }
            ...
            if (url === '/api/logout' && method === 'POST') {
                ...
                const logoutCookies = auth.parseCookies(typeof req.headers.cookie === 'string' ? req.headers.cookie : undefined);
            ...
            if (url.startsWith('/api/ai/')) {
                if (!assertFeature(url, 'ai', res, setCors))
                    return;
            ...
            if (url === '/api/ai/suggestions' && method === 'GET') {
                ...
                writeJson(res, 200, unavailable({ suggestions: [] }, 'AI engine not initialized — start proxy with MASTYF_AI_AI_ENABLED'));
                return;
            }
            if (url === '/api/ai/simulation-pack' && method === 'GET') {
                ...
                try {
                    ...
                    const pack = buildTenantSimulationPack(requestTenantId, simulationInputs);
            ...
            if (url === '/api/ai/report' && method === 'GET') {
                ...
                writeJson(res, 200, unavailable({ report: null }, 'AI engine not initialized'));
                return;
            }
            ...
            if (url === '/api/ai/baselines' && method === 'GET') {
                ...
                try {
                    ...
                    if (existsSync(bp)) {
                        const raw = JSON.parse(readFileSync(bp, 'utf-8'));
            ...
            if (url === '/api/ai/threats/quarantine/policy'
                ...
                try {
                    ...
                    if (!record && listSnapshot) {
                        record = {
                            id: lookupId,
                            source: (listSnapshot.source || 'custom'),
                            severity: (listSnapshot.severity || 'HIGH'),
            ...
            if (url === '/api/aggregate/audit' && method === 'GET') {
                ...
                try {
                    ...
                    try {
                        ...
                        flagged = sem.filter((r) => r.semanticAudit?.suspicious || r.label === 'true_positive').length;
                        semanticAudit = {
            ...
            if (url === '/api/security/threats/quarantine/policy'
                && (method === 'GET' || method === 'POST')) {
                setCors();
                try {
                    let threatKey = '';
                    ...
                    if (!record && listSnapshot) {
                        record = {
                            ...
                            appliedRuleName: listSnapshot.appliedRuleName
                                ...
                                : undefined,
                            policyPath: listSnapshot.policyPath ? String(listSnapshot.policyPath) : undefined,
                            enforcementStatus: (listSnapshot.enforcementStatus || 'skipped'),
                            enforcementDetail: listSnapshot.enforcementDetail
                                ? String(listSnapshot.enforcementDetail)
                                : undefined,
                            sourceKind: (listSnapshot.sourceKind || 'unknown'),
                        };
                    }
                    if (!record) {
                    ...
                    const detail = await buildMonitorQuarantinePolicyDetail(record, requestTenantId, fed.db);
                ...
                try {
                    ...
                    if (removeRule && result.records?.length) {
                        ...
                        for (const record of result.records) {
                            ...
                            const removed = removeSuggestionRuleFromPolicy(record.appliedRuleName, policyForRule, policyWatcher ?? null);
                            if (removed.removed)
                                removedRule = true;
                        }
                    }
            ...
            if (url === '/api/security/threats/quarantine' && method === 'POST') {
                ...
                try {
                    ...
                    const monitorWindow = typeof b.window === 'string' || typeof b.window === 'number'
                        ? b.window
                    ...
                    if (b.all === true || b.all === 'true') {
                        ...
                        const targets = await listBulkQuarantineTargets(fed.db, requestTenantId, monitorWindow, policyMode);
                        let quarantined = 0;
                    ...
                    const targets = await listRelatedMonitorThreatsForQuarantine(fed.db, requestTenantId, row, monitorWindow);
            ...
            if (url === '/api/security' && method === 'GET') {
                ...
                try {
                    ...
                    for (const srv of srvs) {
                        ...
                        if (sc) {
                            const row = securityRowFromScan(sc, srv);
                            reps.push({ ...row, scanned: true });
                            ts += row.score;
                            scanned += 1;
                            activeThreats += row.critical + row.high;
                            const at = sc.created_at;
                            if (at && (!lastScan || at > lastScan))
                                lastScan = at;
                        }
                        else {
                    ...
                    writeJson(res, 200, available({
                        ...
                        worstOffenders: reps.filter((r) => r.scanned && r.score != null && r.score < 50).map((r) => r.name),
                        activeThreats,
                        lastScan,
                    }));
                }
            ...
            if (url === '/api/cost' && method === 'GET') {
                ...
                try {
                    ...
                    const { repriceRecordsForDisplay, buildCostCoverage, resolveSpendHeadline, shouldShowCostHeadline, } = await import('./cost-coverage.js');
                    ...
                    for (const srv of srvs) {
                        if (srv.includes('..') || srv.includes('passwd') || srv.includes('PROBE'))
                            continue;
                        const windowRecs = windowRecords.filter((r) => r.serverName === srv && Date.parse(String(r.timestamp || '')) >= cutoff);
                    ...
                    if (totalCost != null && totalCost > 0) {
                        ...
                        if (displayCoverage.coveragePct < 50 || spanHours < 0.1) {
                            proj = 0;
                        }
                        projectedMonthly = proj > 0 ? proj : null;
                    }
                    const usedModels = Array.from(new Set(windowRecords
                        .map((r) => r.model)
                        .filter((m) => typeof m === 'string' && m.trim().length > 0 && !m.includes('deepseek'))));
                    let pricingModel = active
                        ? `${active.displayName} (${active.source})`
                        : 'per-call stored rates';
                    if (usedModels.length > 1) {
                        pricingModel = `Multi-Model Fleet (${usedModels.slice(0, 3).join(', ')}${usedModels.length > 3 ? '…' : ''})`;
                    }
                    else if (usedModels.length === 1) {
                        pricingModel = `${usedModels[0]} (litellm)`;
                    }
            ...
            if (url.startsWith('/api/analysis/full') && method === 'GET') {
                ...
                try {
                    ...
                    const db = fed.db ?? runtimeHistoryDb;
                    if (!db) {
                        writeJson(res, 200, unavailable({ analysis: null }, 'No history database — start the Mastyf AI proxy with DASHBOARD_ENABLED=true and send MCP traffic through it.'));
                        return;
                    }
                    const { buildMastyfAiFullAnalysis } = await import('../ai/mastyf-ai-full-analysis.js');
                    const analysis = await buildMastyfAiFullAnalysis(db, requestTenantId, {
            ...
            if (url.startsWith('/api/reports/mcp-health') && method === 'GET') {
                ...
                try {
                    ...
                    const db = fed.db ?? runtimeHistoryDb;
                    if (!db) {
                        writeJson(res, 200, unavailable({ report: null }, 'No history database — start the Mastyf AI proxy with DASHBOARD_ENABLED=true and send MCP traffic through it.'));
                        return;
                    }
                    const { buildMcpHealthReport } = await import('../ai/mcp-health-report.js');
                    const report = await buildMcpHealthReport(db, requestTenantId, { windowDays, useLlm });
            ...
            if (url === '/api/health' && method === 'GET') {
                ...
                try {
                    ...
                    for (const srv of srvs) {
                        const recs = await db.getCallRecordsForServer(srv, undefined, requestTenantId);
                        const callLat = recs.length > 0 ? Math.round(recs.reduce((s, r) => s + (r.durationMs || 0), 0) / recs.length) : 0;
                    ...
                    const atRisk = reps.filter((h) => (h.latency != null && h.latency > 200) || (h.successRate != null && h.successRate < 70)).map((h) => h.name);
            ...
            if (url === '/api/policy/suggestions/accept' && method === 'POST') {
                ...
                if (!rule?.name || !rule?.action) {
                    ...
                    const found = loadPendingSuggestions(requestTenantId).find((s) => s.id === String(b.suggestionId || '')
            ...
            if (url === '/api/learning/semantic/outcomes' && method === 'GET') {
                setCors();
                const { loadSemanticAuditRecordsAsync, loadSemanticAuditRecordsWithTenantFallback, SEMANTIC_AUDIT_DASHBOARD_WINDOW_MS, } = await import('../ai/semantic-audit-store.js');
                const { isSemanticAsyncEnabledForTenant } = await import('../tenant/tenant-semantic-config.js');
            ...
            if (url === '/api/learning/label' && method === 'POST') {
                ...
                if (body.semanticAuditId) {
                    ...
                    const ok = await labelSemanticAuditRecord(String(body.semanticAuditId), label, userId, requestTenantId);
                    if (!ok) {
                ...
                if (body.suggestionId) {
                    ...
                    const source = body.source || 'pattern';
            ...
            if (url === '/api/visuals/live' && method === 'GET') {
                ...
                try {
                    ...
                    if (data.meta && typeof data.meta === 'object') {
                        data.meta.dataSources = fed.dataSources;
                    }
            ...
            if (url === '/api/internal/rug-pull' && method === 'DELETE') {
                ...
                const provided = (typeof req.headers['x-mastyf-ai-internal-token'] === 'string'
            ...
            if (url === '/api/setup/mastyf-ai-config' && method === 'POST') {
                ...
                try {
                    ...
                    const patch = { upstreamUrl, listenPort };
                    const token = String(body.authToken || '').trim();
                    if (token)
                        patch.authToken = token;
                    writeSetupFile(patch);
                    writeJson(res, 200, { ok: true });
                }
                catch (e) {
            ...
            if (url.startsWith('/api/vuln-findings/') && method === 'GET') {
                ...
                try {
                    if (sub === 'export') {
                        ...
                        const { buildDisclosurePackage, prepareDisclosurePackage, readDisclosurePackageZip, } = await import('../vuln-discovery/disclosure-package.js');
                        let pkg;
                        try {
                        ...
                        writeJson(res, 200, available(pkg));
                        return;
                    }
            ...
            if (url.startsWith('/api/vuln-findings/') && url.endsWith('/analyze') && method === 'POST') {
                ...
                const id = url.split('/').filter(Boolean)[2];
                try {
                    const { analyzeFinding } = await import('../vuln-discovery/index.js');
                    ...
                    const isLlm = err instanceof Error
            ...
            if (url.startsWith('/api/vuln-findings/') && url.endsWith('/validate') && method === 'POST') {
                ...
                const id = url.split('/').filter(Boolean)[2];
                try {
                    const { validateFinding, onFindingValidated, proposeBlockFromFinding } = await import('../vuln-discovery/index.js');
            ...
            if (url === '/api/compliance/evidence/generate' && method === 'POST') {
                ...
                try {
                    ...
                    const frameworks = ['soc2', 'hipaa', 'pci-dss', 'fedramp', 'iso27001'];
                    if (!frameworks.includes(frameworkRaw)) {
            ...
            if (url === '/api/certification/registry' && method === 'GET') {
                ...
                try {
                    ...
                    const certifications = rows.map((r) => ({
                        ...
                        checks: (() => {
                            try {
                                return JSON.parse(r.checksJson);
                            }
                            catch {
                                return [];
                            }
            ...
            if (url === '/api/benchmark/submit-local' && method === 'POST') {
                ...
                try {
                    ...
                    store.saveBenchmarkSubmission({
                        ...
                        scorecardJson: JSON.stringify(body.scorecard && typeof body.scorecard === 'object' ? body.scorecard : {}),
            ...
            if (url === '/api/industry-standard/sandbox-tiers' && method === 'GET') {
                ...
                try {
                    ...
                    const certifiedTiers = certs.map((c) => ({
                    ...
                    for (const row of certifiedTiers)
                        byName.set(row.serverName, row);
            ...
            if (url?.startsWith('/api/servers/') && method === 'PATCH') {
                ...
                if (body.command !== undefined)
                    patch.command = String(body.command);
                if (body.args !== undefined)
                    patch.args = Array.isArray(body.args) ? body.args.map(String) : [];
                if (body.env !== undefined)
                    patch.env = body.env;
                if (body.transport !== undefined)
                    patch.transport = body.transport;
                if (body.url !== undefined)
                    patch.url = body.url ? String(body.url) : undefined;
            ...
            if (url === '/api/agentic/policy-gen/start-observation' && method === 'POST') {
                ...
                const c = getAgenticContainer();
                if (!c) {
                    writeJson(res, 503, unavailable({ instance: null }, 'Agentic services not initialized'));
                    return;
                }
            ...
            if (url === '/api/agentic/policy-gen/stop-observation' && method === 'POST') {
                ...
                const c = getAgenticContainer();
                if (!c) {
                    writeJson(res, 500, { error: 'Agentic services not initialized' });
                    return;
                }
                const window = c.behaviorCollector.finalizeWindow();
                if (!window) {
                    writeJson(res, 400, { ok: false, error: 'No active observation to stop' });
                    return;
                }
            ...
            if (url === '/api/agentic/policy-gen/generate' && method === 'POST') {
                ...
                if (windows.length === 0) {
                    writeJson(res, 400, { ok: false, error: 'No observation data. Start an observation first.' });
                    return;
                }
            ...
            if (url === '/api/agentic/honeypot/deploy' && method === 'POST') {
                ...
                const instance = c.honeypotManager.deploy({
                    ...
                    template: (body.template || 'decoy-api-server'),
            ...
            if (url === '/api/agentic/red-team/run' && method === 'POST') {
                ...
                writeJson(res, 200, { ok: true, attackCount: attacks.length, categories, payloadPreviews: attacks.slice(0, 5).map((a) => ({ id: a.id, category: a.category, snippet: a.payload.slice(0, 80) })) });
                return;
            }
            ...
            if (url === '/api/agentic/supply-chain/verify' && method === 'POST') {
                ...
                if (!pkg) {
                    writeJson(res, 400, { error: 'packageName required' });
                    return;
                }
                if (c) {
            ...
            if (url.startsWith('/api/agentic/tasks/') && method === 'POST') {
                ...
                if (!match) {
                    writeJson(res, 400, { error: 'Invalid task action' });
                    return;
                }
                if (!c) {
                    writeJson(res, 200, { success: false, error: 'Agentic services not initialized' });
                    return;
                }
            ...
            if (url.startsWith('/api/agentic/threat-prediction/') && method === 'GET') {
                ...
                const risk = c.riskScorer.scoreServer({ name: serverName, transport: 'stdio' }, 0, 0);
            ...
            if (url === '/api/agentic/dashboard' && method === 'GET') {
                ...
                try {
                    ...
                    const summary = await buildAgenticDashboardSummary(fed.db ?? runtimeHistoryDb ?? null, container, requestTenantId, windowDays);
                    if (fed.dataSources?.length) {
            ...
            if (url === '/api/agentic/scheduler/status' && method === 'GET') {
                ...
                const c = getAgenticContainer();
                if (!c) {
                    writeJson(res, 503, unavailable({ tasks: [] }, 'Agentic services not initialized'));
                    return;
                }
            ...
            if (url.startsWith('/api/compliance/evidence/') && method === 'GET') {
                setCors();
                const c = getAgenticContainer();
                const framework = url.split('/').pop();
                if (c) {
                    const posture = c.controlMapper.evaluate(framework, [], []);
            ...
            if (url === '/api/agentic/red-team/results' && method === 'GET') {
                ...
                const c = getAgenticContainer();
                if (!c) {
                    writeJson(res, 503, unavailable({ status: null }, 'Agentic services not initialized'));
                    return;
                }
                const attacks = c.attackGenerator.generateAllAttacks();
                writeJson(res, 200, available({ status: 'available', attackCount: attacks.length, categories: [...new Set(attacks.map((a) => a.category))] }));
                return;
            }
            ...
            if (url === '/api/agentic/supply-chain/status' && method === 'GET') {
                ...
                const c = getAgenticContainer();
                if (!c) {
                    writeJson(res, 503, unavailable({ status: null, modules: [] }, 'Agentic services not initialized'));
                    return;
                }
            ...
            if (url === '/api/agentic/trust-score/compute' && method === 'POST') {
                ...
                if (!c) {
                    writeJson(res, 503, unavailable({ score: null }, 'Agentic services not initialized'));
                    return;
                }
            ...
            if (url === '/api/agentic/dlp/scan' && method === 'POST') {
                ...
                if (!respText) {
                    writeJson(res, 400, { available: false, error: 'responseText required', dataSources: [] });
                    return;
                }
                if (!c) {
                    writeJson(res, 503, unavailable({ result: null }, 'Agentic services not initialized'));
                    return;
                }
            ...
            if (url === '/api/agentic/certification/certify' && method === 'POST') {
                ...
                if (!serverName || !packageName || !version) {
                    writeJson(res, 400, { error: 'serverName, packageName, and version required' });
                    return;
                }
            ...
            if (url === '/api/agentic/fuzzer/run' && method === 'POST') {
                ...
                const blockFn = (_m, _p) => ({ blocked: true, reason: 'No live policy decision function provided to dashboard fuzzer' });
            ...
            if (url === '/api/agentic/sla/check' && method === 'POST') {
                ...
                if (!sn || !tn) {
                    writeJson(res, 400, { error: 'serverName and toolName required' });
                    return;
                }
                if (!c) {
                    writeJson(res, 500, { error: 'Agentic services not initialized' });
            ...
            if (url === '/api/agentic/playbook/run' && method === 'POST') {
                ...
                if (!trigger || !playbook || !severity) {
                    writeJson(res, 400, { error: 'trigger, playbook, and severity required' });
                    return;
                }
                const report = c.incidentPlaybook.run(trigger, 'dashboard', severity, playbook);
                writeJson(res, 200, report);
                return;
            }
            if (url === '/api/agentic/reputation/get' && method === 'POST') {
                ...
                if (!agentId) {
                    writeJson(res, 400, { error: 'agentId required' });
                    return;
                }
                if (!c) {
                    writeJson(res, 503, unavailable({ agentId, reputation: null }, 'Agentic services not initialized'));
                    return;
                }
            ...
            if (url === '/api/agentic/harden/analyze' && method === 'POST') {
                ...
                if (!c) {
                    writeJson(res, 503, unavailable({ serverName: sn, analysis: null }, 'Agentic services not initialized'));
                    return;
                }
                writeJson(res, 200, c.configHardener.analyze({ name: sn, transport: 'stdio' }));
                return;
            }
            if (url === '/api/agentic/collusion/detect' && method === 'POST') {
                ...
                const c = getAgenticContainer();
                if (!c) {
                    writeJson(res, 503, unavailable({ alerts: [] }, 'Agentic services not initialized'));
                    return;
                }
            ...
            if (url === '/api/agentic/rl/thompson' && method === 'POST') {
                ...
                if (!c) {
                    writeJson(res, 503, unavailable({ agentId, sample: null }, 'Agentic services not initialized'));
                    return;
                }
            ...
            if (url === '/api/agentic/rl/bandit' && method === 'POST') {
                ...
                const b = await readBody(req).catch(() => ({}));
                if (!c) {
                    writeJson(res, 503, unavailable({ decision: null }, 'Agentic services not initialized'));
                    return;
                }
                ...
                if (!serverType || !agentTier || !ruleCategory) {
                    writeJson(res, 400, { error: 'serverType, agentTier, and ruleCategory required' });
                    return;
                }
            ...
            if (url === '/api/agentic/rl/sarsa' && method === 'POST') {
                ...
                const decision = c.sarsaThresholds.decide(parameter, state);
                writeJson(res, 200, decision);
                return;
            }
...
Please write a 2-3 sentence summary of what these metrics mean. Also write a short explanation for each metric in simple English. Format your response as JSON: {"summary":"overall summary","metricExplanations":{"trustScore":"...","blockedAttacks":"...","compliance":"...","sessions":"...","decoys":"...","threatMesh":"...","policyGen":"..."}}`;
            ...
            if (url === '/api/agentic/rl/reinforce' && method === 'POST') {
                ...
                const c = getAgenticContainer();
                if (!c) {
                    writeJson(res, 503, unavailable({ strategy: null }, 'Agentic services not initialized'));
                    return;
                }
    ...
    const listenPort = await new Promise((resolve) => {
        const onError = (err) => {
            if (err.code === 'EADDRINUSE') {
                Logger.warn(`[dashboard] Port ${port} already in use — proxy will run without local dashboard/WS. ` +
        ...
        if (shouldRefuseUnauthenticatedDashboardBind(bindHost, authOff)) {
            Logger.error(`[dashboard] Refusing to bind ${bindHost} while DASHBOARD_AUTH_DISABLED=true. Use DASHBOARD_BIND=127.0.0.1 or enable dashboard auth.`);
            resolve(null);
            return;
        }
    ...
    try {
        (await import('./alert-manager.js')).startAlertManager();
    }
    catch { }
    if (process.env.MASTYF_AI_LLM_ENABLED === 'true') {
        try {
            (await import('./llm-health-check.js')).startLlmHealthCheck();
        }
        catch { }
    }
    try {
    ...
    try {
        ...
        Logger.warn(`[autopilot] Service start skipped: ${err instanceof Error ? err.message : String(err)}`);
    }
    return handle;
}
/** Stop WS push loop and close the dashboard HTTP server (proxy/TUI shutdown). */
export async function closeDashboardServer() {
    const handle = activeDashboard;
    if (!handle)
        return;
