...
export function isPgbouncerConnectionUrl(databaseUrl) {
    if (!databaseUrl?.trim())
        return false;
    const lower = databaseUrl.toLowerCase();
    if (/pgbouncer|pooler/.test(lower))
        return true;
    if (/:(6432)(\/|\?|$)/.test(lower))
        return true;
    try {
        ...
        if (host.includes('pgbouncer') || host.endsWith('-pooler'))
            return true;
        if (port === '6432')
            return true;
    }
...
export function isDirectPostgresUrl(databaseUrl) {
    if (!databaseUrl?.trim())
        return false;
    if (isPgbouncerConnectionUrl(databaseUrl))
        return false;
    try {
        const normalized = databaseUrl.replace(/^postgres(ql)?:\/\//i, 'http://');
        const u = new URL(normalized);
...
export function evaluatePgBouncerStartup(ctx) {
    if (ctx.dbType !== 'postgres')
        return { action: 'none' };
    if (!ctx.databaseUrl?.trim())
        return { action: 'none' };
    const haHint = ctx.replicaCount > 1 || ctx.inK8s || (ctx.redisConfigured && ctx.replicaCount >= 1);
    ...
    if (ctx.requirePgBouncer && !viaPooler) {
        return {
            action: 'error',
            message: `[PgBouncer] MASTYF_AI_REQUIRE_PGBOUNCER=true but DATABASE_URL does not target a pooler ` +
                `(expected hostname containing "pgbouncer" or port 6432). See docs/SCALE_AND_RESILIENCE.md.`,
        };
    }
    if (direct && haHint) {
        const msg = `[PgBouncer] DATABASE_URL points at direct Postgres (:5432). ` +
...
export function checkPgBouncerAtStartup() {
    const ctx = {
...
//# sourceMappingURL=pgbouncer-check.js.map