...
import { loadThreatDiscoveryJob, patchThreatDiscoveryJob, readThreatDiscoveryLogTail, threatDiscoveryLogPath, } from './threat-discovery-job-file.js';
const SCRIPTS = {
...
const broadcastedTerminal = new Set();
function jobKey(tenantId, kind) {
    return `${tenantId}:${kind}`;
}
function loadJob(tenantId, kind) {
    return loadThreatDiscoveryJob(kind, tenantId);
}
function writeJob(tenantId, kind, patch) {
    patchThreatDiscoveryJob(kind, patch, tenantId);
}
function readLogTail(tenantId, kind, maxLines = 40) {
...
function inferJobOutcomeFromLog(kind, logTail) {
    if (!logTail.trim())
        return null;
 ...
 */
export function reconcileStaleThreatDiscoveryJob(tenantId, kind) {
    ...
    finishJob(tenantId, kind, Number(job.exitCode ?? 1), String(job.error
        || (logStale
            ? 'Discovery stalled with no progress for 25+ minutes — re-run from the dashboard'
            : 'Discovery exceeded maximum runtime — re-run from the dashboard')), { fromReconcile: true });
    return true;
}
export function getThreatDiscoveryJobStatus(tenantId = DEFAULT_TENANT_ID, kind) {
...
export function isThreatDiscoveryJobRunning(tenantId, kind) {
    const st = getThreatDiscoveryJobStatus(tenantId, kind);
    return st.state === 'running';
}
function broadcastJobEvent(tenantId, kind, type, payload) {
    broadcastDashboardEvent({
        type,
        tenantId,
        payload: { kind, tenantId, ...payload },
...
function finishJob(tenantId, kind, exitCode, error, opts = {}) {
    const key = jobKey(tenantId, kind);
...
function tickWatchers() {
    for (const key of [...watchedJobs]) {
        ...
        if (state === 'failed') {
            finishJob(tenantId, kind, Number(job.exitCode ?? 1), job.error ? String(job.error) : undefined, { skipWrite: true });
            continue;
        }
        if (state !== 'running') {
            cleanupWatcher(key);
...
function startWatcher(tenantId, kind, pid) {
    const key = jobKey(tenantId, kind);
    watchedJobs.add(key);
...
export function resumeThreatDiscoveryWatchers(tenantId = DEFAULT_TENANT_ID) {
    for (const kind of ['threat-lab', 'auto-research']) {
        reconcileStaleThreatDiscoveryJob(tenantId, kind);
        const job = loadJob(tenantId, kind);
...
function spawnDiscoveryJob(tenantId, kind, extraEnv) {
    ...
    if (child.pid)
        startWatcher(tenantId, kind, child.pid);
...
export function startThreatLabJob(tenantId = DEFAULT_TENANT_ID, opts = {}) {
...
export function startAutoThreatResearchJob(tenantId = DEFAULT_TENANT_ID) {
...
export function resetThreatDiscoveryRunnerForTests() {
    watchedJobs.clear();
    runningPids.clear();
...
//# sourceMappingURL=threat-discovery-runner.js.map