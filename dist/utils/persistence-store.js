...
export function getPersistenceStore() {
    if (!_store)
        _store = new PersistenceStore(null);
    return _store;
}
export function setPersistenceStore(store) {
    _store = store;
}
export class PersistenceStore {
    ...
    enc(s) { return isFieldEncryptionEnabled() ? (encryptField(s) ?? s) : s; }
    dec(s) { return isFieldEncryptionEnabled() ? (decryptField(s) ?? s) : s; }
    initTables() {
    ...
    getSsoConfigs(tenantId) {
        return this.ensure(() => {
            ...
            return rows.map(r => {
                try {
                    return { ...r, client_secret: decryptField(r.client_secret) ?? r.client_secret };
                }
                catch {
                    return r;
                }
    ...
    saveSsoConfig(row) {
        this.ensure(() => {
            ...
            if (existing) {
                const stmt = this.db.prepare(`UPDATE auth_idp_configs SET name=?, issuer_url=?, client_id=?, client_secret=?,
           redirect_uri=?, scopes=?, claim_mappings=?, role_map=?, enabled=?, updated_at=datetime('now')
           WHERE tenant_id=? AND id=?`);
                stmt.run(row.name, row.issuer_url, row.client_id, row.client_secret, row.redirect_uri, row.scopes, row.claim_mappings, row.role_map, row.enabled, row.tenant_id, row.id);
            }
            else {
                const stmt = this.db.prepare(`INSERT INTO auth_idp_configs (id, tenant_id, provider_type, name, issuer_url,
         client_id, client_secret, redirect_uri, scopes, claim_mappings, role_map, enabled, created_at, updated_at)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'),datetime('now'))`);
                stmt.run(row.id, row.tenant_id, row.provider_type, row.name, row.issuer_url, row.client_id, this.enc(row.client_secret), row.redirect_uri, row.scopes, row.claim_mappings, row.role_map, row.enabled);
            }
        }, undefined);
    }
    deleteSsoConfig(tenantId, id) {
        return this.ensure(() => {
    ...
    // ── Credentials ──────────────────────────────────────────────────────────
    getCredentials(tenantId, providerId, credentialType) {
        return this.ensure(() => {
            const stmt = this.db.prepare('SELECT * FROM auth_credentials WHERE tenant_id=? AND provider_id=? AND credential_type=? ORDER BY created_at DESC LIMIT 1');
            const rows = stmt.all(tenantId, providerId, credentialType);
            return rows[0] || null;
        }, null);
    }
    saveCredential(row) {
        this.ensure(() => {
            const stmt = this.db.prepare(`INSERT INTO auth_credentials (id, tenant_id, user_id, provider_name, provider_id,
         credential_type, encrypted_token, encrypted_refresh_token, scopes, expires_at, metadata, created_at)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,datetime('now'))`);
            stmt.run(row.id, row.tenant_id, row.user_id, row.provider_name, row.provider_id, row.credential_type, row.encrypted_token, row.encrypted_refresh_token, row.scopes, row.expires_at, row.metadata);
        }, undefined);
    }
    ...
    saveUserPolicy(row) {
        this.ensure(() => {
            const stmt = this.db.prepare(`INSERT OR REPLACE INTO user_tool_policies (id, tenant_id, user_id, username, roles,
         allowed_tools, denied_tools, rate_limit_per_minute, max_tokens_per_call, allowed_paths, denied_paths)
         VALUES (?,?,?,?,?,?,?,?,?,?,?)`);
            stmt.run(row.id, row.tenant_id, row.user_id, row.username, row.roles, row.allowed_tools, row.denied_tools, row.rate_limit_per_minute, row.max_tokens_per_call, row.allowed_paths, row.denied_paths);
        }, undefined);
    }
    deleteUserPolicy(tenantId, policyId) {
        return this.ensure(() => {
    ...
    getFeedSubscriptions(tenantId) {
        return this.ensure(() => {
            const stmt = this.db.prepare('SELECT * FROM threat_feed_subscriptions WHERE tenant_id = ?');
            return stmt.all(tenantId);
        }, []);
    }
    saveFeedSubscription(row) {
        this.ensure(() => {
            const stmt = this.db.prepare(`INSERT OR REPLACE INTO threat_feed_subscriptions (id, tenant_id, name, feed_url, enabled, last_sync, added_count, created_at)
         VALUES (?,?,?,?,?,?,?,?)`);
            stmt.run(row.id, row.tenant_id, row.name, row.feed_url, row.enabled, row.last_sync, row.added_count, row.created_at || new Date().toISOString());
        }, undefined);
    }
    // ── Custom Hooks ─────────────────────────────────────────────────────────
    getCustomHooks() {
    ...
    saveCustomHook(name, code, type, priority) {
        this.ensure(() => {
            const stmt = this.db.prepare('INSERT OR REPLACE INTO custom_hooks (name, code, type, priority, enabled) VALUES (?,?,?,?,1)');
            stmt.run(name, code, type, priority);
        }, undefined);
    }
    deleteCustomHook(name) {
        this.ensure(() => {
            this.db.prepare('DELETE FROM custom_hooks WHERE name = ?').run(name);
        }, undefined);
    }
    // ── Corpus Entries (growing eval payloads) ───────────────────────────────
    getCorpusEntries(verifiedOnly = true) {
        return this.ensure(() => {
    ...
    rejectCorpusEntry(id) {
        this.ensure(() => {
            const entry = this.db.prepare('SELECT block_rule FROM corpus_entries WHERE id = ? AND verified = 0').get(id);
            if (entry?.block_rule) {
                this.db.prepare('INSERT INTO corpus_rule_stats (rule_name, false_positives) VALUES (?, 1) ON CONFLICT(rule_name) DO UPDATE SET false_positives = false_positives + 1, last_updated = datetime(\'now\')').run(entry.block_rule);
            }
    ...
    getCorpusRuleStats() {
        return this.ensure(() => {
            return this.db.prepare('SELECT * FROM corpus_rule_stats ORDER BY false_positives DESC').all();
        }, []);
    }
    getHighFalsePositiveRules(threshold = 3) {
        return this.ensure(() => {
            const rows = this.db.prepare('SELECT rule_name FROM corpus_rule_stats WHERE false_positives >= ?').all(threshold);
            return rows.map((r) => r.rule_name);
        }, []);
    }
    verifyAuditChain() {
        const entries = this.ensure(() => {
    ...
    getUnverifiedCount() {
        return this.ensure(() => {
            const row = this.db.prepare('SELECT count(*) as c FROM corpus_entries WHERE verified = 0').get();
            return row?.c || 0;
        }, 0);
    }
    addCorpusEntry(entry) {
        ...
        this.ensure(() => {
            const stmt = this.db.prepare('INSERT OR REPLACE INTO corpus_entries (id, tool, args, expected_action, category, description, block_rule) VALUES (?,?,?,?,?,?,?)');
            stmt.run(id, entry.tool, entry.args, entry.expectedAction, entry.category, entry.description, entry.blockRule || null);
        }, undefined);
        return id;
    }
    appendAuditEntry(eventType, eventData) {
        const prevRow = this.ensure(() => {
