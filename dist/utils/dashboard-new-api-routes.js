...
async function triggerTribunalForUnverified() {
    const store = getPersistenceStore();
    const unverified = store.getCorpusEntries(false).filter(e => !e.verified);
    if (unverified.length === 0)
        return { ok: true, analyzed: 0, verdicts: {} };
    const verdicts = {};
    let analyzed = 0;
...
function analyzeEntryConfidence(entry) {
    try {
        ...
        if (entry.rule === 'builtin-rate-limit')
            return 'needs_review';
        if (entry.rule === 'default')
            return 'needs_review';
        if (entry.rule.includes('deny-dangerous-tools'))
            return 'verify';
        if (entry.rule.includes('semantic'))
            return 'verify';
        if (entry.rule.includes('injection'))
            return 'verify';
        if (entry.rule.includes('encoding'))
            return 'verify';
        if (entry.rule.includes('ssrf') || entry.rule.includes('url'))
            return 'verify';
        return 'needs_review';
    }
    catch {
...
function registryBadgeSvg(score, grade) {
    const colors = { 'A+': '#22c55e', 'A': '#22c55e', 'B': '#3b82f6', 'C': '#f59e0b', 'D': '#f97316', 'F': '#ef4444' };
...
export async function handleNewApiRoutes(url, method, req, res, setCors, deps) {
    // ── /api/registry/* ─────────────────────────────────────────────────────
    if (url === '/api/registry/scan' && method === 'POST') {
        ...
        const result = registryScanResult({
            packageName,
            cveCount: body.cveCount || 0,
            criticalCveCount: body.criticalCveCount || 0,
            authStrength: body.authStrength || 'none',
            transportSecurity: body.transportSecurity || 'https',
        });
    ...
    if (url === '/api/eval/payloads' && method === 'GET') {
        ...
        const dynamicPayloads = dbEntries.map(r => {
            ...
            return {
                tool: r.tool, args, expectedAction: r.expected_action,
    ...
    if (url === '/health' && method === 'GET') {
        ...
        writeJson(res, 200, { status: 'ready', uptime: process.uptime(), pid: process.pid, auditEntries: chainResult.entries || 0, blockRate: 0 });
        return true;
    }
    ...
    if (url === '/api/eval/verify' && method === 'POST') {
        ...
        const store = getPersistenceStore();
        if (body.id) {
            store.verifyCorpusEntry(body.id);
            writeJson(res, 200, { ok: true, unverified: store.getUnverifiedCount() });
        }
        else if (body.reject) {
            store.rejectCorpusEntry(body.reject);
            writeJson(res, 200, { ok: true, unverified: store.getUnverifiedCount() });
        }
    ...
    if (url === '/api/eval/run' && method === 'POST') {
        setCors();
        const body = await parseBody(req);
        const payloads = body.payloads || CORPUS_EVAL_PAYLOADS;
        if (!deps.policyEngine) {
            writeJson(res, 503, { error: 'Policy engine not available' });
            return true;
        }
        const results = await runPolicyEval(payloads, async (payload) => {
    ...
    if (url === '/api/widgets/metrics' && method === 'GET') {
        ...
        if (db?.getCallRecordsForServer) {
            try {
                ...
                blockedRequests = records.filter((r) => r.blocked).length;
            }
            catch { }
        }
        writeJson(res, 200, {
    ...
    if (url === '/api/threat-feeds/subscriptions' && method === 'POST') {
        ...
        store.saveFeedSubscription({
            id, tenant_id: deps.tenantId || 'default', name: body.name || 'Custom Feed',
            feed_url: body.feedUrl || '', enabled: body.enabled !== false ? 1 : 0,
    ...
    if (url === '/api/hooks' && method === 'POST') {
        ...
        if (!name || !code || !type) {
            writeJson(res, 400, { error: 'name, code, and type are required' });
            return true;
        }
        const hook = createCustomHook(name, code, type, priority || 50);
        if (!hook) {
            writeJson(res, 400, { error: 'Failed to create hook from code' });
            return true;
        }
        if (type === 'before')
        ...
        const store = getPersistenceStore();
        store.saveCustomHook(name, code, type, priority || 50);
    ...
    if (url === '/api/credentials' && method === 'POST') {
        ...
        const id = await credentialBroker.storeCredential({
            ...
            providerName: body.providerName || 'api',
            providerId: body.providerId || 'default',
            credentialType: body.credentialType || 'bearer_token',
            token: body.token,
    ...
    if (url === '/api/user-policies' && method === 'POST') {
        ...
        const body = await parseBody(req);
        const store = getPersistenceStore();
        const id = `up_${(body.userId || 'anon').replace(/[^a-zA-Z0-9_-]/g, '_')}_${Date.now()}`;
        store.saveUserPolicy({
            id, tenant_id: deps.tenantId || 'default', user_id: body.userId || 'anon',
            username: body.username || '', roles: JSON.stringify(body.roles || []),
            ...
            max_tokens_per_call: body.maxTokensPerCall || 5000,
        ...
        const policy = {
            userId: body.userId || 'anon', username: body.username || '',
            tenantId: deps.tenantId || 'default', roles: body.roles || [],
            allowedTools: body.allowedTools || [], deniedTools: body.deniedTools || [],
            rateLimitPerMinute: body.rateLimitPerMinute || 60,
            maxTokensPerCall: body.maxTokensPerCall || 5000,
            allowedPaths: body.allowedPaths || [], deniedPaths: body.deniedPaths || [],
        };
    ...
    if (url === '/api/user-policies/roles' && method === 'POST') {
        ...
        const body = await parseBody(req);
        const engine = getUserToolEnforcementEngine();
        const rolePolicies = body.policies || [];
        engine.registerRolePolicies(rolePolicies.map(p => ({
            roleName: p.roleName, tenantId: deps.tenantId || 'default',
            allowedTools: p.allowedTools || [], deniedTools: p.deniedTools || [],
            rateLimitPerMinute: p.rateLimitPerMinute || 60,
            maxTokensPerCall: p.maxTokensPerCall || 5000,
            allowedPaths: p.allowedPaths || [], deniedPaths: p.deniedPaths || [],
        })));
        writeJson(res, 201, { ok: true });
        return true;
    }
    if (url.startsWith('/api/hooks/') && method === 'POST') {
        ...
        if (body.action === 'disable')
            globalHookRegistry.disableHook(hookName);
        else if (body.action === 'enable')
            globalHookRegistry.enableHook(hookName);
        writeJson(res, 200, { hooks: globalHookRegistry.listHooks() });
        return true;
    }
    return false;
}
//# sourceMappingURL=dashboard-new-api-routes.js.map