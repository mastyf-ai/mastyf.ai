...
import { verifyPassword, validatePasswordAgainstPolicy } from './password.js';
import { requireAuth, requirePermission, setSessionCookies, clearSessionCookies, parseCookies, clientIp, SESSION_COOKIE_NAME, CSRF_COOKIE_NAME, CSRF_HEADER_NAME, } from './auth-middleware.js';
import { AUDIT_ACTIONS } from './rbac-types.js';
import { Logger } from '../utils/logger.js';
import { isKnownDashboardApiKey, resolveRolesForApiKey } from './dashboard-rbac.js';
...
function handleError(res, err) {
    ...
    if (statusCode >= 500)
        Logger.error(`[auth-routes] ${message}`);
...
export function registerAuthRoutes(app) {
    ...
    app.post('/api/auth/setup', async (req, res) => {
        ...
        try {
            ...
            if (adminRole)
                await roleStore.assignToUser(user.id, adminRole.id, user.id);
    ...
    app.get('/api/auth/status', async (req, res) => {
        ...
        const apiKey = (req.header('x-api-key') || bearer || '').trim();
        if (apiKey && isKnownDashboardApiKey(apiKey)) {
            const roles = resolveRolesForApiKey(apiKey);
            res.json({
                authenticated: true,
                authRequired: true,
                authConfigured: true,
                setupRequired,
                identity: 'appliance_api_key',
                dashboardRole: roles[0] ?? 'operator',
                roles,
                permissions: [],
    ...
    async function loginHandler(req, res) {
        ...
        try {
            ...
            if (!userWithHash || !passwordOk) {
                if (userWithHash) {
                    const { locked } = await userStore.recordFailedLogin(userWithHash.id, settings.lockoutPolicy.maxFailedAttempts, settings.lockoutPolicy.lockoutDurationMinutes);
                    await auditLog.write({
                        tenantId,
    ...
    async function logoutHandler(req, res) {
        try {
            ...
            if (token)
                await sessionStore.revokeByToken(token);
            if (req.authUser) {
                await auditLog.write({
    ...
    app.post('/api/users', requireAuth, requirePermission('users.manage'), async (req, res) => {
        try {
            ...
            if (parsed.data.roleIds?.length)
                await roleStore.setUserRoles(user.id, parsed.data.roleIds, req.authUser.id);
            if (parsed.data.groupIds?.length) {
                for (const groupId of parsed.data.groupIds)
                    await groupStore.addMember(groupId, user.id, req.authUser.id);
            }
            await auditLog.write({
                tenantId,
                userId: req.authUser.id,
    ...
    app.put('/api/users/:id', requireAuth, requirePermission('users.manage'), async (req, res) => {
        try {
            ...
            if (parsed.data.groupIds)
                await groupStore.setGroupsForUser(user.id, parsed.data.groupIds, req.authUser.id);
            await auditLog.write({
                tenantId,
                userId: req.authUser.id,
                username: req.authUser.username,
    ...
    app.post('/api/users/:id/reset-password', requireAuth, requirePermission('users.manage'), async (req, res) => {
        try {
    ...
    app.post('/api/users/:id/status', requireAuth, requirePermission('users.manage'), async (req, res) => {
    ...
    app.post('/api/users/:id/force-password-change', requireAuth, requirePermission('users.manage'), async (req, res) => {
        const user = await userStore.findById(String(req.params.id), req.authUser.tenantId);
        if (!user) {
            res.status(404).json({ error: 'User not found' });
            return;
    ...
    // ── Groups ────────────────────────────────────────────────────────────
    app.get('/api/groups', requireAuth, requirePermission('groups.read'), async (req, res) => {
        res.json({ groups: await groupStore.list(req.authUser.tenantId) });
    });
    app.post('/api/groups', requireAuth, requirePermission('groups.manage'), async (req, res) => {
        try {
            ...
            if (parsed.data.memberIds?.length)
                await groupStore.setMembers(group.id, parsed.data.memberIds, req.authUser.id);
            await auditLog.write({
                tenantId: req.authUser.tenantId,
                userId: req.authUser.id,
                username: req.authUser.username,
    ...
    app.put('/api/groups/:id', requireAuth, requirePermission('groups.manage'), async (req, res) => {
        try {
    ...
    app.delete('/api/groups/:id', requireAuth, requirePermission('groups.manage'), async (req, res) => {
    ...
    // ── Roles & permissions ───────────────────────────────────────────────
    app.get('/api/permissions', requireAuth, requirePermission('roles.read'), async (_req, res) => {
        res.json({ permissions: await permissionCatalog.list() });
    });
    app.get('/api/roles', requireAuth, requirePermission('roles.read'), async (req, res) => {
    ...
    app.put('/api/roles/:id', requireAuth, requirePermission('roles.manage'), async (req, res) => {
        try {
    ...
    // ── Settings ──────────────────────────────────────────────────────────
    app.get('/api/settings/auth', requireAuth, requirePermission('settings.read'), async (req, res) => {
        res.json({ settings: await authSettingsStore.get(req.authUser.tenantId) });
    });
    app.put('/api/settings/auth', requireAuth, requirePermission('settings.manage'), async (req, res) => {
