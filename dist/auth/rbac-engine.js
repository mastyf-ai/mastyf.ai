...
const DASHBOARD_TIER_ORDER = ['viewer', 'analyst', 'operator', 'admin', 'tenant-admin'];
function highestTier(tiers) {
    let best = 'viewer';
    for (const t of tiers) {
        if (DASHBOARD_TIER_ORDER.indexOf(t) > DASHBOARD_TIER_ORDER.indexOf(best))
            best = t;
    }
    return best;
}
...
export async function resolveUserAccess(user) {
    ...
    const allRoleIds = new Set([...directRoles.map((r) => r.id), ...groupRoleIds]);
    const allRoles = await Promise.all(Array.from(allRoleIds).map((id) => roleStore.findById(id, user.tenantId)));
    const resolvedRoles = allRoles.filter((r) => !!r);
    ...
    const dashboardRoles = tiers.length > 0 ? [highestTier(tiers)] : ['viewer'];
    return {
        ...user,
...
export async function userHasPermission(userId, tenantId, permission) {
    const db = await getAuthDb();
    const row = await db.get(`SELECT 1 FROM auth_role_permissions rp
     INNER JOIN auth_user_roles ur ON ur.role_id = rp.role_id
     WHERE ur.user_id = ? AND rp.permission_key = ?
     LIMIT 1`, [userId, permission]);
    if (row)
        return true;
    const viaGroup = await db.get(`SELECT 1 FROM auth_role_permissions rp
