...
async function rowToRole(row) {
    ...
    return {
        ...
        dashboardTier: row['dashboard_tier'],
...
export const roleStore = {
    ...
    async ensureSeeded(tenantId = 'default') {
        const db = await getAuthDb();
        if (db.dialect !== 'sqlite')
            return; // Postgres is seeded by migration 020-auth-rbac.sql
        ...
        if (Number(roleCount?.['c'] ?? 0) === 0) {
            for (const r of SYSTEM_ROLES) {
                const id = db.newId();
                const now = db.nowIso();
                await db.run(`INSERT INTO auth_roles (id, tenant_id, name, description, is_system, dashboard_tier, created_at, updated_at)
           VALUES (?, ?, ?, ?, 1, ?, ?, ?)`, [id, tenantId, r.name, r.description, r.dashboardTier, now, now]);
    ...
    async findByDashboardTier(tier, tenantId = 'default') {
        const db = await getAuthDb();
        const row = await db.get('SELECT * FROM auth_roles WHERE dashboard_tier = ? AND tenant_id = ? AND is_system = ' +
            (db.dialect === 'postgres' ? 'TRUE' : '1') +
            ' LIMIT 1', [tier, tenantId]);
        return row ? rowToRole(row) : null;
    },
    async create(input) {
        const db = await getAuthDb();
        ...
        const now = db.nowIso();
        await db.run(`INSERT INTO auth_roles (id, tenant_id, name, description, is_system, dashboard_tier, created_at, updated_at)
       VALUES (?, ?, ?, ?, 0, ?, ?, ?)`, [id, tenantId, input.name, input.description ?? '', input.dashboardTier, now, now]);
    ...
    async assignToUser(userId, roleId, assignedBy) {
        ...
        await db.run(db.dialect === 'postgres'
            ...
            : 'INSERT OR IGNORE INTO auth_user_roles (user_id, role_id, assigned_at, assigned_by) VALUES (?, ?, ?, ?)', [userId, roleId, db.nowIso(), assignedBy ?? null]);
    },
    ...
    async rolesForUser(userId) {
        const db = await getAuthDb();
        const rows = await db.all(`SELECT r.* FROM auth_roles r
