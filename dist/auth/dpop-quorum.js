...
export function retryDelayWithJitter(attempt, baseMs) {
    const exponential = baseMs * 2 ** attempt;
    const jitter = Math.floor(Math.random() * Math.min(exponential * 0.25, 50));
    return exponential + jitter;
}
export function parseQuorumRedisUrls() {
 ...
 */
export async function claimDpopJtiQuorum(clients, keyPrefix, jti, ttlSeconds, tenantId) {
    if (clients.length === 0)
        return false;
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
        const lockVotes = await Promise.all(clients.map((c) => c.set(lockKey, '1', 'EX', 1, 'NX')));
        ...
        try {
            ...
            if (existing.some((v) => v !== null))
                return false;
            const claims = await Promise.all(clients.map((c) => c.set(dataKey, '1', 'EX', ttlSeconds, 'NX')));
...
let quorumClients = null;
export function resetDpopQuorumClientsForTests() {
    quorumClients = null;
}
export async function getDpopQuorumClients() {
    if (quorumClients)
        return quorumClients;
    const urls = parseQuorumRedisUrls();
    if (urls.length === 0)
        return [];
    quorumClients = urls.map((url) => new IORedis(url, {
...
//# sourceMappingURL=dpop-quorum.js.map