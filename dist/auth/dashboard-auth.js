...
import { getLicenseClient, isCloudLicenseKey, } from '../license/license-client.js';
import { parseDashboardRolesEnv, resolveRolesForApiKey, resolveRolesFromSessionPayload, } from './dashboard-rbac.js';
/**
...
export class DashboardAuth {
    ...
    activeTokens;
    sessionMeta;
    apiKeyRoles = parseDashboardRolesEnv();
    ...
    constructor(config) {
        ...
        this.activeTokens = new LRUCache({ max: 10_000, ttl: sessionTtlMs });
        this.sessionMeta = new LRUCache({ max: 10_000, ttl: sessionTtlMs });
        if (this.config.enabled && this.config.apiKey) {
            ...
            Logger.error('[dashboard-auth] Auth enabled but DASHBOARD_API_KEY or DASHBOARD_JWT_SECRET is required — API requests will be rejected until configured');
        }
        ...
        if (authHeader) {
            ...
            if (bearerMatch) {
                ...
                if (isCloudLicenseKey(token)) {
                    ...
                    const licenseOk = license.matchesLicenseKey(token)
    ...
    authenticateApiKey(headers) {
        ...
        if (!presented || !this.timingSafeCompare(presented, this.config.apiKey))
            return null;
        return {
            authenticated: true,
            identity: 'api_key',
    ...
    login(req) {
        ...
        if (body.username &&
            ...
            const roles = this.sessionRolesFromLogin(body);
            const token = this.createSessionToken(tenantId, roles);
            if (priorSession) {
                void import('../audit/dashboard-access-log.js').then(({ appendSessionRotateAudit }) => appendSessionRotateAudit({ tenantId, oldToken: priorSession, newToken: token }));
            }
            StructuredLogger.info({
     ...
     */
    csrfHeadersFromForm(baseHeaders, csrfFromBody) {
        if (!csrfFromBody)
            return baseHeaders;
     ...
     */
    getLoginPageHtml(error, csrfToken) {
...
<div class="footer">Internal deployment — authorized access only</div>
     ...
     */
    createCloudSession(tenantSlug, identity, roles = ['tenant-admin']) {
        if (!this.config.jwtSecret) {
            throw new Error('Set DASHBOARD_JWT_SECRET or MASTYF_AI_CLOUD_JWT_SECRET (same value as cloud AUTH_SECRET)');
        }
     ...
     */
    createSessionToken(tenantId = DEFAULT_TENANT_ID, roles = ['viewer']) {
    ...
    sessionRolesFromLogin(body) {
        ...
        if (body.role)
            return resolveRolesFromSessionPayload({ role: body.role });
        const envRole = process.env['MASTYF_AI_DASHBOARD_LOGIN_ROLE'];
        if (envRole)
            return resolveRolesFromSessionPayload({ role: envRole });
        return ['viewer'];
    }
    /**
    ...
    loginRateKey(tenantId, ip) {
        return `tenant:${tenantId || DEFAULT_TENANT_ID}:login:${ip}`;
    }
    checkLoginRate(ip, tenantId = DEFAULT_TENANT_ID) {
