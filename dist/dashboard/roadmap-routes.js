export async function handleRoadmapApiRoutes(params) {
    ...
    if (url === '/api/agentic/policy/submit-approval' && method === 'POST') {
        ...
        container?.approvalGate.submit('policy-draft', `Approve semantic policy draft: ${rule.name}`, [{
    ...
    if (url === '/api/agentic/policy/apply-approved' && method === 'POST') {
        ...
        if (result.applied)
            markPolicyDraftApplied(requestId);
    ...
    if (url === '/api/provenance/export' && method === 'GET') {
        ...
        if (format === 'signed') {
            ...
            void import('../utils/enterprise-bootstrap.js').then(({ exportSiemEvent }) => exportSiemEvent('provenance_export', { eventCount: bundle.eventCount, merkleRoot: bundle.merkleRoot }));
    ...
    if (url === '/api/threat-model/generate' && method === 'POST') {
        ...
        const report = generateThreatModelFromConfig(configPath, activePolicies, container?.capabilityGraph);
    ...
    if (url === '/api/agentic/reputation/query' && method === 'POST') {
        ...
        const entry = useNetwork
            ? await container?.reputationNetwork.queryWithNetwork(serverName, body.packageName ? String(body.packageName) : undefined)
            : container?.reputationNetwork.queryServerReputation(serverName, body.packageName ? String(body.packageName) : undefined);
        if (body.publishToMesh) {
            const pub = await container?.reputationNetwork.publishToMeshRelay(serverName, body.packageName ? String(body.packageName) : undefined);
