...
import { TokenCounter, countAudioTokensInPayload, countImageTokensInPayload, extractModelFromPayload, } from '../utils/token-counter.js';
...
import { ProxyRequestContextStore, proxyContextTtlMs, releaseSpendReservation, withProxyRequestVault } from './proxy-request-context.js';
...
import { takeDecideLatencyMs } from '../gateway-ledger/gateway-arbiter-client.js';
import { agenticRecordCompletedToolCall, agenticRecordDeniedToolCall, } from './agentic-hooks-bridge.js';
import { recordBlockLearningEvent, fingerprintArgs, redactArgSnippets, redactArguments, ingestPolicyDecision, } from '../ai/block-learning.js';
...
import { applyToolFingerprintFromResult, persistResourceManifest, } from './tool-fingerprint.js';
...
import { checkExpandedPayload, checkRawPayloadSize, } from './payload-guard.js';
import { applyMcpResponsePipeline, mcpResponseBlockJson, runMcpPrePipeline, } from './mcp-request-pipeline.js';
...
export class McpProxyServer {
    child; // Definitely assigned in spawnChild()
    tokenCounter;
    db;
    currentRequestId = null;
    requestContexts = new ProxyRequestContextStore();
    stdoutWriter = new StdioLineWriter();
    responseTracker = new JsonRpcResponseTracker();
    sessionAuth = new ProxySessionAuthStore();
    contextTtlTimer = null;
    serverName;
    defaultTenantId;
    policyEngine;
    pendingPolicyEngine = null;
    policyEvalInflight = 0;
    tenantPolicyRegistry;
    authValidator;
    sessionCache;
    clientInputQueue = new RequestIdLock();
     ...
     */
    pendingInternalResourceListIds = new Set();
    resourcesListIssued = false;
    mcpSessionId = null;
    mcpAgentId = null;
    requestTimeoutMs;
    restartCount = 0;
    ...
    spawnArgs;
    spawnEnv;
    constructor(command, args, env, db, serverName, policyEngine, authValidator, requestTimeoutMs = 30000, maxRestarts = 5, tenantPolicyRegistry) {
    ...
    spawnChild() {
        ...
        this.child.on('exit', (code, signal) => {
            if (signal !== 'SIGTERM') {
                this.failAllPendingRequests(-32005, `Upstream MCP server '${this.serverName}' process exited (code=${code}, signal=${signal})`);
            ...
            if (this.restartCount < this.maxRestarts) {
                ...
                Logger.warn(`[proxy:${this.serverName}] Child process exited (code=${code}, signal=${signal}), restarting with backoff ${delay}ms (attempt ${this.restartCount}/${this.maxRestarts})...`);
     ...
     */
    issueProactiveResourcesList() {
        if (this.resourcesListIssued)
            return;
        if (!this.child.stdin || this.child.stdin.destroyed)
            return;
        this.resourcesListIssued = true;
        const id = `mastyf-res-${Date.now()}`;
        this.pendingInternalResourceListIds.add(id);
        try {
            ...
            Logger.debug(`[proxy:${this.serverName}] Issued proactive resources/list id=${id}`);
        }
        catch (err) {
            this.pendingInternalResourceListIds.delete(id);
            this.resourcesListIssued = false;
            const msg = err instanceof Error ? err.message : String(err);
            Logger.debug(`[proxy:${this.serverName}] proactive resources/list failed: ${msg}`);
        }
    }
    async handleStdoutLine(line) {
        try {
            const msg = JSON.parse(line);
            // Swallow proxy-issued resources/list responses (AI Access inventory).
            if (msg.id != null &&
                this.pendingInternalResourceListIds.has(msg.id)) {
                this.pendingInternalResourceListIds.delete(msg.id);
                if (msg.result?.resources && Array.isArray(msg.result.resources)) {
                    persistResourceManifest(this.serverName, msg.result.resources);
                    Logger.debug(`[proxy:${this.serverName}] Persisted ${msg.result.resources.length} resources from proactive resources/list`);
                }
                return;
            }
            ...
            if (msg.result?.tools && Array.isArray(msg.result.tools)) {
                if (msg.id != null)
                    this.pendingToolsListIds.delete(msg.id);
                ...
                applyToolFingerprintFromResult(this.rugPullState, msg.result, {
                    ...
                    onMismatch: async (_ctx) => {
                        void publishRugPullAlert(this.serverName, this.defaultTenantId, this.rugPullState.fingerprint || '');
                    },
                });
                // Inspector pattern: after tools/list, proactively capture resources/list.
                this.issueProactiveResourcesList();
            }
            // Persist resources/list for AI Access (client-initiated or opportunistic).
            if (msg.result?.resources && Array.isArray(msg.result.resources)) {
                persistResourceManifest(this.serverName, msg.result.resources);
            }
            if (msg.id != null) {
                ...
                if (reqCtx.requestMethod === 'resources/read' || reqCtx.requestMethod === 'prompts/get') {
                    ...
                    if (rp.blocked) {
                        ...
                        this.responseTracker.sendJson(this.stdoutWriter, blockJson);
                    ...
                    this.responseTracker.sendJson(this.stdoutWriter, msg);
                    this.requestContexts.delete(msg.id);
                    return;
                }
                const reqMsg = {
                ...
                if (msg?.result && !isResponseScanSkipped()) {
                    ...
                    if (inspected.redacted || inspected.blocked) {
                        recordSensitiveResponseAccess(flowSessionKey({
                            serverName: this.serverName,
                            toolName: reqCtx.requestToolName || 'unknown',
                    ...
                    if (inspected.blocked && policyMode === 'block') {
                        persistCallRecord(this.db, {
                            ...record,
                            responseTokens: 0,
                            totalTokens: record.requestTokens,
                            blocked: true,
                            blockRule: 'response-gate',
                            blockReason: inspected.blockResponse?.error
                                ?.message || 'Blocked by response gate',
                        }, reqMsg, this.spawnEnv, this.spawnArgs).catch(() => { });
                        ...
                        Metrics.requestsTotal.inc(Metrics.withTenantMetricLabels({ server_name: this.serverName, decision: 'block', authn_success: 'true' }, reqCtx.tenantId));
                        const errMsg = inspected.blockResponse?.error
                            ?.message || 'Blocked by response gate';
                ...
                persistCallRecord(this.db, record, reqMsg, this.spawnEnv, this.spawnArgs).catch((err) => Logger.debug(`Proxy: failed to store call record: ${err?.message}`));
                ...
                Metrics.proxyLatencyMs.observe(Metrics.withTenantMetricLabels({ server_name: this.serverName }, reqCtx.tenantId), proxyLatencyMs);
                Metrics.requestsTotal.inc(Metrics.withTenantMetricLabels({ server_name: this.serverName, decision: 'pass', authn_success: 'true' }, reqCtx.tenantId));
            ...
            if (orphanId != null) {
                this.sendError(orphanId, -32700, `Mastyf AI: malformed JSON from upstream MCP server`);
            ...
            Logger.debug(`[proxy:${this.serverName}] Non-JSON stdout line: ${parseErr instanceof Error ? parseErr.message : 'parse error'}`);
        }
    }
    setupStderr() {
    ...
    failAllPendingRequests(code, message) {
    ...
    resolveOrphanResponseId() {
        if (this.requestContexts.size === 1) {
            return this.requestContexts.ids()[0] ?? null;
        }
        return this.currentRequestId;
    }
    handleRequestTimeout(requestId, toolName, reqCtx, reason) {
        if (!this.requestContexts.get(requestId))
            return;
        const durationMs = Date.now() - (reqCtx.requestStartTime ?? Date.now());
        this.recordDeniedCall(toolName, reqCtx.requestTokens ?? 0, durationMs, 'request-timeout', reason, undefined, reqCtx.tenantId, requestId);
    ...
    armRequestTimeout(requestId, toolName) {
    ...
    recordDeniedCall(toolName, requestTokens, durationMs, blockRule, blockReason, requestArguments, tenantId, requestId) {
        ...
        const record = {
            ...
            tenantId: tid,
            totalLatencyMs: rid ? takeDecideLatencyMs(rid) : undefined,
            traceId: getTraceLogFields().trace_id,
            commandArgs: requestArguments ? [toolName] : undefined,
        };
        StructuredLogger.logBlocked({
            event: 'tool_blocked',
            requestId: rid ?? 'stdio-denied',
            serverName: this.serverName,
            toolName,
            reason: blockReason,
            rule: blockRule,
        ...
        const msg = rid
            ? { id: rid, method: 'tools/call', params: { name: toolName, arguments: requestArguments } }
            : undefined;
        persistCallRecord(this.db, record, msg, this.spawnEnv, this.spawnArgs).catch((err) => Logger.debug(`Proxy: failed to store denied call record: ${err?.message}`));
        const learningEvent = {
    ...
    async handleClientInput(raw) {
        ...
        try {
            ...
            if (peek.method === 'tools/call' && peek.id != null)
                requestKey = String(peek.id);
        }
        catch {
            /* non-JSON handled in processClientInput */
        }
        return this.clientInputQueue.enqueue(requestKey, () => withProxyRequestVault(raw, undefined, () => {
            let traceHeaders;
            try {
                const peek = JSON.parse(raw);
                const tp = peek.params?._meta?.traceparent;
                if (typeof tp === 'string')
                    traceHeaders = { traceparent: tp };
            }
            catch {
                // non-JSON handled in processClientInput
            }
    ...
    async processClientInput(inboundRaw) {
        let raw = inboundRaw;
        // ── Payload size guard ──────────────────────────────────
        ...
        try {
            ...
            if (msg.method === 'tools/call' && hasJsonRpcId(msg.id)) {
                ...
                if (isProxyInflightExceeded(this.requestContexts.size)) {
                    Metrics.proxyInflightRejectedTotal.inc(Metrics.withTenantMetricLabels({ server_name: this.serverName }, this.defaultTenantId));
                    this.sendError(msg.id, -32005, `Mastyf AI: proxy overloaded (${this.requestContexts.size}/${maxInflightEarly} in flight)`, { rule: 'proxy-max-inflight' });
                    Metrics.requestsTotal.inc({
                        server_name: this.serverName,
                        decision: 'block',
                        authn_success: 'false',
                ...
                if (this.rugPullState.blocked
                    ...
                    this.recordDeniedCall(toolName, 0, Date.now() - proxyStartTime, 'tool-fingerprint-mismatch', 'Tool definitions changed mid-session (rug-pull detected)', undefined, tenantForRug, msg.id);
                ...
                const requestModel = extractModelFromPayload(msg) || resolveModelId();
                const reqEstimate = this.tokenCounter.countWithProvider(raw, requestModel)?.tokens ??
                ...
                let requestArguments = msg.params?.arguments;
                if (requestArguments !== undefined) {
                    const { runToolCallPreForwardGuard, toolCallGuardBlockResponse } = await import('./tool-call-pre-guard.js');
                    const preGuard = await runToolCallPreForwardGuard(this.serverName, toolName, requestArguments, requestId, {
                        agentId: this.mcpAgentId ?? undefined,
                        meta,
                        mcpSessionId: this.mcpSessionId ?? undefined,
                    });
                    if (preGuard.blocked) {
                        this.recordDeniedCall(toolName, requestTokens, Date.now() - proxyStartTime, 'payload_or_agentic', preGuard.message || 'Pre-forward guard blocked', requestArguments, requestTenantId, msg.id);
                        const blockResp = toolCallGuardBlockResponse(msg.id, preGuard);
                        this.responseTracker.sendJson(this.stdoutWriter, blockResp);
                        return;
                    }
                    if (preGuard.arguments) {
                        requestArguments = preGuard.arguments;
                        if (msg.params)
                            msg.params.arguments = requestArguments;
                ...
                if (requestArguments !== undefined) {
                    ...
                    if (!expandedGuard.ok) {
                        this.recordDeniedCall(toolName, requestTokens, Date.now() - proxyStartTime, 'payload-expanded-limit', expandedGuard.reason, requestArguments, requestTenantId, msg.id);
                ...
                if (requestArguments) {
                    ...
                    if (multimodalFindings.length > 0 && this.policyEngine?.getMode() === 'block') {
                        ...
                        this.recordDeniedCall(toolName, requestTokens, Date.now() - proxyStartTime, 'multimodal-injection', mmReason, requestArguments, requestTenantId, msg.id);
                    ...
                    if (secretFindings.length > 0) {
                        ...
                        Logger.warn(`[proxy:${this.serverName}] 🔑 SECRET DETECTED in arguments of '${toolName}': ${secretSummary}` +
                            (secretFindings.length > 5 ? `... (+${secretFindings.length - 5} more)` : ''));
                        StructuredLogger.info({
                            event: 'secret_in_args_detected',
                            serverName: this.serverName,
                            toolName,
                        ...
                        if (isProxyEntropyCheckEnabled(this.policyEngine?.getMode()) &&
                            ...
                            if (entropyFindings.length > 0) {
                                ...
                                this.recordDeniedCall(toolName, requestTokens, Date.now() - proxyStartTime, 'arg-entropy', entropyReason, requestArguments, requestTenantId, msg.id);
                        ...
                        if (this.policyEngine?.getMode() === 'block') {
                            ...
                            this.recordDeniedCall(toolName, requestTokens, Date.now() - proxyStartTime, 'secret-scan', dlpReason, requestArguments, requestTenantId, msg.id);
                            this.sendError(msg.id, -32001, `Blocked by MCP Mastyf AI policy: ${dlpReason}`, { rule: 'secret-scan', policy: 'block' });
                            StructuredLogger.info({
                                event: 'request_denied',
                                requestId,
                                serverName: this.serverName,
                ...
                let pendingRotatedSessionToken;
                // ── OAuth 2.1 JWT validation ────────────────────────
                if (this.authValidator) {
                    const msgAuth = OAuthValidator.extractAuthFromMcpMessage(msg);
                    const authHeader = this.sessionAuth.getAuthHeader(this.mcpSessionId, msgAuth, stickySessionAuth);
                    ...
                    if (!token) {
                        ...
                        if (!result.valid) {
                            ...
                            const dpopProof = extractDpopProof({
                                metaAuth: msg.params?._meta?.auth,
                                messageDpop: typeof msg.DPoP === 'string' ? msg.DPoP : undefined,
                            });
                            const dpopUri = `mcp://${this.serverName}/tools/call`;
                            const dpopCheck = await validateRequiredDpop(dpopProof, 'POST', dpopUri, token, requestTenantId, this.policyEngine?.getMode());
                            if (!dpopCheck.valid) {
                ...
                let blockReason;
                let spendReservationId;
                const engine = this.tenantPolicyRegistry?.getEngine(requestTenantId) ?? this.policyEngine;
                if (engine) {
                    const tenantId = requestTenantId;
                    const idempotencyKey = idempotencyKeyFromRequest(msg.params?._meta);
                    ...
                    if (!defense.allowed) {
                        ...
                        this.recordDeniedCall(toolName, requestTokens, Date.now() - proxyStartTime, defense.rule, defense.reason, requestArguments, requestTenantId, requestId);
        ...
        try {
            ...
            if (fwd.method === 'tools/call' && fwd.id) {
                ...
                await withMcpToolCallSpan({
                    serverName: this.serverName,
                    toolName: ctx?.requestToolName || fwd.params?.name || 'unknown',
    ...
    async evaluatePolicyPinned(engine, context) {
