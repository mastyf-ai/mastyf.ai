...
import { createStreamingEconomicsState, inspectStreamingEconomicsChunk, } from './streaming-economics.js';
import { injectIntoUpstreamHeaders, runWithExtractedTraceAsync, withMcpToolCallSpan, } from './trace-context.js';
import { isRpcBatch, validateMcpMessage, } from './http-proxy-security.js';
export class StreamableHttpProxyServer {
    ...
    async start() {
        ...
        await new Promise((resolve, reject) => {
            ...
            this.httpServer.listen(this.opts.listenPort, () => {
                ...
                Logger.info(`[streamable-http:${this.opts.serverName}] Listening on http://127.0.0.1:${this.boundPort}/mcp → ${this.opts.upstreamBaseUrl}`);
                resolve();
            });
        });
        return this.boundPort;
    ...
    async handleRequest(req, res) {
        ...
        return withProxyRequestVault(body, req.headers, async () => {
            let messages;
            try {
            ...
            if (validationError && messages[0].method === 'tools/call') {
                const { jsonRpcErrorBody } = await import('./json-rpc-utils.js');
                res.writeHead(400, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify(jsonRpcErrorBody(messages[0].id, -32600, validationError)));
                return;
            }
            const responses = [];
    ...
    async processMessageTraced(msg, req) {
        const { runMcpPrePipeline, applyMcpResponsePipeline, mcpResponseBlockJson } = await import('./mcp-request-pipeline.js');
        ...
        const upstream = msg.method === 'tools/call'
            ? await withMcpToolCallSpan({
                ...
                tenantId: msg._defenseTenantId ?? relayTenantId,
                spendReservationId: msg._spendReservationId,
            }))
        ...
        fingerprintJsonRpcToolsList(this.rugPullState, upstream, this.opts.serverName, fpTenant, `[streamable-http:${this.opts.serverName}]`);
        const rotated = msg._rotatedSessionToken;
        if (rotated) {
            injectRotatedSessionIntoResult(upstream, rotated);
        }
        if (msg.method === 'tools/call' && upstream.result != null) {
        ...
        if (!pre.blocked && pre.trackResponse && pre.requestMethod && upstream.result != null) {
            ...
            if (rp.blocked) {
                return mcpResponseBlockJson(msg.id, rp.reason ?? 'Resource/prompt blocked');
            }
            if (rp.result !== undefined) {
                upstream.result = rp.result;
            }
        }
        return upstream;
    }
    async relayToUpstream(body, req, opts) {
        ...
        const injectedHeaders = await credentialBroker.injectCredentialIntoHeaders(tenantId, this.opts.serverName, 'bearer_token', headers);
        ...
        return new Promise((resolve) => {
            ...
            const clientReq = (isHttps ? httpsRequest : httpRequest)(reqOpts, (upstreamRes) => {
                const parts = [];
                const streamEcon = createStreamingEconomicsState(opts?.tenantId ?? 'default', opts?.spendReservationId);
    ...
    async maybeBlockMessage(msg, req, fleetCtx) {
        if (msg.method !== 'tools/call' || !this.opts.policy)
            return null;
        let tenantId;
        let authenticated = false;
        let jwtTenantId;
        let agentSub = fleetCtx?.agentId;
        ...
        if (token && this.opts.authValidator) {
            const dpopCheck = await validateRequiredDpop(extractDpopProof({ headerDpop: req.headers['dpop'] }), 'POST', `https://streamable/${this.opts.serverName}/mcp`, token, tenantId, this.opts.policy.getMode());
            if (!dpopCheck.valid) {
                return {
                    jsonrpc: '2.0',
                    id: msg.id,
        ...
        if (!defense.allowed) {
            releaseProxyInflight(this.opts.serverName);
            const phaseLabel = defense.phase === 'semantic' ? 'semantic gate' :
        ...
        if (rotatedSessionToken) {
            msg._rotatedSessionToken = rotatedSessionToken;
        }
        msg._defenseTenantId = tenantId;
        msg._spendReservationId = spendReservationId;
        return null;
    }
}
//# sourceMappingURL=streamable-http-proxy-server.js.map