import { rankSemanticReviewQueue } from './semantic-active-learning.js';
...
function parsePersonaJson(text, persona) {
    try {
        const match = text.match(/\{[\s\S]*\}/);
        if (!match)
            return null;
        const parsed = JSON.parse(match[0]);
        const stance = parsed.stance === 'block' ? 'block' : parsed.stance === 'allow' ? 'allow' : 'neutral';
        return {
            persona,
            stance,
...
const PERSONA_PROMPTS = {
    block_advocate: 'You argue this MCP tool call should be BLOCKED. Cite semantic categories, sync decision, and confidence.',
    allow_advocate: 'You argue this MCP tool call is BENIGN. Cite low confidence, missing attack indicators, or false-positive patterns.',
    auditor: 'You synthesize both advocates. State recommended label (true_positive, false_positive, or needs_review) and dissent if any.',
};
export async function applyTribunalAutoLabels(debates, opts) {
    if (process.env.MASTYF_AI_TRIBUNAL_AUTO_LABEL !== 'true')
        return 0;
    const { labelSemanticAuditRecord } = await import('./semantic-audit-store.js');
    let applied = 0;
    for (const d of debates) {
        if (!d.autoLabelEligible)
            continue;
        const label = d.verdict.recommendedLabel;
        if (label !== 'true_positive' && label !== 'false_positive')
            continue;
...
function heuristicArgument(rec, persona) {
...
function buildVerdict(args) {
    ...
    let recommendedLabel = 'needs_review';
    ...
    const dissent = blockWins && allow.confidence >= 0.5
...
export async function runTribunalDebate(rec, opts) {
    ...
    if (useLlm) {
        ...
        if (llm.isAvailable()) {
            for (const persona of ['block_advocate', 'allow_advocate', 'auditor']) {
                ...
                if (result?.text) {
                    ...
                    const stance = persona === 'block_advocate' ? 'block' : persona === 'allow_advocate' ? 'allow' : 'neutral';
                    args.push({
                        persona,
                        stance,
...
export async function buildTribunalReport(opts) {
    const { debates, queueSize, batchLimit, eligibleTotal, remainingEligible } = await runTribunalForQueue(opts);
    const cfg = getQuorumConfig();
    ...
    Logger.info(`[SwarmTribunal] Report: ${debates.length} debate(s), eligible=${eligibleTotal}, remaining=${remainingEligible}, autoLabels=${autoLabelsApplied}`);
    return {
        generatedAt: new Date().toISOString(),
        queueSize,
...
//# sourceMappingURL=swarm-debate-tribunal.js.map