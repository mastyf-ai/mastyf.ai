...
export class PatternRecognizer {
            ...
            if (sec.score < 30) {
                ...
                if (costReport && costReport.estimatedCostUSD > 0) {
                    insights.push({
                        ...
                        severity: 'critical',
                        description: `${sec.serverName} has critically low security score (${sec.score}) but active cost (${costReport.estimatedCostUSD.toFixed(4)}). High risk: spending on a vulnerable server.`,
     ...
     */
    detectTemporalPatterns(snapshot) {
        const hourly = new Map();
        for (let h = 0; h < 24; h++)
            hourly.set(h, { calls: 0, totalTokens: 0, tools: new Set() });
     ...
     */
    analyzeServerRelationships(snapshot) {
        const relationships = new Map();
        const sorted = [...snapshot.callRecords].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
    ...
    correlateVulnStackSignals(input) {
        ...
        for (const cve of input.packageCves) {
            const related = input.upstreamEvents.filter((u) => u.relatedServer === cve.serverName ||
        ...
        for (const add of input.toolWatchAdds || []) {
            const authFlip = input.upstreamEvents.find((u) => u.relatedServer === add.serverName &&
                u.statusFrom === 401 &&
                u.statusTo === 200);
            if (authFlip) {
                insights.push({
