 ...
 */
import { appendLearnedRule, validateLearnedRule, computeLearnedRuleFingerprint, listLearnedRules, reloadLearnedRules, } from '@mastyf_ai/core';
...
function savePromotionState(state) {
    const path = promotionStatePath();
...
function recordPromotion(fingerprint, ruleId) {
...
function extractPattern(discovery) {
...
export function classifyLearnedRuleTarget(discovery, source) {
...
function buildProbe(discovery, target) {
    if (target === 'argument') {
        const rawArgs = discovery.corpusCandidate.arguments ?? {};
        const safeArgs = snapshotAuditArguments(rawArgs);
        const leaves = flattenStringLeaves(safeArgs ?? rawArgs);
        const usable = leaves.filter((s) => s.trim().length >= 8 && !/\[REDACTED\]/.test(s));
        const probe = usable.find((s) => s.trim().length >= 8)
...
function mapSeverity(discovery) {
    return discovery.confidence >= 0.85 ? 'critical' : 'warning';
}
function buildDraft(discovery, provenance) {
    const regex = extractPattern(discovery);
    if (!regex)
        return null;
    if (isDangerousUnblockPattern(discovery.policyRule.name, regex))
        return null;
...
function queuePendingLearnedRule(discovery, draft, provenance) {
...
export function promoteDiscoveryToCoreRules(discovery, provenance) {
    ...
    Logger.info(`[core-rule-promoter] promoted ${appended.rule.id} (${draft.target}, conf=${provenance.confidence})`);
    return { ok: true, status: 'promoted', ruleId: appended.rule.id };
}
//# sourceMappingURL=core-rule-promoter.js.map