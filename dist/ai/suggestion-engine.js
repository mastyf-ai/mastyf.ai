import { registerDataCollector, recordPolicyDecisionGlobal } from './data-collector.js';
...
import { ComprehensiveReporter } from './comprehensive-reporter.js';
...
export class SuggestionEngine {
    ...
    servers = [];
    analysisTimer = null;
    constructor(collector, baselineLearner, costOptimizer, threatIntel, policyAssist, patternRecognizer, selfImprovement, config) {
    ...
    async runLearningCycle() {
        ...
        if (this.config.enabledModules.includes('baseline')) {
            ...
            const preventiveSuggestions = anomalySuggestions.length === 0
        ...
        if (isAiAutoApplyEnabled() && !this.selfImprovement.isThresholdAdjustmentFrozen()) {
            ...
            const toApply = allSuggestions.filter((s) => {
                ...
                if (s.confidence < minConfidence)
                    return false;
                if (!s.autopilot?.safetyAllowed)
                    return false;
                if (s.autopilot?.recommendation === 'hold')
                    return false;
        ...
        const report = this.reporter.generateFullReport(snapshot, baselines, insights, temporal, autoRuleNames, this.selfImprovement.getState(), pruneList);
    ...
    saveComprehensiveReport(report) {
        try {
            ...
            Logger.debug(`[SuggestionEngine] Failed to save report: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    ...
    recordUserOutcome(suggestionId, action, meta) {
        this.selfImprovement.recordOutcome({
            suggestionId,
    ...
    processPolicyGoal(goal, availableTools) {
        ...
        try {
            const rulesYAML = suggestions
                ...
                if (s.rule.description)
                    lines.push(`    description: "${s.rule.description}"`);
                lines.push(`    action: ${s.rule.action}`);
                if (s.rule.maxTokens)
                    lines.push(`    maxTokens: ${s.rule.maxTokens}`);
                if (s.rule.maxCallsPerMinute)
                    lines.push(`    maxCallsPerMinute: ${s.rule.maxCallsPerMinute}`);
                if (s.rule.tools?.deny?.length || s.rule.tools?.allow?.length) {
                    const toolLines = ['    tools:'];
                    if (s.rule.tools?.deny?.length)
                        toolLines.push(`      deny: [${s.rule.tools.deny.join(', ')}]`);
                    if (s.rule.tools?.allow?.length)
                        toolLines.push(`      allow: [${s.rule.tools.allow.join(', ')}]`);
...
export async function ensureAiEngineInitialized(historyDb, servers) {
...
export function removePendingSuggestion(suggestionId, opts) {
    try {
        ...
        body.suggestions = (body.suggestions ?? []).filter((s) => s.id !== id && (!ruleName || s.ruleName !== ruleName));
        if (body.suggestions.length === before)
            return false;
        ...
        Logger.debug(`[SuggestionEngine] Failed to remove pending suggestion: ${err instanceof Error ? err.message : String(err)}`);
        return false;
    }
}
/** Read persisted pending suggestions (fast — no learning cycle). */
export function loadPendingSuggestions(tenantId) {
    try {
...
export async function initializeAiEngine(historyDb, servers) {
    ...
    const useDbSnapshots = process.env.MASTYF_AI_AI_USE_DB_SNAPSHOTS !== 'false'
    ...
    if (useDbSnapshots) {
        ...
        collector = new DataCollector(historyDb, new SecurityScanner(), costAuditor, new HealthMonitor(historyDb), pricingClient);
    }
    ...
    const engine = new SuggestionEngine(collector, baselineLearner, costOptimizer, threatIntel, policyAssist, patternRecognizer, selfImprovement, engineConfig);
...
export function serversFromNames(names) {
    return names.map((name) => ({ name, transport: 'stdio' }));
}
/** Run learning cycle against the history database (proxy, TUI, scan hooks). */
export async function runLearningCycleForDb(historyDb, servers) {
    if (!isAiLearningEnabled())
        return null;
...
export async function triggerLearningCycleIfEnabled(historyDb, servers = [], opts) {
    if (!isAiLearningEnabled())
        return;
    if (opts?.cliCommand && !isAiLearningOnCliCommands())
        return;
    await runLearningCycleForDb(historyDb, servers);
}
export { recordPolicyDecisionGlobal };
/** Roll back AI learning state to the latest snapshot (CLI / dashboard API). */
export function rollbackAiLearning() {
    if (globalEngine) {
        return globalEngine.getSelfImprovement().rollback();
    }
    return new SelfImprovement().rollback();
}
export async function recordSuggestionOutcome(suggestionId, action, meta) {
