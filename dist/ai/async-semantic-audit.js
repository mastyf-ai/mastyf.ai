...
import { isLocalSemanticEnabled, scoreLocalSemanticRisk, } from './local-semantic-classifier.js';
...
import { isSemanticCircuitOpen, tryBeginSemanticLlmProbe, recordSemanticLlmFailure, recordSemanticLlmSuccess, } from './semantic-circuit-breaker.js';
import { allowSemanticLlmCall, reportSemanticAuditSkipped, } from './semantic-llm-rate-limit.js';
...
import { getRedisSemanticQueueDepth, isRedisSemanticQueueEnabled, tryAcquireRedisSemanticSlot, releaseRedisSemanticSlot, warnLocalSemanticQueueCapsIfNeeded, } from '../utils/redis-semantic-queue.js';
...
let llmByModel = new Map();
let stats = { processed: 0, flagged: 0, dropped: 0 };
let reserveInFlight = 0;
export function isSemanticAsyncEnabled(tenantId) {
    return isSemanticAsyncEnabledForTenant(tenantId);
}
/** @internal test helper */
export function resetSemanticAuditStateForTests() {
    queue.length = 0;
    processing = false;
    if (debounceTimer)
...
function shouldStoreCalibrationRecord() {
    return process.env.MASTYF_AI_SEMANTIC_STORE_CALIBRATION === 'true';
}
async function persistSemanticAudit(job, result, meta) {
    try {
        ...
        Logger.debug(`[async-semantic] Failed to persist audit record: ${err instanceof Error ? err.message : String(err)}`);
        return null;
    }
}
/** Wait for debounced async semantic queue (used by swarm live scenario before proxy exit). */
export async function flushSemanticAuditQueue(maxWaitMs = 15000) {
    const started = Date.now();
    let spins = 0;
    const maxSpins = 20_000;
    while (spins++ < maxSpins) {
        if (Date.now() - started >= maxWaitMs && Date.now() !== started)
            break;
        if (reserveInFlight > 0) {
...
export function enqueueSemanticAudit(job) {
    reserveInFlight += 1;
    void reserveAndEnqueue(job).finally(() => {
        reserveInFlight = Math.max(0, reserveInFlight - 1);
    });
}
async function reserveAndEnqueue(job) {
    if (!isSemanticAsyncEnabled(job.tenantId))
        return;
...
async function runLocalSemanticAudit(job) {
    ...
    if (score.risk >= MIN_CONFIDENCE || shouldStoreCalibrationRecord()) {
        await persistSemanticAudit(job, result, { model: 'local-semantic' }).then((stored) => {
            if (!stored || score.risk < MIN_CONFIDENCE)
                return;
            setImmediate(() => {
...
Categories: prompt-injection, exfiltration, privilege-escalation, encoded-payload, none.`;
        ...
        if (!cachedText) {
            try {
                ...
                if (isEmbeddingCacheEnabled()) {
                    embeddingHit = await getEmbeddingCache().findNearest(job.serverName, job.toolName, job.arguments);
                    if (embeddingHit) {
        ...
        if (cachedText) {
            ...
            if (response) {
                ...
                try {
                    response = await withSemanticTimeout('async_semantic_audit', () => getLlm(job.tenantId).generate(systemPrompt, userPrompt), null);
                    Metrics.recordSemanticScanDuration('async_audit', Date.now() - auditStart, response?.text ? 'ok' : 'no_llm');
                    if (response?.text) {
                        recordSemanticLlmSuccess(job.tenantId);
                        ...
                        try {
                            ...
                            if (isEmbeddingCacheEnabled() && typeof parsed.suspicious === 'boolean') {
                                await getEmbeddingCache().store(job.serverName, job.toolName, job.arguments, { suspicious: Boolean(parsed.suspicious), confidence: typeof parsed.confidence === 'number' ? parsed.confidence : 0, categories: Array.isArray(parsed.categories) ? parsed.categories : [], reasoning: String(parsed.reasoning || '') }, response.model);
                            }
                        }
        ...
        await persistSemanticAudit(job, result, {
            ...
            setImmediate(() => {
                ...
                if (result.suspicious) {
                    void import('../alerting/soar-playbooks.js').then(({ runSoarPlaybooks }) => runSoarPlaybooks({
                        event: 'semantic_flag',
                        toolName: job.toolName,
                        serverName: job.serverName,
...
function broadcastSemanticComplete(job, result) {
    broadcastDashboardEvent({
...
export function buildSemanticAuditJob(ctx, syncDecision) {
    return {
...
//# sourceMappingURL=async-semantic-audit.js.map