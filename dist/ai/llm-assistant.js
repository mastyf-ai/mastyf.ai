...
export function resolveOllamaBaseUrl(explicit) {
...
function classifyHealthFailure(err) {
    const text = err instanceof Error ? err.message : String(err || '');
    if (/timed out|timeout|aborted/i.test(text))
        return 'timeout';
    if (/ENOTFOUND|EAI_AGAIN|dns/i.test(text))
        return 'dns';
    if (/ECONNREFUSED|fetch failed|connect/i.test(text))
        return 'connect';
    return 'unknown';
}
function probeWithHttpClient(endpoint, timeoutMs = 5000) {
    return new Promise((resolve) => {
        try {
            const target = new URL(`${endpoint}/api/tags`);
            const requester = target.protocol === 'https:' ? httpsRequest : httpRequest;
            const req = requester(target, { method: 'GET', timeout: timeoutMs, headers: { Accept: 'application/json' } }, (res) => {
...
function ollamaThinkEnabled(model) {
    if (process.env.MASTYF_AI_LLM_OLLAMA_THINK === 'true')
        return true;
    if (process.env.MASTYF_AI_LLM_OLLAMA_THINK === 'false')
        return false;
...
export class LlmAssistant {
    ...
    async generate(systemPrompt, userPrompt) {
        ...
        for (let attempt = 0; attempt <= this.config.maxRetries; attempt++) {
            try {
                ...
                const budgetMs = this.config.hotPath === false
     ...
     */
    async generatePolicyRule(goal, availableTools) {
...
Never wrap the JSON in markdown code blocks. Output ONLY valid JSON.`;
        const result = await this.generate(systemPrompt, `Generate a policy rule for: ${goal}`);
        if (!result)
            return null;
        try {
...
Be direct and specific. No disclaimers or suggestions.`;
        const userPrompt = `Anomaly detected on "${params.serverName}" using tool "${params.toolName}":
...
Never wrap the JSON in markdown code blocks. Output ONLY valid JSON.`;
        const userPrompt = `Analyze CVE-${params.cveId} (${params.severity}):
...
Focus on interconnected risks and top-priority actions. Be direct.`;
        const insightText = insights
            ...
            .join('\n');
        const result = await this.generate(systemPrompt, `Synthesize these cross-layer governance insights:\n\n${insightText}`);
        return result?.text || null;
    }
...
//# sourceMappingURL=llm-assistant.js.map