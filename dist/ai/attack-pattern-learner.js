export function attackMinBlocks() {
...
export function attackMinConfidence() {
...
function inferArgPatterns(reasons, toolName) {
    ...
    const patterns = frags.slice(0, 5).map((f) => {
        ...
        if (f.includes('SELECT'))
            return 'SELECT\\s+.*\\bFROM\\b';
        return escapeRegex(f);
    });
    return [{ field, patterns }];
}
/** Build one attack suggestion from a group of blocked records (same rule+tool). */
export function suggestFromBlockedGroup(blockRule, toolName, recs) {
    const minBlocks = attackMinBlocks();
    if (recs.length < minBlocks)
        return null;
 ...
 */
export function learnAttackPatterns(records) {
    const blocked = records.filter((r) => r.blocked && r.blockRule);
    if (blocked.length < attackMinBlocks())
        return [];
...
//# sourceMappingURL=attack-pattern-learner.js.map