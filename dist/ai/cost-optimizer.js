export class CostOptimizer {
    ...
    costAuditor;
    budgetCap;
    constructor(db, costAuditor, budgetCap) {
        this.db = db;
    ...
    async analyzePatterns(records, inputPricePerM, outputPricePerM) {
        ...
        for (const [, group] of grouped) {
            ...
            const trend = ratio > 1.3 ? 'increasing' : ratio < 0.7 ? 'decreasing' : 'flat';
            patterns.push({
    ...
    async detectBurstPatterns(records) {
        const bursty = new Map();
        const callsPerBucket = new Map();
        for (const r of records) {
            const key = `${r.serverName}:${r.toolName}`;
            if (!callsPerBucket.has(key))
                callsPerBucket.set(key, new Map());
    ...
    suggestRules(patterns, burstMap) {
        const suggestions = [];
        for (const p of patterns) {
