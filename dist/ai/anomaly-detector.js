...
export function getAnomalyDetector() {
    if (!singleton)
        singleton = new AnomalyDetector();
    return singleton;
}
export function resetAnomalyDetectorForTests() {
    singleton = null;
}
export class AnomalyDetector {
     ...
     */
    scoreArgumentLayer(criticalCount, warningCount, maxConfidence, categories) {
     ...
     */
    scoreBaselineLayer(profile, currentIssues, currentCritical) {
     ...
     */
    scoreIntentLayer(session, currentSuspicious, anomalyScore) {
        if (!session) {
            return {
     ...
     */
    recordCall(serverName, toolName, argIssues, criticalIssues, warningIssues, maxConfidence, categories, anomalyScore, wasBlocked) {
        if (!this.isLicensed())
            return;
        ...
        if (this.profiles.size > 1000) {
            const oldest = [...this.profiles.entries()].sort((a, b) => a[1].lastUpdated - b[1].lastUpdated)[0];
            if (oldest)
                this.profiles.delete(oldest[0]);
        }
        const sample = {
            timestamp: Date.now(),
            argIssues,
    ...
    trackSessionCall(sessionKey, tenantId, toolName, suspicious, anomalyScore) {
     ...
     */
    async evaluate(serverName, toolName, criticalCount, warningCount, maxConfidence, categories, sessionKey, tenantId) {
        if (!this.isLicensed()) {
            return {
                confidence: 0,
                layers: {
        ...
        this.recordCall(serverName, toolName, argScore.totalIssues, criticalCount, warningCount, maxConfidence, categories, argScore.score, false);
        // Weighted fusion using exponential combination
        const weightedSum = argScore.score * LAYER_WEIGHTS.argument +
        ...
        let primaryLayer = 'none';
        const contributions = [
            { layer: 'argument', value: argScore.score * LAYER_WEIGHTS.argument },
            { layer: 'baseline', value: baselineScore.score * LAYER_WEIGHTS.baseline },
            { layer: 'intent', value: intentScore.score * LAYER_WEIGHTS.intent },
        ];
        ...
        if (confidence > 0.5) {
            Logger.warn(`[anomaly] High anomaly score (${confidence.toFixed(3)}) for ${serverName}/${toolName} ` +
