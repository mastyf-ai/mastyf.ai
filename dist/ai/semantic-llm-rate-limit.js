...
export function getAsyncMaxPerMin() {
    return getMaxPerMin({ async: true });
}
export function reportSemanticAuditSkipped(reason, tenantId) {
    semanticAuditSkippedTotal.inc({
        reason,
        tenant_id: tenantId?.trim() || DEFAULT_TENANT_ID,
    });
}
export function getSemanticLlmMaxUsdPerMin(opts) {
...
export function getAsyncMaxUsdPerMin() {
    return getSemanticLlmMaxUsdPerMin({ async: true });
}
function localBucketKey(tenantId, kind) {
    return `${tenantId}:${kind}`;
}
function getLocalBucket(tenantId, kind) {
...
async function checkRedisLimits(tenantId, estimatedUsd, opts) {
    ...
    if (maxUsd > 0 && estimatedUsd > 0) {
        ...
        const usdResult = await rl.checkAndIncrement(usdKey, maxMicroUsd, WINDOW_MS, tenantId, microUsd);
        if (!usdResult.allowed)
            return false;
    }
    const countKey = 'semantic-llm';
    const countResult = await rl.checkAndIncrement(countKey, maxPerMin, WINDOW_MS, tenantId);
    return countResult.allowed;
}
export async function allowSemanticLlmCall(tenantId, opts) {
    ...
    if (countBucket.count >= maxPerMin)
        return false;
    const maxUsd = getSemanticLlmMaxUsdPerMin(opts);
    if (maxUsd > 0 && estimatedUsd > 0) {
        const usdBucket = getLocalBucket(tid, usdKind);
        if (usdBucket.usd + estimatedUsd > maxUsd)
            return false;
...
/** @internal */
export function resetSemanticLlmRateLimitForTests() {
    localBuckets.clear();
}
//# sourceMappingURL=semantic-llm-rate-limit.js.map