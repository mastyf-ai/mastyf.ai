...
import { readAutoCorpusManifest, setAutoCorpusManifestStatus, setAllPendingAutoCorpusStatus, } from './auto-corpus-writer.js';
...
const PROMOTION_STATE_PATH = join(process.env.MASTYF_AI_THREAT_RESEARCH_STATE_PATH || join(homedir(), '.mastyf-ai'), 'corpus-promotions.json');
...
function loadPromotionManifest() {
    ...
    try {
        return JSON.parse(readFileSync(PROMOTION_STATE_PATH, 'utf-8'));
    }
    catch {
        return { timestamp: new Date().toISOString(), count: 0, entries: [] };
    }
}
function savePromotionManifest(manifest) {
...
function isAlreadyPromoted(advId) {
    const manifest = loadPromotionManifest();
    return manifest.entries.some((e) => e.advId === advId);
}
// ── Category Mapping ─────────────────────────────────────────────────
const ATTACK_CLASS_TO_CORPUS_DIR = {
...
function categoryFromAttackClass(attackClass) {
    return ATTACK_CLASS_TO_CORPUS_DIR[attackClass] || 'edge-cases';
}
// ── Manifest Update ──────────────────────────────────────────────────
function incrementManifestCategory(category) {
...
// ── Policy Verification ──────────────────────────────────────────────
async function verifyFixtureBlocked(toolName, args, expectedBlockRule) {
    try {
        ...
        const policyConfig = load(readFileSync(policyPath, 'utf-8'));
...
export async function promoteToCorpus(discovery, provenance) {
    ...
    const verifyResult = await verifyFixtureBlocked(candidate.toolName, candidate.arguments ?? {}, candidate.ruleHint);
    if (!verifyResult.blocked) {
        Logger.warn(`[auto-corpus-promoter] Fixture ${provenance.advId} not blocked by current policy: ${verifyResult.reason || 'unknown'}`);
        return {
            ok: false,
    ...
    Logger.info(`[auto-corpus-promoter] Promoted ${provenance.advId} → ${relPath} ` +
...
export async function promoteBatchToCorpus(discoveries) {
    const results = [];
...
export function listPendingPromotions() {
...
export function approveAutoCorpusEntry(advId) {
    ...
    const entry = manifest.entries.find((e) => e.advId === advId);
    if (!entry)
        return { ok: false, error: writer.error || 'Entry not found' };
    entry.status = 'approved';
    savePromotionManifest(manifest);
    return { ok: true };
}
/** Export: mark an entry as rejected */
export function rejectAutoCorpusEntry(advId) {
...
export function resetPromotionStateForTests() {
    if (existsSync(PROMOTION_STATE_PATH)) {
...
//# sourceMappingURL=auto-corpus-promoter.js.map