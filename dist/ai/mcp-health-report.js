...
import { filterRecordsInWindow, parseWindowDays, windowRangeMs, windowToLabel, } from '../utils/time-buckets.js';
import { LlmAssistant } from './llm-assistant.js';
import { Logger } from '../utils/logger.js';
import { existsSync, readFileSync } from 'fs';
import { load } from 'js-yaml';
function isHealthReportLlmEnabled(useLlm) {
...
function windowLabelForSummary(windowDays) {
...
async function buildServerSections(db, tenantId, windowDays) {
...
async function topBlockRuleLabels(db, tenantId) {
    const srvs = await getAllActiveServerNames(db, tenantId);
...
function computeVerdict(atRisk, blocked, total) {
    if (atRisk.length >= 2 || (total > 0 && blocked / total > 0.5))
        return 'critical';
    if (atRisk.length > 0 || blocked > 0)
        return 'attention';
    return 'healthy';
}
function formatMarkdown(report) {
    ...
    for (const b of report.executiveSummary)
        lines.push(`- ${b}`);
    lines.push('');
    ...
    if (report.securityPosture.topBlockRules.length) {
        ...
        for (const r of report.securityPosture.topBlockRules)
            lines.push(`  - ${r}`);
    }
    lines.push('');
...
async function buildLlmNarrative(report) {
    ...
    try {
        const result = await llm.generate('You explain MCP (Model Context Protocol) server health to a non-technical operator. Use ONLY the cited facts. Write 3-5 short paragraphs in plain English. Do not invent numbers.', `Facts:\n${corpus}\n\nWrite a plain-language health briefing:`);
        if (!result?.text?.trim())
            return null;
...
export async function buildMcpHealthReport(db, tenantId, opts) {
    if (!db)
        return null;
    ...
    const traffic = readSwarmJsonFile('traffic-summary.json', tenantId);
    const userServers = readSwarmJsonFile('user-servers-session.json', tenantId);
    const executiveSummary = [];
    if (summary.totalRequests > 0) {
        executiveSummary.push(`${summary.totalRequests.toLocaleString()} tool calls through Mastyf AI in the last ${windowDays} days.`);
        executiveSummary.push(`${summary.blockedRequests.toLocaleString()} were blocked (${summary.passRatePct}% allowed).`);
    }
    else {
...
//# sourceMappingURL=mcp-health-report.js.map